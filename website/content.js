...
window.KEEL = {
  meta: {
    ...
    installAlt: "pip install \"git+https://github.com/berkayturanci/keel@v1.3.0\"",
  ...
  docs: [
    ...
    },
    {
      ...
      body:
        ...
        "<p>Two scope limits follow: keel-visual watches <b>one repo at a time</b> (<code>dash</code> enumerates that project's worktrees via <code>git worktree list</code>; it doesn't aggregate across separate repos — run one instance per project), and it sees a run only on the <b>same filesystem</b> it reads — a run on this machine (your <code>keel ship</code> or a local agent) is visible, but one executing on a different machine (a remote/cloud session) writes its records there, not here.</p>",
