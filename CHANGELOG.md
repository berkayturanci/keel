...
- **s7 probes the jury panel before it dispatches it, and a project can allow a recorded host-bench fallback when it cannot be staffed** (#1066). On a tier whose `knobs.team.review.by_tier` names `jury`, the panel *is* the review, and #1014 round 3 deliberately made it so no operator flag can take it back off. That is right while the panel can run. When it cannot — an agent CLI not installed, unauthenticated, or out of quota — the tier had no way forward at all: `keel evidence-verify --pr N` reported `missing: jury-verdict` against a panel the machine could not convene, and nothing an operator could type would change it.
  ...
  - **A surface that only verifies is pinned to what the ship measured, not to its own machine.** Recording the measurement is not by itself enough: `keel evidence-verify` and `keel merge` run wherever CI puts them, so re-probing there rewrote the contract of a change reviewed somewhere else — a panel project whose ship measured two vendors requires the panel verdict, and the same pull request checked on a bare runner measured one, dropped the panel item and demanded `review-verdict-1..3` from a host bench nobody sat. Both surfaces now take the ship's decision: first from the `ship_run` ledger entry written for **this head** of that pull request at `run_context.jury_panel`, which carries fallbacks too so a fallback-reviewed change is not held to a panel a workstation *could* have convened; and failing that from a head-pinned `keel.jury-verdict.v1` posted on the pull request (the panel sat, and the ballots prove it from the one place a hosted runner can read — the run ledger and the jury artifact both live under the gitignored `.keel/state/`). **The ledger outranks the posted verdict** (#1068 round 5): the ledger records what this run actually did, a comment records what somebody put on the pull request, and with the verdict ranked first a run that had shipped under the fallback — three host reviewers, recorded in its own ledger at this exact head — had that contract replaced by a leftover or collaborator-posted verdict and dropped `review-verdict-1..3` from the required set. A same-head record that says nothing about a panel is the run speaking too: it did not ship under one, so no comment may say otherwise on its behalf, and the surface measures instead. The whole precedence now lives in one function, `juryavail.pin`, rather than in the order of two `if`-statements at a call site. Both sources are pinned to the current head, and for the same reason: a pull request outlives its heads, so a ship of an earlier head that fell back would otherwise still be answering for the head being verified now — a stale run relaxing a live gate. A record from any other head is no record at all, and the surface measures for itself instead, which refuses either way round. Only with neither source does a verification surface probe, as it did before. `availability.source` publishes which it was — `probe`, `pull-request` or `run-ledger` — and a pinned record carries `probed: false` whichever of the two it came from, because "we were told" and "we checked" are not the same claim. **Neither source is read without a head.** The two pins were hardened one at a time and diverged: the ledger pin refused a blank head while the posted-verdict pin fell through to `evidence._matches_head`, which reads a blank `head_sha` as *no head filter* — right for counting evidence inside a gate that is head-agnostic throughout, wrong for a pin, which takes `review-verdict-1..3` off the required set outright. Any trusted jury marker anywhere on the pull request therefore counted as this head's on a run that could not resolve one (a detached checkout, an API answer with no `headRefOid`). One predicate, `juryavail.is_pinnable_head`, is now asked once above both sources: with no head the surface pins nothing and measures its own machine, the same answer a record from another head already got. `evidence._matches_head` keeps its permissive reading and now says why it differs. The residue is narrow and fails closed: a fallback-shipped change verified with no readable ledger on a machine that *can* staff the panel is held to the panel it did not run. Two machines can still *plan* the same tier differently, and each still says which it did.
