...
- **The jury is the tier-3 review panel: one dispatch, mapped onto the s7 verdicts** (#1015): s7 (N reviewers) and s8 (the `jury` gate) were separate mechanisms and `evidence.required_items` demanded *both* at tier-3 — three distinct reviewer verdicts **and** a jury verdict. A tier-3 pull request therefore paid for three host reviewers plus a four-agent panel reading the same diff, while the panel's per-reviewer ballots (which *are* the cross-vendor review) reached no gate at all: ai-jury posted one consensus comment and kept the ballots in its JSON report. On a tier whose `knobs.team` policy is `review: jury`, s7 now dispatches the panel **once** and its ballots become the review evidence.
  ...
  - **The downgrade changes what gates, never who reviews.** A panel below `MINIMUM_JURY_VENDORS` participating vendors is still downgraded `gating → advisory`, exactly as before — and the bench does **not** move with it. `reviewers.source` on a panel tier is always `jury`, because the participating-vendor count reaches the six review-aware commands as unevenly as the jury flags do (only `evidence-verify` and `keel merge` can read a posted verdict), so a bench that followed it would have `keel plan` requiring the panel's ballots while `evidence-verify` demanded a host bench of the same PR — the disagreement #1014 exists to prevent, along a new axis. Fewer participating vendors therefore buys no fewer eyes in the only way that survives every surface: the ballots stay required in full, and the thin vendor span is refused by `panel_vendor_check` instead.
  ...
  - **keel does not adopt the policy in the change that ships it.** `projects/keel.yaml` keeps its three explicit tier-3 seats: on a panel tier nothing can take the panel back off (#1014, round 3), so the switch commits every tier-3 keel PR to a jury run and is made deliberately, once `keel review --from-jury` has been exercised on a real pull request. `"3": jury` with a gating panel stays the documented example in `configuration.md#team` and the fixture the cross-surface tests run.
