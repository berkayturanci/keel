...
- **Ship ledgers now report the branch's net changed files after a base merge** (#1174). `keel ship` prefers the fetched `origin/<base_branch>` ref for its three-dot diff, so files introduced by merging an updated base branch are not counted as this run's changes; offline repositories fall back to the configured local base ref.
