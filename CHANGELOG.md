...
- **`keel review --from-jury` no longer invents substance for empty or ABSTAIN ballots** (#1150). `ballot_scope` always opened with `Checked the changed-file diff as ai-jury panelist …`, which is keel's own `verdict_substance` escape hatch, so an abstention still passed the gate by construction. Schema ≥1.2 `counts_as_review` / `scope_substantive` / `scope` / `testing` / `abstention_cause` were ignored. Abstentions entered `Panel.ballots`, inflated `panelists` / `jury_panel_size`, and could be posted as `review-verdict-*`.
  ...
  - **And the mirror of it: a review that rendered as an abstention.** Withholding the `checked …` clause from *every* ballot with no finding path caught the ballot the fix is not about — a ≥1.2 record declaring `counts_as_review: true` with no `scope` prose, which is the shape of a seat that read the diff and found nothing. `ballot_is_review` admitted it to the panel and `ballot_scope` then described it as not having reviewed, and `verdict_substance` refuses that verdict: the posting path would have failed on the one ballot every consumer most expects to succeed. `verdict_substance` accepts an explicit `checked …` clause precisely so a clean review stays expressible without inventing a file reference, so a ballot that *is* a review gets one. Whether a ballot counts is decided in one place; the scope line reports that decision instead of making a second one. The rendered body is fed back through `verdict_substance` in the test, because the contradiction was only visible at the gate.
