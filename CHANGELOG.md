...
### Added
- **A Cursor listing manifest, and a guard so the next one cannot drift** (#1139). keel shipped two per-agent manifests; in Cursor the plugin installs but carries no logo or category and renders as a bare repository name.
