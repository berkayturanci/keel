...
### Added
- **Retrieved learnings reach the implement and review briefs** (#1155). `capture.retrieve_relevant_learnings` had been shipped, documented and tested since the capture contract was written, and **nothing in `src/keel` called it**. Every learning keel wrote was write-only: an agent implementing issue N never saw that issue N-40 hit the same trap, so the capture half of the backbone produced artifacts no step consumed. `keel ship` now reads the learning directories at s3 and hands the hits to `assess` the way it hands over the jury-availability probe — the assessment stays pure, and one rendered **Relevant past learnings** section reaches both the s4 implement brief and every s7 reviewer brief.
  - **`policy_pack.capture.learning.source`** names the directories, and unset it is the one the sink writes to — a project that turned capture on has exactly one place its learnings live, and a second setting to keep in step with the first is a second setting to get wrong. A list reads several (a repo-local folder plus a shared cross-project one) and they are ranked together, so a shared folder's best lesson can outrank a weak local one.
  - **The placeholder set is smaller than the sink's, deliberately.** `{pr}`, `{date}` and `{slug}` name one *document*; a directory naming a single PR would retrieve that PR's lesson and nothing else. They are refused in a `source` — and a sink path that uses one is skipped as a default source rather than read as a folder literally called `{pr}`.
  - **Front matter is matched exactly, and outweighs prose.** A file declaring `changed_files: [src/keel/ledger.py]` is *about* a task touching that path; one that merely says "ledger" eight times is worded like it, and text scoring alone ranked the second above the first. Files without front matter are plain Markdown and score on their text, so a lesson keel wrote and a lesson a person wrote both rank.
  - **The diff is empty on the run that most needs a lesson.** Retrieval happens before s4 has written a line, so scoring against `git diff` matched on the title alone. `--declared-file` — what the run *states* it will touch — is the only file list that exists that early, and it is what makes a path match possible at all.
  - **The plan contract carries it, because that is the one s4 reads.** Retrieval was wired into `keel ship`, and the adapter composes the implement brief from what `keel plan` printed — s5 is where ship runs. So the brief this whole change exists for saw no lesson on the path the adapter actually takes, and `review_merge_contract.reviewers.past_learnings` was always empty there. Both contracts carry the same block now, and a plan with nothing to retrieve carries an empty one rather than omitting the key.
  - **And the contract's own field names are not prose in any suffix.** Subtracting the `---` block covers Markdown; the reader also opens `.json` and `.txt`, which carry no delimiter, so `schema: keel.learning.v1` and `repo: keel` were counted as words again — the same false positive, in the suffixes the reader promises to open.
  - **Scoring subtracts keel's own scaffolding.** Every document this writer produces carries the same three headings, so counted as prose they are a match every file shares: an issue titled *"What changed in the merge window"* scored `what` and `changed` against all three learnings and opened the brief with three unrelated lessons. Measured through `keel ship`.
  - **A declaration outranks any amount of prose.** Added into one total, `min(count, 5)` per query token compounds while the exact-match bonus does not — so on the multi-word query `keel ship` actually builds, a file repeating those words led the brief over the one that *declared* the path the task touches. Declared matches are the first sort key; text only orders ties, in the reader and in the CLI's cross-directory re-sort.
  - **And admission is two distinct query words, not a point floor.** The ship path applied its own threshold while the unit test used the function's default, so the test passed against a rule no real run used and a handwritten note that is not named after its own words never reached a brief. One rule now: repetition is not evidence.
  - **Scoring reads the body, not the document.** Front matter carries `repo:` and `schema: keel.learning.v1`, so scoring it as prose made every learning this repository has ever written match every task in it. Measured: three learning files, one relevant, and two of them came back.
  - **No learnings means no section** — not an empty heading, not a note that none were found. A project with an absent or empty directory gets exactly the brief it got before this existed and pays one `is_dir()` for it. What *was* surfaced is recorded on the ledger as `capture.retrieved`, so a later run can tell a lesson nobody had from one that was put in front of the implementer and still not applied.
