...
### Fixed
- **A verdict that names a symbol without calling it is a review, not a receipt** (#1106). `verdict_substance`'s anchor set read a dotted identifier only *with* the parentheses, so a gate verdict on ai-jury#753 that traced `cache.cache_key`, `config.config_hash`, `JuryConfig.__post_init__` and `adapters.GenericCLIAdapter._prompt_mode` through eleven symbols, four call sites and three test classes was told it "names nothing concrete". Every anchor missed for the same reason — the reviewer wrote `cache.cache_key`, and adding parentheses to something you are not calling is worse prose. A gate that refuses genuine review over punctuation teaches the operator to reword verdicts until the checker is happy, which is the receipt-shaped behaviour #926 exists to stop; and a verdict is head-pinned, so the only remedies were deleting a real review or re-running the seat and hoping it phrased things differently. Three shapes are read now: a dotted identifier with no parentheses (`module.symbol`, `Class.method`, both sides two characters so "e.g." is not one), several *distinct* bare identifiers (`collect_static_hints`, `_prompt_mode`, `TheHintsBlockIsPartOfTheKey`), and an escape clause opened by any of six verbs — `checked`, `traced`, `read`, `ran`, `inspected`, `verified` — whose object may sit on the next line as a bullet, because "Checked:" with the list under it is the same clause laid out.
  - **Measured against the record, not asserted.** Replayed over all 1,323 review verdicts posted across keel and ai-jury (384 pull requests): every one of the 816 that passed still passes, 202 of the 507 refused now pass, and the number of pull requests carrying at least one accepted verdict rises from 254 to 326. Of the 75 verdicts in that corpus opening `Reviewed <title>…`, 68 stay refused, and all ten renderings of the default `render_review_verdict` template stay refused.
  - **A bare identifier anchors only when several appear**, because ordinary domain words share the shape: "GitHub" is CamelCase and names nothing. The floor is two *distinct* ones — at one, the corpus says the rule would have admitted 18 of the 75 rubber stamps #926 is named for; at two it admits none.
  - **"Reviewed" is not one of the verbs and will not be.** It is the first word of the #926 receipt and of the default template's "Scope reviewed:" line; on the record, admitting it would have passed all 75 stamps and all ten templates. "Confirmed" and "assessed" are out for the weaker version of the same reason — they report a conclusion, not an act of inspection.
  - **The extensionless path the issue proposed was refuted by the corpus and is not shipped.** A bare `word/word` token was meant to read `docs/CHANGELOG` and `.github/workflows`; replayed, it read `1/3`, `2/3`, `I/O`, `offline/live` and `build/lint/bandit` — slash-separated alternatives are simply how these reviewers write — matching 305 of the 507 refused verdicts on prose that points at nothing. A stricter two-slash variant was no better in kind and reached only nine verdicts no other anchor did; `docs/CHANGELOG` in the verdict this issue is about is carried by the dotted and bare-identifier anchors anyway. `tests/test_verdict_substance.py::AnExtensionlessPathIsNotAnAnchor` records the decision so it is not re-proposed from the issue text alone.
  - **The two halves stay independent, which is what lets the anchor set be generous.** An anchor asks whether the reviewer pointed at anything; `_VERDICT_NOVELTY_FLOOR` asks whether the prose is the pull request title said again. A verdict that names a symbol *and* is otherwise the title restated still fails the second, so widening the first cannot readmit the #926 shape by itself — pinned by a test that restates a title full of identifiers. The called-identifier pattern is kept beside the new dotted one rather than replaced by it, because it also reads single-character segments: nothing that anchored a verdict before this change stops anchoring one.
