...
- **An outside sink's artifact reference says it is machine-local** (#1185). `capture.artifact` is recorded relative to `--root` for a sink inside the checkout and absolute for one outside it — a shared `~/knowledge` folder — and the run ledger is **committed**, so that absolute path travels to teammates and CI runners where it names nothing. The duplicate reuse then handed a later run a path it could not read, dropped it, recorded `applied` with no artifact, and `keel capture-verify` reported `applied-without-artifact` — the finding reserved for a file that is genuinely missing — for a file sitting exactly where it was written. The capture block now records `artifact_scope: repository | machine` beside `artifact`, describing the **sink**: where this project writes, not where one path happens to point. A project with no sink at all records no scope, which is a third answer and not `machine`. `learning_sink_in_worktree` cannot answer this: it is False for an outside sink, for no sink **and** for a switched-off capture, because it answers *who commits the file* — a question the hook does gate. The path's shape is not gated on anything, so the two now share one path test (`_sink_writes_in_worktree`) and ask it separately; a dormant in-repo sink had been recording `machine` for a path every clone can see. `capture-verify` reports a **note** for a `machine`-scoped record (`applied-elsewhere`, counted, listed beside the findings in the human output as well as in `--json`, never failing) and keeps `applied-without-artifact` for a record whose project could have written where this host can read. Dropping the unreadable path stays in the CLI, not in `capture.duplicate_learning_artifact`: the pure layer cannot see whose host it is on, and refusing a `machine` path there would record `applied` with no artifact on the very machine where the file is real — raising the finding the reuse exists to prevent. A record written before the field existed is read from the path's own shape — anchored on any platform means outside — so nothing already in a ledger changes meaning.
