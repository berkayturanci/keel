...
- **54 tests never ran when their file was run as a script** (#1141). `unittest.main()` calls `sys.exit()`, so a class defined *below* `if __name__ == "__main__"` is never reached by `python tests/test_cli.py` — it is not skipped, it does not exist yet. `unittest discover`, which imports the module, sees all of it, so CI stayed green while the loop a person actually uses (`PYTHONPATH=src python tests/test_cli.py -k delegate`) quietly omitted whole classes. A test that does not run cannot fail.
  - **Five files, measured before and after.** `tests/test_cli.py` 548 → 586 (the delegate CLI wiring and the TDD-order gate), `tests/test_api_delegate.py` 60 → 65, `tests/test_ship.py` 99 → 102, `tests/test_runner.py` 38 → 40, and `keel-visual/tests/test_runstate.py` 80 → 86. No test body changed; the sentinel moved to the end of each file.
