...
### Fixed
- **The extensions docstring names every slot that may block, not `pre-merge` alone** (#1100). `src/keel/extensions.py`'s module docstring is the closest thing this repo has to a spec for the frontmatter mini-language it parses, and it said `on_fail: block` was "valid only in the `pre-merge` slot". What `parse_extension` actually enforces is `model.py`'s `may_block` flag, which `guard` (s3), `tester` and `test` (s8) carry too. A *documented* restriction the code does not have is the expensive direction to be wrong in: an author who wants a blocking `guard` reads that line, believes the slot cannot block, and files a bug or builds a workaround for a restriction that was never there — against a backbone whose whole point is that the slot list and its properties are knowable from the documentation. Every other surface already named the four (`docs/keel/extensions.md`'s rule and its `may block?` column, `docs/proposals/keel-architecture.md`, `AGENTS.md`); the docstring was the only one that had drifted, which is why no reader comparing two documents would have caught it.
