...
- **A Rebased PR Could Never Be Merged Again** (#945):
  ...
  - Narrowing is limited to the capture-status branch of `ledger._is_merged_ship_run`; the assessment branch is independent and untouched, so a run whose gates passed still reads as merged through its `merge` recommendation. The tests assert both halves so the scope of the change cannot be misread as broader than it is.
