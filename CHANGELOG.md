...
### Added
- **`keel capture-land` — the learning file reaches the base branch** (#1163). `policy_pack.capture.learning.sink` (#1154) writes one Markdown learning per applied capture, and with the default relative path — `.keel/learning/`, which keel dogfoods — the file landed in the working tree untracked and keel stopped there: s2 cuts the next worktree from `origin/<base_branch>`, every CI runner clones fresh, and s10's pre-clean deletes the worktree outright. So keel wrote a lesson on every merge and threw it away, and #1155's reader — the whole reason the sink exists — found an empty directory on the only path a later run takes. `durable_artifacts.commit_required` already answered *whether* the file had to be committed; nothing answered *how*, and `durable_artifacts.land_command` now names what does. The command runs with **`--root .`**, not the worktree: s10's pre-clean has already removed that directory and the s11 ledger append writes under the primary checkout, so pointed at the worktree it read an empty history, reported `no-artifact` and exited 0 — a green s11 that lands nothing. **A refusal is not contention.** git exits 1 for both, so the two are told apart by its own words: `fetch first` / `non-fast-forward` means the branch moved and rebuilding will work, and anything else — a protected branch, a declining `pre-receive` hook — refuses again, so it stops after one attempt and reports what the server said instead of claiming the branch *"moved under every one of 3 attempt(s)"*. That case is not hypothetical here: this repository's `main` requires pull requests with `enforce_admins: true`, so keel writes its own lessons and does not land them, and `configuration.md` says so rather than calling an in-repo sink durable unconditionally. Containment is checked on the **resolved** path — `git hash-object` follows symlinks and the exactly-one-file check counts paths, not where their bytes came from — and the path refusal is applied to the artifact as written *and* after backslashes become slashes, because those disagree: `\etc\hostname` is drive-relative rather than absolute, so no `PurePath` calls it anchored, and the rewrite made it `/etc/hostname`, which `os.path.join(root, …)` resolves by discarding the root. Two encoding faults that only Windows and non-ASCII names reach: subprocess output is decoded as UTF-8 with `surrogateescape` rather than the ANSI code page, which crashed any repository holding a Cyrillic filename, and the diff read passes `-c core.quotePath=false … -z`, without which a non-ASCII artifact came back C-quoted and the safety check refused it forever. The landing is confined to the **configured sink**, not to the repository: every other path test asks whether git could address the path, and the answer is yes for `config/private.env` as much as for a lesson, so a ledger record naming one would have fast-forwarded the base branch with it while the exactly-one-file check agreed. Containment is compared by path component, so `.keel/learning-notes/` is outside `.keel/learning`.
  - **Plumbing, not a checkout, because the obvious recipe cannot run here.** "Switch to the base branch, pull, add, commit, push" fails on the topology keel uses for itself: s2, `overnight` and `swarm` all execute s0–s12 inside a worktree while the primary checkout holds `base_branch`, so `git switch <base>` there exits 128 with *'<base>' is already used by worktree* — measured on this repository, and the reason three rounds of hardening on #1160 went into a recipe that could never have executed. The commit is built with `hash-object` / `ls-tree` / `mktree` / `commit-tree` against `<remote>/<base_branch>` and the base branch is never checked out, so the same command runs unchanged from a worktree, from the primary checkout and from a fresh CI clone. The tree composition is a **pure string function** (`capture.upsert_tree_entry`), so "this commit adds exactly one file" is asserted offline rather than only in a live push, and it orders entries the way git does — a directory sorts as if its name ended in `/`, which puts `learning.md` before the `learning/` tree.
  - **It is not a merge and does not touch one.** One commit carrying one file; no pull request, no merge claim, no merge window. `keel merge` at s10 remains the only path a pull request takes to the base branch, which is the invariant the issue was written around.
  - **Two ships finishing s11 at once both land theirs.** The push is a plain fast-forward, never forced: a rejection means another ship pushed first, so the commit is rebuilt on the branch as it now is and pushed again, bounded by `--attempts` (default 3). Forcing would have discarded the other ship's lesson and whatever else arrived with it. Exercised against a real bare origin with a genuine race, not a stub.
  - **Re-running lands nothing twice.** Identical content reports `already-landed` and pushes nothing, so a resumed or retried s11 does not add a commit per attempt.
  - **It refuses to push what it did not check.** The finished commit is verified to differ from its parent by *exactly* the artifact path before the push, and an unreadable diff fails closed rather than reading as "changes nothing". An artifact path that is absolute or climbs out of the checkout (`../x`, `/etc/x`, `C:\x`, a UNC share) is refused rather than normalised — this is the one input whose destination is a shared branch. The commit message is consumer-neutral and carries no vendor trailer: it lands on every consumer's base branch and keel cannot know whose co-authorship to stamp on it.
  - **Fail-soft, like every capture step.** `landed`, `already-landed`, `not-required` (a sink outside the checkout, which git never sees) and `no-artifact` all exit 0, because s11 runs after the merge already happened and a capture with nothing to do must not fail it; only `failed` exits non-zero, and it is reported rather than rolled back. `/keel:ship`'s s11 carries the recipe and the six adapter surfaces are regenerated; `docs/keel/configuration.md` no longer says an in-repo sink is undurable, and keel's own `projects/keel.yaml` comment no longer says the file is thrown away.
  - **The three lessons already written by #1169, #1170 and #1171 land with this change**, which is what the mechanism is for: they had been stranded on a session branch since the runs that wrote them.
- **Past learnings reach implementation and review briefs** (#1155). `keel plan` and `keel ship` retrieve matching lessons from `policy_pack.capture.learning.source`, which accepts one directory or a list and defaults to the sink directory. Before implementation, repeat `keel plan --declared-file PATH` for expected files; the adapter carries the same **Relevant past learnings** section into both briefs. Declared labels and paths outrank prose, while text matches require two distinct query words. YAML block and flow lists are supported, and ordinary prose beginning with `Issue:` remains searchable. Empty results add no section; results are capped at five and a 4,000-character brief budget. The run ledger records retrieved fingerprints in `capture.retrieved`.

- **`knobs.loop` / `--loop` — a bounded, gate-verified s4 iteration loop that composes with `implement_mode: tdd`** (#1165). An s4 implementer got exactly one pass; when the gates came back red after it, nothing sent the work back to the seat that wrote it — s9's fix loop reads review findings and s6's budget is for a pushed branch — so a red `make test` after a single pass fell to the host or blocked an issue two iterations from green. The industry answer, the Ralph loop, re-feeds the same prompt until the agent says it is done; keel keeps the loop and replaces the judge. After each implement iteration the orchestrator runs the command gates: green ends the loop, red starts the next iteration with the same brief plus the gate output, up to `max_iterations` (1–10, default 3). **The completion criterion is the gate run, never the implementer's text**: a delegate that says it is done with red gates is an iteration failing, and a red run at the last iteration is `budget-exhausted`, which blocks the issue and exits non-zero from `keel loop brief`, the shape `keel fixloop brief` already uses. It is a policy around the s4 profile, not a third `implement_mode`, so it composes with `tdd` — wrapping phase B only, since phase A's red run is its proof — and a third enum value would have made the two mutually exclusive, when a loop against tests the implementer has to satisfy is the combination worth having. The loop judges the gates it can make green — `keel run-gates --defer-jury --json`, new in this change, reports the guard- and test-phase gates of kind `command` or built-in with the plan beside the outcomes — and defers the rest: an agentic Lego the command runner never executes, the jury under `--defer-jury`, a `pre-merge` gate. A deferred gate is listed in the brief, never counted as green and never holds the loop open, so a project with a blocking agentic tester can still reach `done` and no iteration convenes a panel. Found by the round-1 reviews: the first cut fed the loop `keel ship --json`, whose gate outcomes carried neither `on_fail` nor `not_run`, so a failing soft gate held the loop open and an unrun blocking gate read as a pass — the outcome document now carries both; `keel loop brief` resolves the policy exactly as `keel ship` did (a loop that is off is a refusal and its `source` is the truth), its budget flags hold the schema's bounds, a rendered brief handed back as the base is refused, an empty report is not a pass, the title is one safe line, a line longer than the cap is clipped rather than dropped, and a `--loop-iteration` record needs a hex SHA (recorded in lowercase) and a number recorded once and, while the loop is on, within the budget. The round-2 reviews found the deferral re-entering through `tdd-order` under `implement_mode: tdd` — the order gate's *other gates green* input took every outcome, a red `pre-merge` gate included — so that input is now the judged gates, and the recipe tolerates `run-gates`'s exit code, which is the s8 verdict with the deferred gates in it.
  - **The pure half is `keel.loop`**, deterministic and I/O-free at 100 % line + branch: `resolve` (`--loop` > `knobs.loop` > off, `enabled` defaulting to true when the block is present, out-of-range values reading as defaults because the schema refuses them at load time), `parse_gates` (a `keel run-gates --json` report, a `keel ship --json` document, a `{gate_outcomes: [...]}` envelope, or a bare list; an unknown phase is refused, not deferred), `decide` (a soft gate that failed does not hold the loop open; a blocking gate nobody ran is deferred — listed, never counted green, never holding the loop open), `render_brief` (the base brief verbatim plus one appended section, byte-stable) and `iteration_block` (the ledger record).
  - **Gate output is quoted data, never instructions.** The brief becomes the implementer's prompt and a test suite prints whatever a test — or a fixture the implementer wrote in the previous iteration — told it to, so every line is blockquoted, a leading `#` or `>` and a setext underline are escaped, both comment delimiters are defanged and NUL is dropped, a line reading as one of the brief's trailer keys becomes inline code, and each gate's output is capped separately at `gate_output_max_bytes` (default 16 KiB) with a visible marker, a line longer than what is left clipped rather than dropped.
  - **`keel loop brief`** decides one iteration and renders the next brief; `knobs.loop` is the budget, so an unreadable config is a refusal (`no-config`), not a loop nobody bounded — `--max-iterations` is the explicit alternative. `keel plan` / `keel ship --json` publish `contract.implement_mode.loop` (`enabled`, `max_iterations`, `gate_output_max_bytes`, `source`, `wraps`) beside the s4 profile; `keel ship --live --append-ledger --loop-iteration K=SHA:pass|fail` records `run_context.implement_loop`; the closure comment renders `Implement: loop (k/N iterations: <sha> red → <sha> green)`, after the TDD phases on a test-first run, and is byte-identical to before for every other run. A project with neither the knob nor the flag publishes `enabled: false`, records `null`, and its `config_hash` does not rotate.
  - **`/keel:ship` gains a Looped s4 section** beside Test-first s4: the per-iteration gate run, the `keel loop brief` call, one commit per iteration, the same seat every time (escalation stays s9's), the phase-B-only rule under `tdd`, and the budget-exhausted exit; the six adapter surfaces are regenerated.
- **The learning document links each changed file, so a graph reader gets the edge** (#1166). The sink (#1154) named the files a lesson is about in one place, the front-matter list `changed_files:` — right for keel's own reader, which scores text and will match on the list, and a plain string to every *link-following* reader the sink exists to serve (Obsidian, an agent reading an index). Not graphify: its Markdown extractor resolves a link only when the target is itself a document, so the path reaches it through the semantic pass that reads the whole file — `docs/keel/configuration.md` says so, and `docs/keel/comparison.md` was corrected to match in #1186. Measured on a document the sink writes: the body named no file at all, so a knowledge graph built over the repository showed every lesson as an isolated node. The body now ends with a fourth fixed section, **Files**, one CommonMark link per entry — relative to the document's own directory for an in-repo sink (`../../src/keel/capture.py` from the default `.keel/learning/`), the file on GitHub at the merged head for a sink outside the checkout when owner, repo and head are known, and the bare path in backticks otherwise, never a relative link that resolves to nothing. Destinations are percent-encoded so a space or a parenthesis cannot end a link early; link text escapes the characters that would end it. The front matter is byte-identical to before, `_front_matter` and `retrieve_relevant_learnings` are unchanged, and `learning_sink_plan` decides the link form from the sink's *shape* through the same `learning_sink_in_worktree` that decides who commits the file. The relative prefix is derived lexically from the normalised directory — `posixpath.relpath` consulted `os.getcwd()` for two relative arguments, so a template such as `{owner}/../learnings` with `owner` unset rendered a different link per working directory — and an expanded template that climbs out takes the outside form. The link text escapes `<` and `>` as well as the bracket pair and the backslash, so a file name a merged PR chose cannot open an autolink or raw HTML inside the document, and the emphasis and strikethrough delimiters, the ampersand and the backtick too, so `__init__.py` reads as `__init__.py` rather than a bold `init` and a backtick pair cannot swallow what sits between it, and a bare path carrying a backtick is fenced with a longer run rather than written plain. A measured side effect: the front-matter list already matched a query naming a file once, and the link matches it again (in its text, or in its destination when the text carries an escape), so such a lesson scores higher than the front matter alone gave it (1 → 3 on the test document). Found by the round-1 reviews: the Windows legs would have gone red on two tests that resolved a POSIX link through `os.path`, the escape set was incomplete, and the changelog's first cut claimed a retrieval the reader already had.
- **`docs/keel/comparison.md` covers the agent-host loop plugins** (#1167). The June 2026 comparison positioned keel against merge queues, AI reviewers, issue-to-PR agents, policy gates and orchestration frameworks, and said nothing about the plugins that ship a loop of their own inside an agent host — although `--compound` is literally the compound-engineering profile and `compound-learning` is the capture marker. A **Category 5** section now covers the Ralph loop (a host stop hook with the model as its own completion judge), the compound-engineering plugin (the closest overlap, of vocabulary rather than layer) and graphify (not a workflow: a reader of the learning directory the sink writes), in the same five-part shape as the other categories, with rows in the comparison and borrowed-ideas tables, a paragraph in the positioning statement and the sources. It names #1165 (the s4 iteration loop), #1166 (learning files a graph reader can link), #1155 and #1163 so the roadmap can be followed from the comparison.
- **A built-in capture extension: one Markdown learning per applied capture** (#1154). `policy_pack.capture` with `mode: extension` has always let a project produce durable learning content after a merge, and keel shipped no implementation of it — so every project that wanted learnings wrote the whole s11 hook itself, and keel captured nothing from its own runs. `policy_pack.capture.learning.sink` names a directory; keel renders the document and writes it. A directory of Markdown with stable front matter is the entire contract: no vault format, no wikilinks, no plugin API, and keel never learns what reads that directory.
  - **The path becomes `capture.artifact`.** That field already exists to make an `applied` capture provable rather than asserted — `capture-verify` reports `applied` with no artifact as a finding — so a project with a sink stops passing `--capture-artifact` by hand for a file it did not write.
  - **The writer and the reader had to be shipped together.** `retrieve_relevant_learnings` is the only thing in this repository that reads a learning directory, and it took a file's title from its first line. Measured before anything was changed: a document with front matter is titled `---` and summarised `schema: keel.learning.v1` under that rule. Shipping the writer alone would have filled a folder with learnings the one existing reader mis-reads, so the reader learned front matter in the same change.
  - **The front matter is valid YAML, which took quoting.** An issue title routinely contains a colon — `capture: built-in Markdown learning sink` — and written bare the block does not parse. keel's own reader splits on the first colon and would never have noticed; every other tool that reads it would, and being readable by other tools is the whole point. A test parses the block with a real YAML parser.
  - **A template typo is refused where the config is read.** `path: "~/k/{repoo}/learnings"` would otherwise create a directory called `{repoo}`, successfully, on a machine nobody is watching. The placeholder set — `{owner}`, `{repo}`, `{base_branch}`, `{date}`, `{pr}`, `{slug}` — is closed and validated at load time.
  - **A relative path resolves against `--root`, not the working directory.** The default `.keel/learning` otherwise wrote wherever keel happened to be launched from, which on a CI runner is not the repository at all. An absolute or `~` path is used as written, because pointing the sink at a shared knowledge folder outside the checkout is the feature.
  - **`.keel/learning/` is committed, not runtime-ignored.** Everything else keel writes under `.keel/` is disposable per-run state; a learning git throws away is one the read path can never find. `tests/test_workspace.py`'s subtree sweep is what forced the question — a new `.keel` subtree must be classified, not merely written.
  - **Fail-soft, as the capture contract requires:** a sink that cannot be written records `skipped:capability-unavailable` and does not touch a merge that already happened. A `duplicate` learning decision writes nothing, which is the fingerprint dedupe doing its job.
  - **Seven findings from the gate seats, five of them serious, all in the first cut.** The write path **never redacted**, while the docs and this entry claimed it did — a learning file is a durable artifact made of an issue body and gate output, the two places a secret is most likely to have been pasted. The **fingerprint dedupe was dead on the only path that writes**: `learning_decision` was called without the existing records, so `duplicate` could never be returned outside a unit test that passed them by hand. The **document had no content** — description and all three sections defaulted to empty, so every file read `_Not recorded._` three times. The **write was not gated on `--live`**, so a dry run scattered files into a knowledge folder. And the **dogfood omitted `learning.enabled`**, which is what `learning_decision` actually reads, so keel would have written files while recording that it had decided not to.
  - **The YAML quoting was a deny-list, and deny-lists are wrong about what they have not met.** It listed `:`, `#`, `"` and a newline; a title starting with `-`, `*`, `&`, `!`, `@`, `%`, `|`, `>` or a backtick then failed to parse or came back as something else, `{a}` became a mapping, `[a]` a list, and `yes` became `True`. Twelve shapes, all legal issue titles. It quotes unless the value is plainly safe now, and sixteen shapes round-trip through a real parser in the suite.
  - **And the placeholder check only matched the shape of a correct name** (`[a-z_]*`), so `{Repo}` and `{base-branch}` were as invisible as they were wrong — in the guard written to catch typos.
  - **And the command keel dogfoods passed no title and no body.** `--issue-title` / `--issue-body` are `keel plan` flags at the start of a run; s11 is `keel ship … --live --append-ledger --issue <N> --pull-request <PR> --capture-status applied`, and a later invocation inherits nothing. Measured on exactly that: a file titled `Learning`, an empty description, a `learning` slug and `_Not recorded._` under every heading — an artifact making `applied` "provable" by pointing at a document that records no lesson, on the only path keel runs itself. When `--issue` names one and the flags are empty the facts are read from the host, fail-soft, the way the blocker gate already reads them; the ledger record and the document are given the same ones, so the dedupe fingerprints one lesson rather than two.
  - **And the files were empty on that same path, for a different reason.** s11 runs after s10 has squash-merged, so `--root .` is the primary checkout sitting on `base_branch` and `git diff --name-only main...HEAD` reports nothing — measured. The document recorded `changed_files: []` and the fingerprint hashed an empty list, which is the field the sink filename exists to tell two lessons apart by. `--pull-request` names the PR whose files those were, so they are read from the host when the local diff has none.
  - **And the first cut of that fix crossed the two fingerprints.** The sink was handed the host's PR files while the ledger record kept hashing the empty post-merge diff, so the document's front matter and `capture.learning.fingerprint` named different lessons; a second ship then wrote a *second* file — the sink found no match — while the record called it a `duplicate` of the first and pointed `capture.artifact` at the new one. Both seats found it, and the commit that introduced it claimed the opposite. The ledger hashes the capture's own file list now, while `changes.files` still records what git reported: an empty diff is a fact about the run, and the two are deliberately not one field.
  - **`sink: {}` is a declaration, not an omission.** Every field of a sink is optional, so an empty block is a project taking the documented defaults — and collapsed to a falsy `{}` it read as *no sink at all*, giving that project the pre-#1154 behaviour of writing nothing while the same block naming its `kind` wrote.
  - **And `date` was the one scalar left unquoted.** `2026-09-09` bare is a YAML *timestamp*: a real parser returns a `date` object where keel's reader returns the string, which is the single disagreement all this quoting exists to prevent.
  - **A gate nobody ran was recorded as passing.** `not_run` is the fourth outcome state — an agentic gate reaches the command runner as `ok=True, not_run=True` so a soft gate does not spuriously fail the run — and the document rendered it `ok`, teaching the next run that a gate nobody executed had passed, inside the artifact that exists to make `applied` provable. `record_gates_passed` refuses exactly that certification.
  - **And the summary was a heading.** A keel issue opens with `## Deliverable`, so the first non-empty body line is a heading and the front matter carried it as the lesson's one-line description — the field every retrieval hit shows a reader. The reader already skips headings when it falls back to the body; the writer was feeding one through the field that bypasses that.
  - **And the write happened before the append was known to land.** The ledger no-ops a second record for the same `(PR, head)`; the sink did not, so a retry whose fingerprint had *moved* — a `gh` outage on the first attempt, a label fetched on the second — scattered a second document into the sink, possibly a shared knowledge folder, that no ledger record would ever name. The clash is asked first now, keyed on the pair alone, before anything durable happens.
  - **And the import cycle it needed a workaround for is gone.** `config` reads this module to validate `policy_pack.capture.learning`. `capture` named `config` only for `ProjectConfig` in signatures, which `from __future__ import annotations` already leaves as a string at runtime — but CodeQL still counts a `TYPE_CHECKING` import as a reverse edge, and `ProjectConfig` is defined *after* `config`'s import of `capture`, which is the `py/cyclic-import` alert that kept the CodeQL check red. Capture now duck-types `policy_pack` and does not import `config` at all.
  - **And a suffix the reader would skip was accepted too.** `retrieve_relevant_learnings` opens `.md`, `.json` and `.txt`; a filename ending `.markdown`, or in nothing at all, was written into the sink successfully and then skipped by the only thing that reads it, with `capture.artifact` still naming it. One tuple holds the set now and the validator is held to it.
  - **And `fingerprint` was the last bare scalar.** A sha256 that happens to be all digits is an `int` to a real parser and a 64-character string to keel's reader — in the field that *identifies* the lesson.
  - **A `filename` is a name, not a path.** `{pr}/{fingerprint}.md` passed every other check, `mkdir(parents=True)` created the directory happily, and `retrieve_relevant_learnings` — the only reader — globs one level and skips directories, so the lesson landed where nothing would ever read it. That is the writer/reader disagreement this change opened with, arriving through a template the validator accepted. Nesting belongs in `path`.
  - **And neither obvious shape for the recorded path survived alone.** The join of whatever `--root` was meant "relative to the directory *that* run was launched from", so a relative `--root repo` prefixed the root twice reading its own file back; made absolute instead, it named a filesystem location — and the file is *committed*, so the next worktree and every CI runner hold the same lesson somewhere else, where the reuse could no longer find it. A path inside the checkout is recorded the way the repository names it and resolves against whatever root reads it; one outside stays absolute, because nothing else can name it.
  - **And the recorded path meant whatever `--root` happened to be.** Written as the join of a relative `--root repo`, the artifact was `repo/learnings/<file>.md` — relative to the directory *that* run was launched from — and reading it back through the same join prefixed the root twice, found nothing, and recorded `applied` with no artifact for a file still on disk. Both working shapes (`.` and an absolute root) hide it. The recorded path is absolute now, so it means one thing.
  - **And refusing a separator in the *template* was not enough.** `{base_branch}` is a legal filename placeholder and `feat/sink` is a normal branch, so the expansion carried a slash anyway, `mkdir(parents=True)` made the directory, and the lesson landed one level below where the only reader looks — `capture.artifact` still naming it, so `applied` looked provable. Only `{slug}` was slugified; every other value went in raw. The expanded name is one path component by construction now.
  - **And the orphan was still reachable by dropping one flag.** The write was gated on `--live`, the clash on `--live --append-ledger`; a run with the first and not the second wrote a document no ledger row would ever name. The artifact exists to be named by a record, so it is written only when one will be appended.
  - **And a relative directory template could expand to an absolute path.** `{repo}/learnings` with `repo` unset became `/learnings` — the filesystem root — while every reader of the template's *shape* still called it in-repo. The filename's expansion was flattened for exactly this reason; the directory's was not.
  - **And a relative record was written with this host's separators.** `str(PurePath)` gives `\\` on Windows, which a POSIX reader takes as one filename rather than three components — in the one field whose whole purpose is being read on a *different* machine. It is `as_posix()` now, guarded in the source as well as end to end, because `str()` and `as_posix()` are indistinguishable on POSIX.
  - **And the duplicate reuse never asked whether the hook was live.** `learning_decision` answers `duplicate` on a fingerprint match *before* it reads the enabled flags, so `duplicate` reaches a `capture.enabled: false` or `marker-only` project — and a returned path is treated as permission to replace the operator's own `--capture-artifact` with a stale sink file, for a project whose contract says `extension-owned`. Fifth reader of the same question.
  - **And the test for it was itself platform-bound.** It named `C:/knowledge` literally, which on Windows is a perfectly good absolute path — so the runner wrote there correctly and the assertion `Path(root) / "C:"` resolved to the *drive*, not a subdirectory, and failed on all three Windows legs. The test names a path anchored on the **other** platform now, so both sides read the same property, and it lists the checkout rather than joining onto it.
  - **…and only half of that fix moved.** The predicate asked `workspace.is_root_anchored`; the *writer* still asked this host, so `C:/knowledge/learnings` was joined under the root on macOS and produced a `C:` directory inside the working tree the adapter is told not to commit. Both halves ask the same question now — and a path anchored on another platform is not writable here at all, so it fails soft the way an unwritable directory does rather than landing somewhere invented.
  - **And "absolute" meant "absolute *here*".** `C:/knowledge` is not absolute to POSIX and `/srv/knowledge` is not absolute to Windows, so each host called the other's absolute path in-repo — and would have told the adapter to `git add` it, committing a `C:` directory into the repository or reaching outside it. A keel config is the same text wherever it is read, and `workspace.is_root_anchored` is the question already asked that way for `#953`.
  - **And `../learnings` is relative and still outside.** It is the documented "folder next to the checkout" shape without the leading `~`, and reported as in-repo it sent the adapter to `git add` a path git refuses — leaving the file off `base_branch` and the next worktree empty, which is the failure the flag exists to prevent, arriving through the flag. The path is normalised before the question is answered.
  - **The contract, the commit flag, the decision — all four readers ask one question now.** A dormant `sink:` under `capture.enabled: false` or `mode: marker-only` published `project_destination: "sink"` and `commit_required: true`, telling an adapter core would write while the writer refused, so neither did. And `capture_status: deferred` fell through `learning_decision`'s `skipped:*` check and answered `create-learning` with `durable_artifact: true` for a run the writer refuses. Same write/record disagreement, two more readers.
  - **And keel writes the file without keeping it — which this change does not fix.** With the default relative path the learning lands in the working tree, **untracked**: invisible to the next worktree (s2 cuts that from `origin/<base_branch>`) and discarded by every CI runner. `durable_artifacts.commit_required` says per project whether the path is inside the repository, so a reader can tell — but committing it from the run that wrote it is **not** a one-liner on the topology keel uses for itself: s2, `overnight` and `swarm` all execute inside a worktree while the primary checkout holds `base_branch`, so `git switch <base>` there exits 128 with *already used by worktree*, measured. Three rounds of gate findings went into a recipe built on that command before the topology was checked; it is gone, the surfaces say plainly that an in-repo sink is not durable yet, and landing it properly is #1163. **An absolute or `~` sink path needs no git step** — but it is durable only on the machine that wrote it: the recorded `capture.artifact` is that machine's absolute path and the run ledger is committed, so a teammate or a CI runner finds no file there. Portable artifact references are #1163 too.
  - **keel dogfoods it.** Its own `project.yaml` turns capture on with the default sink, so keel stops being a project that ships a capture contract and captures nothing.
  - **And gating only the write inverted the same disagreement.** The record still said `create-learning` with `durable_artifact: true` for a run that produced nothing, because `learning_decision` did not read the pair the writer now did. One predicate — `capture_hook_enabled` — answers for both, and a test asserts they agree on every combination.
  - **And the parent switches were never consulted at all.** `policy_pack.capture` says whether a project runs a content hook: `enabled: false` is a project that does not, `mode: marker-only` records the marker *without* one — the schema's own words — and the sink **is** that hook. A configured sink was permission to write one level above the four `learning.*` refusals below, and the suite never varied either field.
  - **A configured sink is a destination, not permission to write.** The gate refused only a `duplicate` decision, so four policies a project actually writes — `learning.enabled` false, `enabled` omitted, `mode: defer`, `mode: marker-only` — each planned a write while the record beside it recorded `durable_artifact: false`. The write is gated on the `create-learning` decision now, which is the one answer whose `durable_artifact` is true, and no decision at all writes nothing.
  - **Redaction ran in the wrong order, and broke the document it protected.** `ghp_` + 36 letters is *plain* YAML — letters, digits, an underscore — so the quoter correctly let it through bare; replacing it in the finished document then put `[REDACTED:github-token]` inside that bare scalar, and a real parser reads a **list** where a title belongs. The description is the issue body's first non-empty line, so a body opening with a pasted token hit it. The values are sanitized *before* the document is rendered from them now — which also stops the filename, built from the title, from carrying most of a token into a committed path. And `redaction.sanitize` recurses into dicts, lists and strings but passes a **tuple** through untouched, so the four sections had to be handed over as a list: the first cut of this fix left the description holding its secret until the document pass put it back in the same broken place.
  - **And "the fields a secret is likely to be in" was the wrong set.** A `changed_files` entry that *is* a token and nothing else is plain YAML too, so the same break happened one field over. Every value the document is rendered from goes through redaction now, which is the only version of this rule that does not need to be right about where a secret was pasted.
  - **A pattern that will not compile no longer ends the command in a traceback.** `keel ship` has a handler for an invalid `capture_redaction` where the *ledger* is sanitized; the sink reached for the policy earlier, outside every handler. It does still end the run — keel refuses to append a record it cannot redact, prints why and exits 1, and writing an unsanitized one would be the worse answer — so this is deliberately **not** the fail-soft an unwritable directory gets, which downgrades the claim and exits 0.
  - **An empty `labels:` is a null, not `[]`.** The contract calls these fields sequences, so a consumer that iterates them raised `TypeError` on any merge that recorded none — which is most of them.
  - **The fingerprint is in the filename, because the fingerprint is the identity.** Date, PR and slug do not distinguish two lessons: a second `create-learning` run on the same pull request the same day — different labels, different files, a different lesson — resolved to the same path and the write destroyed the first, leaving its ledger record pointing at a document that says something else. The dedupe cannot help there; it suppresses *identical* fingerprints, and these differ. A custom `filename` without `{fingerprint}` is refused where the placeholder typos are refused, because its only other symptom is a file that quietly stops existing.
  - **And a keyword with a trailing space walked through the allow-list.** `yes ` is a plain scalar, `yes ` is not in the keyword set, and a YAML parser drops the space and returns `True` — the same non-string the allow-list was written to stop, through the one gap the keyword check had.
  - **A control character is not a newline either.** `_yaml_scalar` turned `\n` into a space and left every other one alone: a raw carriage return survives inside quotes, and keel's reader splits lines on it — so it read a title of `"foo` and dropped the rest while a real parser read `foo bar`, the two disagreeing about one file. A NUL made PyYAML refuse the document outright, and `--issue-title` is a raw CLI string that can carry either. All of them become spaces, because a title is a line.
  - **And the body needed the same rule as the front matter.** The document's `# {title}` heading took the raw string, so a title carrying a newline wrote a heading and then whatever followed it as Markdown of its own — `foo` + newline + `## injected` became `# foo` and an `## injected` section, beside a front matter that had quoted the same value.
  - **And "control character" was the wrong set, in the guard written to stop exactly this.** `str.splitlines()` breaks on `\x85`, `\u2028` and `\u2029`, and none of them is in C0 — so a title carrying one still opened a Markdown section of its own, through the fix for the version of the bug that used a newline. The pattern is every separator Python breaks on now, and the test names them.
  - **`issue: null`, not `issue:`.** Both read back as `None`; only one says so on purpose, and a bare key looks like a field somebody forgot to fill rather than a merge linked to no issue.
  - **And the contract now says who writes the file.** `durable_artifacts.project_destination` said `extension-owned`, which was the whole truth until keel shipped a writer — an adapter that read it and wrote its own artifact had it overwritten, because `keel ship` puts the sink's path into `capture.artifact`. A machine-readable contract that is wrong about who writes is worse than one that says nothing.
  - **And the real bug was in `redaction.sanitize`, one field at a time.** It walked `str`, `list` and `dict` and returned a **tuple unchanged** — silently, looking exactly like a value that had been checked. keel's own vocabularies are tuples, so the sections went out with a secret, then `changed_files`, then `labels`; each fix converted the field in front of it to a list and left the next one. `sanitize` walks tuples now, preserving the type, and the learning sink hands it every value it renders rather than a chosen sample.
  - **And the reuse itself needed the gate it was written to serve.** `learning_decision` answers `duplicate` on a fingerprint match *before* it looks at the capture status, so a `not-run` or `skipped` record was handed the earlier run's path — the exact contradiction `keel ship` refuses at its flag boundary (*a run that never reached capture produced no artifact*) and the one `record_marker` states for `deferred` and `skipped`. Only an `applied` capture borrows one, which is the claim the reuse exists to keep provable.
  - **And the dedupe was manufacturing the gap the artifact exists to close.** A `duplicate` writes no file and still records `applied`, and `applied` with no artifact is exactly the finding `capture-verify` raises — so the feature whose stated purpose is to make `applied` provable produced an unprovable one every time the fingerprint matched. The lesson is not missing; it is the file the run this one duplicates wrote. That path is copied forward, but **only when it still resolves**: a record can name a file since deleted, or one written on another machine into a shared folder this checkout cannot see, and an artifact that resolves to nothing is a worse answer than none. A *relative* recorded path resolves against `--root`, the rule the write path already follows — `--root` defaults to `.`, so a live run records a relative `.keel/learning/…md`, and resolving it against the process directory would have found nothing on every run launched from anywhere but the repository.


- **Install instructions for every agent keel runs inside** (#1156). The README documented the **CLI** — Homebrew, curl, pipx/uv — and, for plugins, Claude Code alone. `agy` appeared zero times; `Cursor` appeared once, in a PR-reviewer comparison table. A four-agent install done from this repository had to be reverse-engineered from the tree, and two failures came out of it: on Antigravity the install silently imported the wrong surface (#1137), and a user who removed keel from Cursor could not put it back, because there is no CLI to install a Cursor plugin and nothing written down said so.
  - **A badge row that is an index.** Four badges beside Install, each anchored to its own `<details>` box, so clicking the agent you use opens its commands. Every badge is required to resolve to an anchor that exists.
  - **Install *and* Update, per agent.** Install is guessable by analogy; update is not. `claude plugin install` is a no-op on an installed plugin; `claude plugin update` needs the qualified `keel@keel` and exits 1 on the bare name; `agy plugin install` overwrites in place and keeps the enabled flag; Cursor has no install command at all. All four measured against keel 1.22.0.
  - **What each Cursor route registers, measured rather than assumed.** A locally installed Cursor plugin registers **skills only**. keel's 17 `/keel:<command>` entries, where they appear in Cursor, are being read out of Claude Code's plugin cache — `~/.claude/plugins/cache/keel/keel/` holds `1.19.3`, `1.21.1` and `1.22.0`, each with all 17 commands and only the last with `skills/`, which is the release the root `skills/` directory landed in. They work by accident of a neighbouring tool's cache and would go if it were pruned.
  - **`agy plugin list` reports the import, not the disk.** It records components at install time, so an install predating a layout change keeps the old answer — keel reads `"components": ["commands"]` from a 2026-09-08 import while the checkout beside it carries `skills/`. Re-run the install after a release that adds a component directory.
  - **`docs/keel/plugin.md` was titled for one agent** — "keel as a Claude Code plugin" — which is how a reader concluded the plugin was a Claude-only artifact. It is "keel as an agent plugin" now and points at the install page; a test requires both pages to point at each other.
  - **`<br>` is not a newline to `textContent` either.** Replacing the `&&` with a line break made the copy a single run-on `…/keel/plugin install keel`. Both seats caught it, and the lesson is that a one-line pill is the wrong shape for a two-step flow: the pill copies the marketplace command, and the second step is stated beside it in prose. A test refuses a `<br>` inside anything a copy button targets.
  - **`&&` does not join two slash commands in an agent session.** The copy target read `/plugin marketplace add … && /plugin install keel`, which is shell syntax in a place that is not a shell. Two lines.
  - **And the guard written for that button compared nothing.** Every button on the page carries `data-copy-target`, so the loop's equality check `continue`d on all four — the third vacuous guard in this change. The property it asserts now is the one that makes "copies what it shows" true by construction: every copy button reads the element's own text rather than carrying a second copy of the command that can drift from the first.
  - **And the homepage's copy button did the same thing in the other direction.** It showed both plugin commands and copied only the first — introduced while fixing the button that showed one. It uses `data-copy-target` now, the pattern the `pip install` button beside it already used, so the button copies the element's own text. A test compares every copy button on the page against what it shows.
  - **A copyable line commented out its own second step.** The site card's Antigravity recipe was `agy plugin install <url>   # then: agy plugin enable keel`. Pasted, the enable is a comment — and `install` alone leaves the plugin disabled, which is the exact failure the line was added to prevent. Two lines now, and a test refuses the shape.
  - **"Re-running the install is the update for Antigravity and Codex" was half wrong.** For Codex it is `plugin marketplace upgrade` then `plugin add`. The card says which is which; the homepage's one-click copy stopped at `/plugin install keel` while the box beside it said `marketplace add` comes first, and the `aria-label` still called the card Claude's.
  - **The "same agents" guard was still filtering.** Comparing `anchors() & AGENTS` reduced to `AGENTS == AGENTS`; keeping "ids in AGENTS or starting with `zed`" was the same filter with a hole cut for one fixture. Both seats said so, in both repositories. It compares what each document *declares* — the README's badge targets, the install page's contents list — with nothing filtered, and the fifth-agent mutation goes through the real assertion.
  - **And `sections()` still did not stop at `</details>`.** Adding headings to the boundary list moved the Cursor box's end hundreds of lines out, so a missing `**Update**` was satisfied by unrelated prose again. A box ends at its own closing tag, whatever the next boundary is.
  - **An explicit anchor under a heading is a duplicate id.** `install.md` had both, so the rendered page carried two elements answering to `claude-code` — a hazard for anything that resolves an anchor by lookup. The headings already supply the slugs, so the explicit anchors are gone from that page and the guard treats a heading as the anchor it is. The README keeps its own, because a `<details>` box has no heading.
  - **One copyable block held two mutually exclusive routes.** The Cursor update fence listed the `git pull` and the marketplace re-index together, so copying it ran both — for a reader who took one. Two fences, each labelled with the route it belongs to.
  - **Clicking a badge did not open the box it named.** The anchors sat inside `<summary>`, and navigating to an id inside a `<details>` scrolls to it without expanding it — so the badge row's one promise landed the reader on a collapsed box. Each anchor sits before its `<details>` now, and the intro says what actually happens.
  - **Cursor's marketplace route has no verified update path, and both pages implied it did.** `marketplace update` re-indexes the *marketplace* — Cursor's own words — which is not the same as moving an installed plugin forward, and this session did not establish that it does. Said plainly, the two routes are an honest trade: marketplace registers the commands, the local checkout has an update that is a `git pull`.
  - **"No `pip` required" was the wrong promise, in four places.** The plugin removes the `keel install-adapter` step; it does not remove the CLI, whose absence the command bodies would hit on their first line. `plugin.md`'s heading, the site card, and the homepage's "install it as a plugin *instead*" all said otherwise, in the same change that added the prerequisite to the install page.
  - **"None of them is a re-install" was over-general.** For Antigravity and Codex, re-running the install *is* the update; only Claude Code's `plugin install` is a no-op, and there `plugin update keel@keel` is the command. The card said the Claude behaviour of all four.
  - **`What ships` omitted a manifest that exists.** `.codex-plugin/plugin.json` is on disk and a registered release surface, and the inventory table under a page retitled for four agents did not list it. The table is now checked against `.*/plugin.json` on disk, so the next manifest arrives in it rather than beside it. The Cursor row's "keel's 18 skills already load in Cursor" was corrected too: those come from Claude Code's plugin cache, while what a *local* Cursor install registers is one skill.
  - **"Skills only" in Cursor is one skill, and saying which one matters.** `.cursor-plugin/plugin.json` names `./skills`; the repository root's `skills/` holds `keel-onboard` and nothing else, while the 17 workflow skills live in `.agents/skills/`, which no manifest points at — that surface is written into a project by `keel install-adapter skills`. A local Cursor install gives onboarding and nothing more, which is a materially different promise from "skills".
  - **A four-agent caption over one agent's copyable fence is still one recipe.** The site card was retitled and the other three named in prose while the only `<pre>` a reader can copy stayed Claude's two slash commands. All four are copyable now, and a test asks for each command by name.
  - **The plugin was still being sold as the CLI's replacement**, in the README's badge-row intro and in `plugin.md`'s closing comparison — which contradicted the prerequisite paragraph added to the install page in the same change. The plugin removes the `install-adapter` step, not the CLI.
  - **The landing page had a copy button for it.** `website/index.html` read "In Claude Code? Install it as a plugin" beside a one-click copy of `/plugin install keel`, and its command section offered "`keel install-adapter all` or the Claude Code plugin". Both name the four agents and link the install page now, and a test refuses either sentence coming back.
  - **The install page never stated its own prerequisite.** #1156 asked for it written once; the opening said the CLI and the plugin "are independent", which reads as the opposite. The plugin removes `pip install` from *distribution*, not `keel` from the requirements — the command bodies shell out to it.
  - **A review note was left in user-facing prose.** The Cursor box ended with "the box offered the second install and only the first update", which is a changelog sentence about a previous draft. Replaced with what a reader needs: two routes, two update commands, and the fact that whether re-indexing advances an installed plugin was not exercised from here.
  - **The install page and the README disagreed about which Cursor route comes first**, in the same change set. Marketplace leads in both now, labelled with what each registers.
  - **The public site was still publishing the one-agent recipe.** `website/content.js`'s card was titled "Claude Code plugin", embedded only Claude's two commands, and cited the very page this change had stripped of that framing — so a reader on keel-ship.dev met the story the repository had just stopped telling. It is "Agent plugin" now, names the other three, says `/plugin install` is not an upgrade, and links to the install page; a test refuses the old title and requires the link. The leftover framing in `docs/keel/cli.md` and `plugin.md`'s opening line went with it.
  - **The Cursor box led with the route that does not do what the box says.** It was titled "local checkout", opened with `git clone`, and then explained that a local plugin registers skills only and the marketplace is what registers the `/keel:` commands — so copying the first fence did not fix the failure the issue names. Both routes are in the box now, marketplace first, each labelled with what it registers.
  - **"Everything above installs the CLI" was not true of what was above it.** The line sat under `keel install-adapter`, which writes adapter files into a project and needs the CLI already installed. Reworded to say what each route actually needs.
  - **The instruction pages are discovered now, not listed.** A hardcoded four-name tuple was this module's own defect class one level in. Discovery found three more pages printing an install command with no pointer to the page that owns them: `docs/keel/cli.md`'s adapter table, `AGENTS.md`'s directory listing, and `editors/vscode/README.md` — which is the *other* product and now says so at the top, the same note `editors.md` got.
  - **The Cursor box offered two install routes and one update.** It named `cursor-agent plugin marketplace add` as the route that registers commands and then documented only `git pull`. Both are there now, and the install page says plainly that whether re-indexing advances an installed plugin was not exercised from here — which is why the local checkout is the route it recommends.
  - **The module docstring overclaimed**, saying the pair is checked "against the surfaces they describe". These tests compare anchors, badge targets and substrings; both pages could agree on a wrong marketplace name and stay green. It says so, and says the commands were measured by hand instead.
  - **The anchor check was a no-op, and the same bug was in both repositories.** Its pattern left the relative prefix outside the capture, so `](../../README.md#install)` was captured as `README.md`, resolved against the linking page's own directory as `docs/keel/README.md`, found missing — and **skipped silently**. Every cross-link it was written for was invisible to it, and a deliberately broken anchor passed. The prefix is inside the capture now and a missing file is a finding rather than a `continue`.
  - **`docs/keel/editors.md` shares the word "Cursor" with the plugin and nothing else.** It installs `berkayturanci.keel-vscode` and gives neither the skills nor the `/keel:<command>` set, and said nothing about where the agent install lives — so a reader looking for "how do I get keel into Cursor" landed on the wrong product. It says so at the top now, and a test requires every page giving an install instruction to point at the install page.
  - **A GUI claim this session could not make.** The first cut said a local checkout "appears in **Settings → Plugins** as `keel (Local)`" as plain fact, while the page's own preamble promises that unexercised claims are marked. A CLI session cannot see Cursor's GUI. Both the README box and the install page now say it is *reported*, and say which session could not confirm it.
  - **`docs/keel/plugin.md` kept its own Claude-only install recipe** under the corrected title, which is the shape that made it read as a Claude-only artifact in the first place. It hands the recipes to the install page and keeps what belongs to it: what the namespace does to `commands/ship.md`.
  - **`docs/keel/editors.md` is a different product.** The install page's "See also" called it "editor and agent surfaces"; it installs `berkayturanci.keel-vscode` with `code --install-extension` and gives neither the skills nor the `/keel:<command>` set. Said plainly now.
  - **The anchor check came over too**, widened to match `README.md` — the first cut's pattern was lowercase-only, so the new page's own `](../../README.md#install)` links were invisible to the test written to catch a link into a renamed heading.
  - **The guard's own two holes, closed before they shipped.** The sibling change in ai-jury had a gate seat find both, and the same code was here. `test_the_two_documents_cover_the_same_agents` intersected each anchor set with the known agent list, but the tests beside it already assert `AGENTS <= anchors`, so the comparison reduced to `AGENTS == AGENTS` and a fifth agent added to one page only passed. And the per-agent split ran the **last** agent's body to the end of the file, so a missing `**Update**` in the Cursor box would have been satisfied by any `**update` in the hundreds of lines after it. Both are fixed and both mutations are tests.
  - **`tests/test_agent_install_docs.py` keeps the pair honest.** Two documents covering one subject is how ai-jury#781 happened. The README boxes and `docs/keel/install.md` must name the same agents, every badge must resolve, and every box must carry both sections. Verified by mutation in four directions.
- **One GitHub Action, at keel's root, carrying the contract that works** (#1153). The issue was filed on the belief that the root `action.yml` had no documented consumer. The opposite was true: it is the file GitHub publishes — the Marketplace listing instructs `uses: berkayturanci/keel@vX.Y.Z` — while `berkayturanci/keel-action`, which every doc pointed at, has no Marketplace entry and no releases. So the tests defended the published file and the docs sent readers to the unpublished one. Both contracts were broken, measured against keel 1.22.0: the root file's own defaults (`command: ship` with `live` and `jury` forced on) exited 1 on keel's consent gate, `command: swarm` exited 2 because there is no such subcommand, and its declared `status`/`decision` outputs were never written; `keel-action` pinned keel 1.15.0 and passed `--root .` to `swarm-plan`, the one sibling whose parser refused it. The root file now carries `keel-action`'s interface — real subcommand names, an array-built argv, `exit-code` and `output-file` actually written to `$GITHUB_OUTPUT`, a step summary, an optional PR comment — under the published `name`, which does not change because GitHub keys the listing on it. The default run is a **dry assessment**; a mutating run names its consent scopes through `args`, because the gate is not something an action talks its way past. `swarm-plan` accepts `--root` for parity with every sibling. `tests/test_action.py` no longer lists input names: it builds the argv the run block builds and hands it to `build_parser()`, which is what would have caught both defects — and did catch a third, that `evidence-verify` and `verify-merge` require `--pr`, so the action now refuses those up front by name instead of letting argparse exit 2 about a flag the caller never wrote. A new CI job runs the file itself with `uses: ./` and asserts both outputs carry values, so the Marketplace action can never again be a file nothing in this repository has executed — and it earned its keep on the first run, catching that GitHub executes a composite `shell: bash` step as `bash -e`, so the step died on keel's first non-zero exit before writing its outputs or honouring `fail-on-block: false`. The run blocks now set `+e` explicitly. The same review found `--pr` and `--issue` appended to *every* command: `command: validate` with a `pr` input exited 2 on a flag the caller never wrote, `command: plan` died on `invalid choice: '7'` because argparse abbreviation bound `--pr` to `--profile`, and on the swarm commands `--issue` does not record anything — it **selects the work**, so the action silently added an issue to the plan. Each is forwarded only where its parser takes it and where it means what the input says, and the test derives both sets from `build_parser()` rather than restating them. The docs and the site card point at `berkayturanci/keel@vX.Y.Z`, and both are registered release surfaces so the tag moves with every release.
- **`keel run-gates --phases` scopes a run to the phases it judges** (#1172). The s4 loop (#1165) judges the guard and test phases and defers the rest — listed, never counted green, never holding the loop open — but the runner still *executed* every planned command gate, a `pre-merge` Lego included. A project with a `release-check` Lego paid for that command on every iteration, and its red result was what made `run-gates` exit non-zero on an otherwise green one; the recipe tolerated that with `|| true`, which also swallowed a real failure. `--phases guard,test` reports a gate outside the scope `not_run` with its `on_fail` and never runs its command — the shape `--defer-jury` already used for the panel — so `keel loop brief` still lists it as deferred and the report's shape is unchanged. The Looped s4 fence passes it and drops `|| true`. An unknown phase is refused with exit 2 rather than silently scoping the run to nothing, which would have reported every gate `not_run` and exited 0: a green answer from a run that checked nothing. Without the flag every planned phase runs, which is s8, and that is untouched.


### Fixed
- **Issue intake no longer treats scope-exclusion sections as a rejection** (#1168). `## Out of scope`, `## Non-goals`, and `## Not in scope` headings and their bullets now remain descriptive; intake blocks only on an out-of-scope label or an explicit declaration by the issue anywhere outside those sections, and records the deciding label or sentence in `issue_intake.reason`.
- **Ship ledgers now report the branch's net changed files after a base merge** (#1174). `keel ship` prefers the `origin/<base_branch>` ref for its three-dot diff when the checkout already has it, so files introduced by merging an updated base branch are not counted as this run's changes; offline repositories fall back to the configured local base ref.
- **A ship run recorded against a superseded head no longer makes the pull request unmergeable** (#1157). `ledger.existing_capture_marker` matched on **pull request number alone**, so once any record carried a capture marker every later append was refused with `duplicate-capture-marker`. The readers that gate the merge — `ledger.gates_pass_for_head`, `juryavail.is_ship_run_for_head` (#1068) — ask for a passing record on the **current head**. The two keys disagreed, the ledger is append-only, and a pull request whose first ship run was red could therefore never satisfy the gate again. Reported from a live session: the first record was written for `48681d19`, whose jury failed on a false-positive base64 finding; every green run afterwards was skipped silently and the merge kept asking for a head no run was allowed to write.
  - **The documented recovery was the one action this design exists to prevent.** `existing_capture_marker`'s own docstring said it: "nothing in this module can remove a line — so the only exit is editing the ledger by hand." Hand-editing an append-only audit ledger to make a gate pass is forging audit history; a permission classifier refused it twice in that session, correctly.
  - **The writer is keyed the way every reader that gates reads.** A clash is now a marker already recorded for this pull request *and this head*. Re-running the same append after a crash mid-s11 still clashes — the case the check was added for — and a new head is a new run, which is what the merge gate is asking about. Records naming no head are compared to other records naming no head: the same rule applied to the value they have, not an exemption from it.
  - **`capture.verify_session` moved with it**, or the deadlock would have shifted one step later: it counted markers per pull request with no head filter, so a superseded head's marker would have failed verification for "multiple capture markers found for merged PR" — again recoverable only by hand. A merged pull request does not change head, so the count is scoped to the last record's head, and two markers on *one* head are still refused.
  - **Three consequences of the scoping, found by the gate seats and closed with it.** `capture.verify_session` reads the counted head off the last record *carrying a marker*, not the last record: a `--capture-status not-run` row re-recording gates after a rebase (#945) carries none, and reading the head off it looked past a real applied marker on the merged head, reported the capture missing, and had `reconcile_session` plan to emit a marker that already existed. `ledger.capture_health_summary` counts pull requests rather than rows, or one capture showed up in morning/wrap/status as `applied: 1, skipped: 1`. And `docs/keel/cli.md` and `docs/keel/parameter-reference.md` said "one capture marker per PR", which is no longer what either command enforces.
  - Note the shape: this check was added because "re-running the same append is the most natural thing to do after a crash mid-s11, which made the obvious recovery the very action that bricks the run." It closed that and re-created it one level out, where a *different* head bricked the run.
- **`keel review --from-jury` no longer invents substance for empty or ABSTAIN ballots** (#1150). `ballot_scope` always opened with `Checked the changed-file diff as ai-jury panelist …`, which is keel's own `verdict_substance` escape hatch, so an abstention still passed the gate by construction. Schema ≥1.2 `counts_as_review` / `scope_substantive` / `scope` / `testing` / `abstention_cause` were ignored. Abstentions entered `Panel.ballots`, inflated `panelists` / `jury_panel_size`, and could be posted as `review-verdict-*`.
  ...
  - **The flags may remove a ballot; they may never admit an empty one.** An earlier cut of this fix read `counts_as_review: true` as admitting a ballot on its own, and then minted `Checked the changed-file diff…` for it when the record named no path — the escape hatch this entry is about, re-opened for the one ballot that cut believed it was rescuing. The belief was wrong and measurable: ai-jury derives `counts_as_review` from `scope_substantive`, which is derived from the `scope` prose, so `is_review({"role": "panelist", "verdict": "APPROVE", "scope_substantive": scope_is_substantive("")})` is `False` and no ai-jury report can carry that record. It is inconsistent input, and honouring it means keel writing the substance the report failed to supply. A ballot counts when the report gave prose to post or a path to name; the flags can only subtract. The clean seat that read the diff and found nothing is not affected — a ≥1.2 report describes it in its own `scope`, which keel posts as written. Found by a gate reviewer, against the cut that introduced it.
...
- **Two `config_hash` neutrality tests asserted only half of what they promised** (#1187). `TestLoopKnob.test_unset_is_none_and_does_not_rotate_config_hash` checked that *setting* `knobs.loop` rotates the hash and never that leaving it unset does not, so the guard in `config.py` that omits the key entirely — the one thing carrying a guarantee stated in the #1165 entry, in `docs/keel/configuration.md` and in #1165's acceptance — was the sole survivor of 51 mutations: fold it away and all 4,687 tests still passed while `config_hash` for `projects/keel.yaml` moved. 100 % branch coverage cannot see it, because the branch runs either way and only the value changes. `test_an_absent_team_does_not_rotate_config_hash` was weaker still: it hashed the same config twice and asserted the two agreed, which is a property of `config_hash` being a function. Both now assert against the **empty declaration** — `loop: {}` and `team: {}` are declarations keel keeps, so never writing the key must hash differently from writing it empty, which is exactly what a version that dropped the distinction would collapse.
- **An explicit loop budget is recordable, and an off policy publishes no budget** (#1173). `keel loop brief` always took `--max-iterations N`; `keel ship` had no counterpart, so a run that looped four times under an explicit budget reached s11 and was refused — *"iteration 4 exceeds the budget of 3"* — against a policy it had never run under, after the work was done. `keel ship` now takes the same flag with the same 1..10 bound: it outranks both `knobs.loop` and `--loop`, publishes `source: flag:--max-iterations`, and is what `--loop-iteration` is judged against. Without it every number is still judged against the project's policy, so the over-budget refusal is unchanged. Separately, `iteration_block` published `max_iterations` from an **off** policy — the resolved default, or a disabled block's own number — beside `enabled: false`, while `iteration_problem` skips the budget check entirely when the policy is off: `--loop-iteration 99=…` was accepted and then recorded next to `max_iterations: 3`. A record made with the loop off now publishes `max_iterations: null`; `closure._loop_part` already rendered the policy-off form and needed no change. The resolution moved into `loop.resolve`, so `keel plan`, `keel ship` and `keel loop brief` share one spelling of the precedence and the `flag:--max-iterations` literal exists once.
- **Every command diffs against the same base ref** (#1184). #1174 moved `keel ship` to `origin/<base_branch>` so a base merge would not count another author's commits as this run's changes, and left two other spellings behind: `keel run-gates` still diffed the local `<base_branch>`, and `_gather_branch_facts` used a bare `origin/<base_branch>` with no fallback at all. `_gate_runner` runs the **jury** on that diff, so the same jury on the same branch at the same head received different input depending on the entry point — measured on a branch that merged an updated base: `<base>...HEAD` listed the upstream author's file and `origin/<base>...HEAD` did not. The two that diff now go through `_ship_base_ref`, and a test pins them to each other rather than to a spelling. The **name** is shared through `_remote_base_ref`; the **fallback** is not. `keel verify-branch` keeps asking about `origin/<base>` or nothing: `_ship_base_ref` falls back to the local branch so a diff always has something to diff against, and taking that fallback here would answer the ancestry verdict against a ref that may be days behind while the summary still printed `origin/<base>` — a pass reported for an origin nobody observed, where `docs/keel/cli.md` and `branchscope._check_ancestry` both promise an advisory skip. Caught by the round-1 gate seat, which ran it: `base_tip_sha` became the local SHA and the verdict came back `ok`/`pass` with `ancestry.advisory: False` where it had been a skip. The gather also resolves the ref lazily, inside the guard that already skips live git for a caller who supplied `--base-tip-sha`. The changelog's claim that ship prefers the *fetched* ref is corrected: `git.fetch` exists in `keel.git` and nothing in keel calls it, so what ship prefers is whatever the checkout already has — keeping that ref fresh belongs to whatever drives keel, and a fetch inside a diff would put a network call in the middle of one.
- **`comparison.md`'s third-party claims, corrected against the sources** (#1186). Four did not survive measurement. **graphify does not resolve a Markdown link to a code file** — its extractor's `_MD_LINKABLE_EXTS` is document extensions only and its README says reference edges run between docs — so #1166's links produce no edge there; the path reaches graphify through the semantic pass that reads the whole file, which is what `docs/keel/configuration.md` already said and the #1166 changelog entry did not. **`/deepen-plan` and `/triage` are not compound-engineering commands**: the first was folded into `/ce-plan` in March 2026 and survives only in that repo's changelog and upgrade-cleanup list, the second was never a shipped plugin command, and neither cited write-up mentions either. **`graphify.net` is not the project's site** — the repository's `homepage` is graphify.com, and graphify.net is an unaffiliated listing that still reports the licence as MIT, which is one of the stale listings this page warns readers about. And the plugin has 35 skills with **no** `agents/` directory, writes one document per capture into `docs/solutions/` rather than one file read every session, and documents installs for around sixteen hosts. The state-file claim about the Ralph loop is true but was cited to the README, which never mentions it; it now cites `hooks/stop-hook.sh`. Separately, the page described #1165, #1166 and #1155 as proposals for the seven hours between it landing and all three shipping — contradicting `README.md` on the same commit. A new guard compares the page against the changelog rather than a fixed phrase list, so an issue recorded as delivered cannot be cited in the future tense.
- **The intake scope scan reads the whole issue again, minus its boundary sections** (#1188). #1182 stopped `## Out of scope` headings from refusing the issue that wrote them, but did it by narrowing *where* intake looked — the title, the **first sentence** of the opening paragraph, and four section names — rather than by skipping the sections themselves. Three shapes therefore reached `can_mutate_code: true`: a declaration in the second sentence of the opening paragraph (which the changelog said was covered), one under any other heading, and most consequentially `## Decision — this issue is out of scope; closing`, the most natural place for a maintainer to record a closure, after which `/keel:ship` would proceed to mutate code. The two filters are now independent and each does one job: a scope-exclusion section (`Out of scope`, `Non-goals`, `Not in scope`, `Not in this change`, and their kin) is removed **whole**, heading and bullets, and what remains is searched in full for a sentence shaped like a declaration *about the issue* — so ordinary prose saying some detail is out of scope for this change still reads as prose, wherever it sits. `non-goal` is deliberately not a declaration marker: it names a boundary of the change, which is the same statement the section makes. Each heading in the set is pinned by a test carrying a sentence that would match if the section were not dropped.
...
- **An `implement` delegate through agy edits the worktree keel gave it** (#1134). `keel delegate run --role implement --provider agy:… --cwd <worktree>` dispatched correctly and changed nothing: agy works in its own copy under `~/.gemini/antigravity-cli/scratch/<basename>`, so the run returned prose describing files it had edited while the worktree stayed clean. `implement` is the one role whose whole product is a modified working tree, and the first thing downstream would have seen is a pull request with no diff. keel passes `--add-dir <cwd>` now on every agy dispatch that has a working directory.
  ...
  - **`--print-timeout` goes with it, because keel's own `--timeout` reached nothing.** agy's print mode stops at its own 5m default, so the `--timeout 900` dispatch that found this bug died at 298s with agy's `timeout waiting for response`. A bound that does not reach the process it bounds is not a bound; keel passes its timeout in agy's own duration spelling.
  - **The read-only path takes the flag too, and was checked separately.** A reviewer reading a scratch copy is the same defect in a milder form — the copy is whatever agy last synced, not what keel checked out. `--sandbox --add-dir` was run against a real worktree and answered from the real file, so what backs the read-only promise is undisturbed.
  - **The directory has to be absolute, and the CLI is what makes it so.** The child is started *inside* `cwd`, so agy resolves a relative `--add-dir` against that directory: `--cwd worktrees/foo` would have named `<root>/worktrees/foo/worktrees/foo` and missed the worktree exactly as before. `--add-dir .` is not a way out — measured, and it ended on agy's timeout with the tree untouched, the same as no flag at all. `keel delegate run` resolves `--cwd` with `abspath`; the planner refuses what is left and says why in a warning, because it is pure and `abspath` would read the *host's* working directory into a frozen, JSON-stable document. Found by the gate review, whose planner tests all used absolute paths and so could not see it. `is_absolute_cwd` accepts Windows and UNC spellings too: a plan may be read on a platform other than the one that wrote it.
  - **`claude` and `codex` take neither flag**: both run where they are started. Neither was exercised in the `implement` role here, so `docs/keel/models.md` says that rather than implying they were.
- **A profile or registry entry can name the vendor its label reports** (#1129). `knobs.delegate_profiles` and `~/.keel/providers.yaml` both restrict `vendor` to the **transport** — `cli` for every local coding-agent CLI — so two entries driving Grok and GPT through one `cursor-agent` binary reported `vendor: cli` in the delegate contract and were both labelled `agent:cli`. `reviewers.require_distinct_vendors` reads exactly that field: it could not tell them apart, and would equally have refused two genuinely different reviewers. The optional `vendor_label` says which maker an entry reaches, and both the contract and the label follow it.
  - **Measured against the two rules that consume it, not asserted in isolation.** `tests/test_vendor_label.py` feeds the resulting vendors to `evidence.distinct_vendor_check` and the resulting labels to `evidence.attribution_check`, and pins the pre-fix state — two entries, one vendor, `duplicated: ["cli"]` — so the fix cannot be undone quietly.
  - **Unset is unchanged.** Every built-in already names itself, `attribution_labels` enumerates a profile's declared label instead of the generic one, and the field is omitted from the hashed profile dict when absent, so a project that never writes it keeps its labels and its `config_hash`.
  - **The value is checked because it becomes a label keel applies.** It may not shadow a built-in vendor — that recreates the ambiguity from the other direction — may not restate `cli`, and is limited to the characters every existing vendor token uses. A project profile reports a bad value through `keel validate`; the registry warns and keeps the generic label, fail-soft like its other rules.
  - **The vocabulary had to learn the label too.** `agents.known_vendors` is what refuses a vendor keel could never have produced, and adding the field without adding it there split the vocabulary from the labels. Both gate seats traced the same three symptoms independently: `keel doctor` demanding `agent:xai` while `keel attribution --vendor xai` answered *unknown vendor*; that command refusing `--vendor xai --profile grok` as contradicting a profile whose own attribution says exactly `agent:xai`; and `ship --live --append-ledger` warning that `xai:grok-4.6` — the implementer it had just recorded — was not one of keel's delegate vendors. `known_vendors` now includes each profile's `label_vendor()`, the contradiction check compares against it, and a test asserts the agreement rather than the two constants: everything `attribution_labels` can write is something `known_vendors` accepts.
  - **And the registry validated the wrong value.** `_text()` returns `None` for a bool, an int or a list, and an unset label is silent — so a `vendor_label: on` (which YAML resolves to `True`) was swallowed without the promised warning, which is exactly the case the project-profile validator reports. The raw value reaches `vendor_label_errors` now. Also found by both seats.
  - **What it costs, written down beside the field.** The contract's `vendor` no longer separates a `cli` entry from a `local` one; nothing branches on that — `_transport_of` runs both as a configured binary, and `transport` still separates `api` from the rest. And a registry entry's label cannot be enumerated by `keel doctor`'s label check, because the registry lives outside the repository, so an operator who labels one creates `agent:<label>` themselves.
- **A catalogue card that names a delegate says how to define it** (#1132). Six cards showed `--delegate cursor|aider|opencode|trae|kimi|hermes`, and keel ships a profile for none of them. That is the *point* of the generic `vendor: cli` profile — keel does not hardcode a list of coding agents — but the card showed only the half a reader can copy. Each of those cards now carries a note naming the `knobs.delegate_profiles` entry it needs and linking §5 of `models.md`, which documents the mechanism with a worked example. The Hermes card's command names the delegate it was describing in prose (`keel swarm-run … --delegate hermes`) instead of leaving it implicit.
  - **The existing guard could not have caught this.** `TestTheIntegrationsCatalogRunsWhatItShows` parses every `keel …` card through the real CLI parser, which catches a command the CLI *rejects*. Checked rather than assumed: `keel implement … --delegate opencode --dry-run` succeeds and reports `delegate: opencode`. The name is accepted and passed through; resolution happens later, against a project and a registry the test does not have. A reader who copies the card, runs it and sees a dry run succeed can reasonably conclude it is wired.
  - **And the card parser was reading 29 of 30.** Its closing pattern required a trailing comma, which the last object in a JavaScript array literal does not have — so a `--delegate` card added at the *end* of the catalogue would never have been checked at all. Both gate seats found it independently and named the same reproduction. The comma is optional now, and `test_the_parser_sees_every_card` compares the parse against the id list so a silent subset cannot come back. The note lookup also compared the whole `--delegate` token, so `--delegate cursor:grok-4.6` would have been flagged against a note that correctly says `delegate_profiles.cursor`; the model comes off first now, as it already did one branch above.
  - **The rule the test enforces is about the card, not the name.** A `--delegate` that is not a built-in provider must be one the card tells you to define, in a note that names `delegate_profiles.<that name>`. The first cut accepted any note mentioning `delegate_profiles`, so renaming a card's delegate while leaving its old note behind still passed — the same drift one level down. Found by mutation, along with a second test that the note's `models.md#…` fragment is a heading that exists.
- **A vendor-reported timeout is a timeout, not a quota refusal** (#1133). A delegate run whose vendor stopped on its own timer came back `error_code: "rate-limit"` with `timed_out: false`. The two codes send an operator in opposite directions — `rate-limit` says *wait out the quota window*, `timeout` says *retry now, smaller or longer* — and on the run that found this the correct action was to raise `--timeout`, which is exactly what the reported code advised against.
  - **The classifier was reading the transcript, not the failure.** `rate_limited` was handed `CommandResult.output` — stdout and stderr glued together, which this module already documents as a diagnostic rather than something to parse. For an agent run that is hundreds of KB of the model's own prose about code. Measured over the 368 vendor transcripts on disk: **six** contain a bare `429`, and not one is a status code — a timestamp (`16:33:40.429`), two git shas, a `publish.yml:429` reference, and a line number in a code listing's gutter. Those six strings are now fixtures.
  - **So the signal is bounded and it is the vendor's.** A stream-json vendor contributes the `status` and `error` of its final `result` frame — deliberately not its `response`, which is the model's answer; everything else contributes stderr plus the tail of stdout. `429` also needs a status-shaped context now, because three digits are not a status code.
  - **`timed_out` means the run ran out of time, against either bound.** Which timer fired stays visible in `exit_code` (124 is keel's wrapper) and in `error`, which quotes the vendor. When a message reads as both a timeout and a quota refusal, `timeout` wins — the recoverable reading — and that precedence is asserted rather than left to the order of an `elif`.
  - **The bounded signal had to hold in the case the frame was meant to cover.** The first cut fell back to the stdout tail when a stream-json run produced *no* result frame — a crash, or a stream truncated mid-write — and for that vendor the tail is the model's own prose. A run that died mid-sentence about rate limiting came back `rate-limit`: the same wrong recovery, reached by the code added to fix it. There is no fallback now; stderr alone is the account, and stderr alone is what a frameless stream contributes.
  - **stderr no longer overrides a frame that said why.** It was prepended on every classification, so the same sentence this function refuses to read from the transcript flipped the result when it appeared on the other stream — a defect this module's own docstring predicts, since agent CLIs write progress to stderr. A frame carrying an `error` is now the whole signal; stderr joins only when the frame said nothing about the cause, where it is the only account there is.
  - **And the frame's fields are labelled rather than concatenated bare.** A numeric `status` is the one `429` that really *is* a status code, and stripping it to a bare token put it out of reach of the pattern that requires status-shaped context — self-defeating for the field this function chose to read. All three found by the gate review, and each pinned by a mutation.
  - **`timed_out` has a third case, which the doc now says.** A detached run that passes its deadline reports `error_code: lost` with `exit_code: null` — no process left to have one. The new paragraph claimed 124-or-vendor and was wrong for it.
  - **Verified by mutation, after the first cut of the tests failed to catch one.** Restoring whole-transcript classification, the bare `429`, the answer rejoining the signal, and `timed_out: false` each fail the class. The first cut's false-positive tests used the timeout fixture, where `timeout` was reached first either way — so they passed against the unfixed classifier, and the fixtures were moved onto a non-timeout failure.
- **`keel-onboard` reaches Antigravity** (#1137). `agy plugin install …/keel` imported 17 commands and **zero** skills. `.claude-plugin/plugin.json` declared `"skills": "./skill"` and it was correct — agy does not read it. It discovers components only by root-level directory convention (root `skills/`, `commands/`, `agents/`, `mcp_config.json`, `hooks.json`), measured with throwaway plugins fed to `agy plugin validate`. So the one skill a new user needs first was missing on that agent, and undiagnosably: they install keel, see `keel-cmd-ship`, and have no guided setup path. `skill/` → `skills/`, and both manifests follow it.
  - **Measured on the real tree, before and after.** `skill/` + `"./skill"` gave `skills: skipped (not found)` beside `commands: 17 processed`; `skills/` + `"./skills"` gives `skills: 1 processed` beside the same 17. Eighteen entries, no name appearing twice.
  - **Why this does not spring #1138's trap, which was the open question.** Adding a root `skills/` beside `commands/` normally doubles the surface — agy converts each command into a skill named `keel-cmd-<name>`, so a root skill with a command's name puts two entries for one job in front of a reader with different descriptions. That does not happen here for a structural reason rather than a lucky one: `skills/` holds exactly `keel-onboard`, and `onboard` is not one of the 17 commands. `test_no_skill_shares_a_name_with_a_command` keeps it structural — the day someone adds `skills/keel-ship/`, it fails and #1138 has to be decided rather than discovered by a user.
  - **`.agents/skills/` is untouched.** It is the cross-agent standard's path, read by Codex and Gemini; agy ignores it either way, which was also measured.
- **56 tests never ran when their file was run as a script** (#1141). `unittest.main()` calls `sys.exit()`, so a class defined *below* `if __name__ == "__main__"` is never reached by `python tests/test_cli.py` — it is not skipped, it does not exist yet. `unittest discover`, which imports the module, sees all of it, so CI stayed green while the loop a person actually uses (`PYTHONPATH=src python tests/test_cli.py -k delegate`) quietly omitted whole classes. A test that does not run cannot fail.
  - **Five files, measured on this head.** `tests/test_cli.py` 548 → 588 (40 tests: the delegate CLI wiring and the TDD-order gate), `tests/test_api_delegate.py` 60 → 65, `keel-visual/tests/test_runstate.py` 80 → 86, `tests/test_ship.py` 99 → 102, `tests/test_runner.py` 38 → 40. No test body changed; the sentinel moved to the end of each file. The first cut of this entry said 54 and 586, measured before #1140 added two tests to `DelegateCommandTest` — one of the stranded classes, so the recovered count moved with it. Found by the gate review.
  - **It is now checked, because it kept happening.** Three separate changes in one week each added a class below the sentinel, and each was caught by a gate reviewer rather than by anything mechanical. `tests/test_sentinel_is_last.py` parses every `test_*.py` in both test trees with `ast` — a `__main__` string in a docstring is not a sentinel, and one indented inside a class is not a module-level one — and fails on any definition that follows it. Verified by mutation in both trees.
- **`keel doctor --providers` reads the registry it was pointed at** (#1130). `delegate run` took `--registry`; `doctor` did not, so the flag was silently not a flag: the command ran against the default `~/.keel/providers.yaml`, printed `registry: … (not present)`, and the entry the operator had just written did not appear. Nothing was wrong with the registry — the one command that answers *"can this machine reach the provider I configured?"* was answering about a different file, at exactly the moment the registry is at a non-default path. The flag is the same flag, with the same wording and the same precedence `delegate run` uses (flag > `$KEEL_PROVIDERS` > default), and `providerprobe.collect` already accepted `registry_path`, so only the CLI half was missing.
  - **Tested on the rendered report, not on the plumbing.** The first cut spied on the keyword argument reaching `providerprobe.collect`, and the gate review showed that is not the bug's shape: keep `registry_path=args.registry` and overwrite `registry_path` in the payload before it renders, and a kwarg assertion stays green while the operator sees the default path again. The tests now write a real registry to a temp path and assert the `--json` report names that path *and* lists its entry — verified against that exact mutation.
  - **`--registry` without `--providers` says it did nothing.** Before the flag existed that spelling was an argparse error; accepting it and reading no registry would be the same quiet mismatch one flag further out. Also found by the review, along with the `doctor` synopsis in `docs/keel/cli.md`, which listed an example using `--registry` without the flag appearing in the signature above it.
  - **Attribution.** The direction came from a `keel delegate run --role implement` dispatch to **agy / gemini-3.8-flash-high**, which identified `providerprobe.collect`'s existing `registry_path` parameter as the seam before the run ended on a vendor timeout (#1133) with its edits in a scratch copy (#1134). The review seat was **grok / cursor-grok-4.6-high-fast**, which produced all three findings above.
  - **That note goes to stderr.** The first cut printed it to stdout, where under `--json` it landed *above* the report — so `keel doctor --registry X --json | jq` died on the warning about the flag it had just been given. A diagnostic that breaks the machine-readable output is a worse bug than the one it warns about; a test now parses the whole of stdout as one JSON document. Found by the second gate round.
  - **The default-path test asserted a substring and is now an equality.** It blanked out `~/.keel/providers.yaml` and asserted the remainder held no `/providers.yaml` — which any *wrong* path also satisfies, including a `$KEEL_PROVIDERS` leaking in from the machine running the suite. The three tests now pin the whole precedence chain against an injected `HOME` and `$KEEL_PROVIDERS`, and are verified against a mutation that returns a plausible wrong default. Also from the second round, which further caught the class sitting below `if __name__ == "__main__"`, where a standalone `python tests/test_doctor.py` never reached it.
  - **Tested on the path reaching the probe, not on the flag parsing.** The symptom was a command that *succeeds* while reading the wrong file, so an assertion that the argument parses would have passed against the bug. Verified by mutation: dropping `registry_path=args.registry` fails the suite.

### Removed
- **The AWS Bedrock and Azure OpenAI cards, which claimed support keel does not have** (#1131). The catalogue advertised both as backends. `bedrock` appears in `src/keel/` only in **model-id parsing** — `cost.py` splitting `anthropic.claude-…-v1:0` — and `azure` only as `AZURE_CLIENT_SECRET` in `config.py`'s **secret-redaction list**. Neither is mentioned anywhere in `docs/keel/`, and `models.md`, which documents Anthropic, OpenAI, Google, OpenRouter, DeepSeek, Groq, Together, vLLM, Ollama and the agent CLIs, has no section for either. The commands the cards showed were not keel's: `export ANTHROPIC_BEDROCK_AWS_REGION=…` is Claude Code's variable, `AZURE_OPENAI_ENDPOINT` is the OpenAI SDK's.
  - **Removed rather than documented, because neither route has ever been run.** Bedrock would work only by Claude Code inheriting an environment keel never reads; Azure OpenAI authenticates with an `api-key` header and a deployment in the path, which is *not* the shape keel's `openai-compatible` client sends — so whether it works is unknown, and nobody here has the credentials to find out. A support claim nobody can verify is worth less than no claim, and these two are the entries most likely to be read as a procurement promise.
  - **The count is now checked — every count, not the ones this edit touched.** Nine places state a number: the header comment, the sidebar badge, the search placeholder, the status line, the prose above the pills, and a badge on each of the five filter pills. Nothing tied any of them to the list, so removing two cards left them all wrong with the suite green. The first cut of `TheStatedIntegrationCountIsTheNumberOfCards` then repeated the mistake one level up — it matched only the four counts the fix had edited, and the gate review found the tab bar reading `All 32` directly above `Showing 30 integrations`, with an `LLM Backends 8` pill that filters to six cards. Every assertion now *derives* its number from the catalogue: per-category card counts against the pills and the prose, the total against the header, the status line and the section comments, and `30+` checked as the floor it is. Verified against seven mutations, including the bug's own shape — drop a card, touch no count, eight failures.
  - Either backend can come back the way every other one arrived: run it end to end, document it in `models.md`, then add the card.

### Added
- **A Cursor listing manifest, and a guard so the next one cannot drift** (#1139). keel shipped two per-agent manifests; in Cursor the plugin installs but carries no logo and renders as a bare repository name.
  - **Measured first, and the premise was half wrong.** A `cursor-agent` session asked for every skill and plugin available to it lists all **18** `keel-*` skills and `keel` itself. So *"Cursor and the rest fall back or have no entry point"* is only half right: the functionality is already there. What is missing is presentation. `docs/keel/plugin.md` says so, and says the GUI listing has not been confirmed from a CLI — a probe plugin loaded with `cursor-agent --plugin-dir` did not surface, which is inconclusive rather than negative.
  - **The manifest uses the host's fields.** Cursor's plugin reference documents `logo` — a repo-relative path it resolves against raw.githubusercontent.com — as the listing asset, and documents no `displayName`. The sibling change in ai-jury shipped `displayName` first and a gate reviewer caught it against the published schema; this one carries `logo`, the same `skills` path #1137 established, and nothing the host does not read. `TheCursorManifestMatchesTheSchemaItTargets` checks every key against that documented set.
  - **`category` and `tags` are not manifest fields, and the keyword named the wrong host.** Cursor's reference carries two tables: the plugin manifest's, and a *"Plugin entry fields"* one for entries in a marketplace manifest's `plugins` array — which is where `category`, `tags` and `source` live, in a file keel does not ship for Cursor. The allowed set stays the manifest's own, and says why, so widening it later takes an argument rather than a guess. Separately, the `keywords` array had been copied wholesale from the Claude manifest, so the file advertising keel *to Cursor* was tagged `claude-code`; it says `cursor` now, as the Codex manifest says `codex`. Both raised by the gate seats, and the keyword is pinned by a test.
  - **The version-sync guard is the part the issue said must come first.** `EveryPluginManifestIsARegisteredSurface` discovers every `.*/plugin.json` in the tree and fails if it is not in `RELEASE_SURFACES` — and fails the other way for a registered file that no longer exists. Without it the third manifest keeps the version it was born with while everything else moves: invisible for one release, permanent after, since nothing else reads these files.
  - **`.agy/`, `.kimi-plugin/`, `.grok-plugin/`, `.omp-plugin/`, `.devin-plugin/`, `.opencode/` and `.pi/` are deliberately absent.** None can be exercised here, and a manifest nobody has watched load is a claim rather than a fix — the bar this repository set when it removed the AWS Bedrock and Azure OpenAI cards (#1131). `.agy/` is redundant besides: #1137 makes Antigravity find the root `skills/` directory by convention, measured end to end.
- **The bot-branch rule keel did not have, after a bot used its absence** (#1127). A pull-request branch opened by an automation is a **read-only input**: the bot pushes from its own checkout, so its next push replaces the branch and silently reverts whatever landed in between. `CONTRIBUTING.md` now says so, `AGENTS.md` — the file the automations are actually pointed at — carries the short form and links it, and `tests/test_bot_branch_rule.py` compares the two prefix lists so one statement of the rule cannot drift from the other.
  ...
  - **The first cut of that test could not make the assertion it claimed.** Its prefix pattern was a closed alternation of the seven expected names, so it caught a *dropped* prefix and was blind to an *added* one — while the entry said `cursor`'s absence was asserted. It reads every backticked token in a clause bounded by the rule's own prose now — two intermediate cuts each still missed something, one anchored on `` `jules` `` so a name inserted *before* it stayed invisible, the next on the word "spelling", which `AGENTS.md` also uses about a deprecated config key, so it read an empty list every assertion then compared against nothing. `cursor` is checked against the documents rather than against a constant this file writes. The anchor test had the same shape: it slugified a hardcoded heading, so `## Bot-owned branches are read-only (rule)` passed while the link broke; it derives the heading from the file now. Both found by the gate review.
...
- **A verdict is no longer judged on a document the checker silently truncated** (#1120). `_verdict_prose` dropped every line containing `keel.review-verdict.v1` as a *substring*. The marker is rendered on its own bare first line, so the header slice above that filter had already removed it; the substring test therefore only ever reached prose that **quotes** the marker — the words a reviewer writes when the change under review is the evidence protocol itself. The review of #1119 lost its entire scope this way: a 1,700-character line naming `docs/keel/cli.md`, `src/keel/evidence.py`, `cache.cache_key` and `review-vendor-distinctness` in backticks was deleted whole, leaving 72 characters of template boilerplate, and the gate refused the verdict for naming nothing — on the pull request that documents the naming rule. The marker is now matched as a whole line.
  ...
  - **Measured, and small on history: 1 recovered, 0 regressed.** Replayed over all 1,421 verdict comments in both repositories, 48 (3.4 %) lose at least one body line to the filter — the largest a 1,105-character `Testing noted:` paragraph — but most still pass on some other line, so accepted moves 925 → 926. The count is not the reason to fix it; a checker reading a document it has truncated is, and the truncation lands hardest on the reviews keel's own development produces.
  - **The fallback the substring test served is kept.** A body with no blank line leaves the header slice at 0, so its bare marker line still has to be removed, and `<!-- keel.review-verdict.v1 -->` stays covered by the existing HTML-comment filter. Both are pinned. So is the regression itself, through the real renderer — every other test in `tests/test_verdict_substance.py` hands the checker a hand-written string, which is why 59 tests and a 1,421-verdict replay all missed this.
- **`docs/keel/cli.md` says the evidence gate refuses a verdict that names nothing** (#1106 follow-up). The rule has been enforced since #926 and widened by #1106, and the CLI reference — where an operator looks up what `keel evidence-verify` requires — described the marker, the head pin and the vendor distinctness check but never the substance check at all. A reviewer could read that page end to end and still not know why their verdict was refused. Found by a parallel pull request against the same issue, which spotted the gap this one left; its wording listed six verbs the shipped rule does not have, so the paragraph here describes what actually runs.
- **Every job in every workflow carries a ceiling, and the release job's is above its own waits** (#1116). `publish.yml` held zero `timeout-minutes` keys — and so did the other seven workflows: **16 jobs, none bounded**, all inheriting GitHub's six-hour default, while the sibling repository bounds all 16 of its own and wraps every network call besides. The issue asked which workflows already had ceilings; the answer was none, so this bounds all of them rather than the one where it was noticed.
  - **Sized from measured runs, not from round numbers.** Twenty completed runs per workflow, read from the Actions API: the slowest job in the repository is `test (py3.12 / windows)` at 4.8 minutes, and the publish job is 0.9. Each ceiling is written next to the measurement that justifies it. One number in that data is not a job at all — `keel evidence (required)` appears in the API at 11.5 minutes, but it is a check-run `keel-ship.yml` publishes and its clock is the wait for a verdict, not runner time; sizing a ceiling from it would have been sizing from a queue.
  - **The ceiling is a backstop; the per-call bound is the mechanism.** A ceiling cannot say *what* stalled, so every network call in `publish.yml` now carries its own: `curl` gets both `--connect-timeout` and `--max-time` (the first alone leaves a peer that connects and then goes quiet unbounded), and `pip` and `gh` get a `timeout` wrapper, because pip's `--timeout` bounds one quiet read rather than the call and `gh` has no request timeout at all. The install retry from #1111 is wrapped whole rather than per attempt: the script runs `"$installer"` as a single word, and `timeout(1)` is GNU coreutils that `macos-latest` does not ship — a bound inside it would have to be skipped on the runners its own test executes the real script on.
  - **The verify ceiling is 60 because a cancelled job files no report.** `release-broken` is opened by an `if: failure()` step, and `timeout-minutes` *cancels* rather than fails — so a ceiling below this job's waits would silence the loudest signal in the release chain for a release already on PyPI. Its bounds sum to 2,880 s; `tests/test_workflow_ceilings.py` recomputes that from the file for all three jobs rather than trusting the comment, and refuses a loop whose trip count it cannot resolve instead of reading it as one.
  - **Writing the test found two defects in the arithmetic that wrote it.** A `while read` loop closed with `done < <(jq …)` is still a close; comparing the line to `"done"` left it open and multiplied every later bound by two, reading 5,280 s where the truth was 2,880 s. And a bound inside a command substitution — `sums="$(timeout 60 gh release view …"` — was invisible to the token scan, which under-counts, the unsafe direction for a ceiling. Both are pinned by their own tests.
  - **The first cut of the scanner read `if curl` as a call to `if`.** It skipped wrappers, assignments and flags to find a command's head, but not shell keywords — and not the interpreter in `python -m pip install`, which it reported as a call to `python`. Both are the two call shapes this issue is mostly about, so `test_each_call_carries_its_own_bound` was passing by looking away: it saw 4 calls where `publish.yml` has 13, and pronounced every `curl` and every `pip` in the release bounded without reading one. Found by both gate seats. The scan now resolves the tool a command actually runs, and a test asserts it finds all three tools and at least twelve calls, so a scanner that stops seeing cannot keep passing.
  - **The bound added to protect the `release-broken` report could swallow it.** That step runs under `set -euo pipefail`, and a failed command substitution in an assignment *is* a failure — so `existing="$(timeout 60 gh issue list …)"` exiting 124 on a slow `gh` would end the reporter before it filed anything, for a release already on PyPI. It ends in `|| true`: an empty result already means "no open issue with this title", the branch below then creates one, and a duplicate report beats silence.
  - **`release_smoke.py` is a network client, and the scan could not see it.** It builds its own venv and resolves the published requirement a second time, but `python <script>` was classified as the interpreter, so `test_each_call_carries_its_own_bound` never read it and its `timeout 300` was deletable without this suite noticing. It is named in the table now, the scan finds 14 calls rather than 13, and `python scripts/smoke.py` is still not one. Both found by the gate review of this change.
  - **One existing test failed on a comment, and was using the raw file where its siblings use `_code`.** `test_the_release_guards_run_before_the_build` compares the position of the guard against `python -m build`; a new comment in the job header mentioning `python -m build` matched earlier than the guard, and the assertion reported the guard as running last. The class already had the comment-stripping helper this test skipped.
- **A verdict that names a symbol without calling it is a review, not a receipt** (#1106). `verdict_substance` read a dotted identifier only *with* its parentheses, a path only *with* a slash and an extension, and offered "checked …" as the only way to say a clean review out loud — so a gate verdict on ai-jury#753 that named eleven symbols, four call sites and three test classes was refused as naming nothing, because it wrote `cache.cache_key` rather than `` `cache.cache_key` ``. A verdict is now accepted when it carries an **anchor** — a path, a `file.py:42`, a backticked token, or a called `module.function()`, where the punctuation is the author pointing — or **two** of the unbackticked forms (a bare filename, a bare dotted `module.symbol`, a `snake_case` identifier), or a **"Checked X, Y and Z"** clause, whose object stays free-form because 35 verdicts in the corpus pass on that and nothing else.
  - **One bare token is not enough, and that is the whole shape of the rule.** `Node.js` and `evidence.py` are spelled identically; so are `GitHub.com` and `Config.parse`. Three rounds of refining character classes were each undone by the next token of the same shape — including `claude.ai`, which the tool's own "Generated by Claude Code" footer put in every verdict. What separates a review from a mention is not the spelling of one token but how many the verdict has.
  - **Measured, not asserted.** Replayed over every verdict comment in both repositories: `main` accepts 829, this accepts 925, **zero** that pass on `main` are refused, and 96 genuine reviews are recovered. Receipts stay refused — the 75 verdicts opening `Reviewed <title>`, all 10 renderings of the default template, and the three verbatim #926 verdicts. `_VERDICT_NOVELTY_FLOOR` is untouched and still catches a title said back on its own.
  - **`checked`'s object now ends at a sentence rather than at any period.** It buys a filename — `evidence.py` no longer reads as `evidence` — and costs a trailing `;`, `!` or `?` clause: "Checked config; found nothing of concern in the rest" passes on `main` and is refused here. No verdict of the 1,421 measured is written that way, which is why the corpus shows no regression — a weaker claim than "takes nothing away", and the one the evidence supports.
  - **The verb widening this issue proposed is not shipped, because it decided nothing.** `traced|read|ran|inspected|verified` could not keep a free-form object — "Read the whole diff and everything looks correct" is the #926 receipt with a synonym at the front — and requiring their object to name something made the branch dead by construction: the object is part of the prose, and naming something is the test the prose already takes. Across 1,421 verdicts it decided none of them. Removed rather than shipped as dead code, with the dead end written down so the next attempt does not rediscover it.
  - **The extensionless path this issue proposed is not shipped either.** A bare `word/word` read `1/3`, `2/3`, `I/O` and `build/lint/bandit`; slash-separated alternatives are how these reviewers write. `AnExtensionlessPathIsNotAnAnchor` records the refutation.
  - **Three shapes it still accepts are recorded as tests, not chased.** Two product names in one sentence corroborate each other; so does English written with underscores; and a missing space forges a dotted token ("the whole diff.See cli.py"). Each needs a list — of products, of words, of hostnames — that can never be finished. A period-plus-capital rule was written for the third and reverted: measured, it truncated `module.Class` and every bulleted object. This gate refuses rubber stamps; it does not resist an author working to defeat it, and anyone willing to type a backtick always satisfies it.
- **The release verification waits on the surface it installs through, not the one it reads digests from** (#1111). `publish.yml`'s `verify` job polls PyPI's **JSON API** until both distributions are listed, and then installs — and `pip` does not resolve through the JSON API. It reads the **simple index**, which is a different cache and lags behind it. Cutting v1.21.0 the gap was twenty-five seconds wide: both distributions were listed on attempt 1, both artifacts hashed to the digests PyPI declares for them, and `pip install keel-workflow==1.21.0` then reported a version list ending at 1.20.0. The whole five-minute `PYPI_WAIT_ATTEMPTS`/`PYPI_WAIT_SECONDS` budget had been spent watching a surface that was already ready; the surface that was not ready got exactly one attempt. The job went red and filed `release-broken: v1.21.0` (#1110) — the wording a maintainer drops everything for — against a release a plain re-run then verified with nothing changed, which is the expensive direction to be wrong in: do that twice and the signal stops being read.
  - **The install gets the budget, because the install is the assertion.** `.github/scripts/pip-install-with-retry.sh` retries it on the same two variables the JSON poll reads, so an index that cannot resolve the version *yet* is a wait rather than a result, and one edit still moves both waits. The alternative the issue offers — gate on a third surface, `pip index versions` or a scratch `pip download --dry-run` — would only narrow the gap: nothing binds the index page one request reads to the page the next request resolves against, and it would put an explicitly experimental subcommand between a correct release and a green job. Retrying the assertion closes it by construction.
  - **It is a wait and not a softener.** A spent budget still exits non-zero, with an `::error::` naming the requirement, the attempts made and the elapsed seconds; the installer's own output stays on stdout, so a wheel that really is broken retries for the full five minutes and then fails with pip's words in the log the report quotes. Because the step now fails only once the budget is gone, the `if: failure()` handler that files `release-broken` can no longer fire on a first miss. Parsing pip's error text to tell a lag from a real defect was rejected: it saves five minutes on a job that is about to fail anyway, in exchange for a guess about wording that changes between pip releases.
  - **The digest cross-check is untouched.** PyPI's own `digests.sha256` against the bytes it serves is the useful half and was never the broken part; it is what the JSON document is fetched for, and a test now pins that it survives.
  - **A release can only be cut once, so the shell is tested instead of the workflow.** `tests/test_pypi_install_retry.py` runs the real script under real `bash` against a stub installer that fails N times and then succeeds, and a stub `sleep` that records the wait rather than taking it — proving that a miss is retried, that the interval is honoured between attempts and not after the last one, that a spent budget still exits 1, and that a mistake in the call (no requirement, an installer that cannot run, a budget that is not a number) is diagnosed as exit 2 before any waiting starts. `tests/test_publish_release_chain.py` pins the shape in the workflow: nothing in the `verify` job resolves the requirement without the wrapper, both waits read one pair of variables, and the report is still `if: failure()` and still after the install.
  - **Each attempt bypasses pip's HTTP cache**, or the retry is decorative. PyPI serves `/simple/<project>/` with `cache-control: max-age=600, public` and pip honours a fresh response without revalidating, so attempt 1 during index lag records the version list that is *missing* the release and every attempt inside the next ten minutes reads that failure back off local disk without opening a connection. The whole budget is 285 s, comfortably inside the window — the loop would have retried its own answer. `--no-cache-dir` also makes each attempt a real download, which is what this job claims to be verifying. Found by the gate review of this change.

## [1.21.0] - 2026-09-06

### Fixed
- **"A role may be spelled in either vocabulary" is stated once** (#1107). `contracts.py` (the `implement` contract's `routing_keys`) and `swarm.py` (narrowing a resolved role) each spelled out `{*knobs.implementer_agents, *knobs.team.implement_by_role}`, with near-identical comments; both learned the second vocabulary in the same commit, `2bb6d59` (#1014), having each read the deprecated knob alone before it. That is the defect #1103 closed one call deeper — a rule stated twice is a rule that can disagree with itself — so the union now lives in `agents.known_roles(config)`, beside `legacy_team_seats`, the other bridge from the deprecated knob into the current vocabulary. `keel.team` could not host it: it takes exactly one keel import (the leaf `keel.vocab`) and cannot read a `ProjectConfig` without `keel.config` importing it back (#1050). No behaviour changes — `routing_keys` serialises byte-identically and the narrowing resolves identically for every vocabulary — and `sorted()` stays at the contract's call site, because the contract's ordering is the contract's business and not the rule's.
- **The step verifier checks the whole handoff schema, not the two fields easiest to fabricate** (#1101). `_check_handoff_schema` validated `schema_version` and `step_id` while `build_handoff` emitted ten fields, and the marker check searched the rendered body for a substring — so a handoff carrying a constant and a value the agent already knew, with no summary, no evidence ids and no provenance, passed the guard whose whole purpose is refusing a handoff an agent made up. The schema is now one declaration, `stepverifier.HANDOFF_FIELDS`, that the producer builds the document from, the ship contract publishes as `handoff_schema.required_fields` / `.fields`, and the verifier enforces; the verifier cannot fall behind the renderer again without failing a test, and the published list — which had itself already drifted, omitting `next_step` and `producer` — is derived rather than retyped. Every field is refused by name (`handoff field missing: summary`), and three checks go past presence: `rendered` must equal the canonical rendering of the handoff's *own* fields, which a body pasted from another step cannot satisfy; `provenance` must be a canonical untrusted-output tag bound to this step, agreeing with `producer` when one is named; and a `complete` handoff must claim every evidence id its step requires, naming each one it does not — an empty `evidence_ids` on a step that reports finished review is exactly the fabrication this guard exists for. The wire shape is unchanged, so `keel.step-handoff.v1` is not bumped, and every handoff a released `build_handoff` produced still verifies for all thirteen backbone steps — with one exception worth stating rather than glossing: a handoff whose `summary` was whitespace only. Older builds stored that as written (the `or` default is not reached by a truthy string) and the new blank-field rule refuses it. The producer is fixed here so it cannot be written again, but a document already on disk from before this change will be refused, correctly — it says a step is done and describes nothing. A step the contract requires no public evidence from still carries an empty list legitimately, because that is what the renderer emits for ten of the thirteen.
  - **The source has to name an agent even when the handoff names no producer.** The producer cross-check read `source.agent_id` only when `producer` was a non-blank string, so a handoff with `producer: null` — which `build_handoff` legitimately emits — cleared the provenance check with a `source` that named nobody at all. That is the same shape as the defect above: a guard passing on the absence of the thing it checks. `agent_id` is now required unconditionally, which costs a legitimate producer nothing, since `provenance.source_tag` always writes the key and fills it with `unknown-agent` when the caller names no one. Found by the gate review of this change, after its own tests had covered every provenance case but this one.
  - **And "canonical" now means the shape the renderer emits.** The check read five members of the tag and accepted any tag carrying them, while `provenance.source_tag` always emits `source.vendor`, `source.model`, `capability_scope.allowed_capabilities` and `capability_scope.unknown_capabilities` as well — so a tag that was not the renderer's passed a check whose message said canonical. The required key sets are derived from `source_tag` itself, the same way `HANDOFF_FIELDS` is derived rather than retyped, so a member added to the tag later fails this instead of going unread. Keys, not values: `vendor` and `model` are legitimately `None` when the caller knows neither. It runs last, after every specific check, so a step mismatch is still reported as a step mismatch. Second finding from the same gate seat, in the same place, one round apart; a third round added type checks on those members — `vendor`/`model` a string or `None`, the two capability lists actually lists. Their *values* are not checked and extra members are deliberately allowed through: `vendor` and `model` are free-form text the producer copies from the run, so there is nothing to compare them against, and a tag carrying a member this version has not heard of is a newer producer rather than a forgery — refusing it would make the verifier the reason a handoff cannot cross a version boundary. A fourth round found the synchronisation pointing the other way: `build_handoff` wrote `summary or "No summary recorded."`, and a whitespace-only summary is truthy, so the canonical producer stored a blank field that the verifier — which refuses one — then rejected. It strips before defaulting, and a test round-trips every backbone step through the producer into the verifier so the two cannot disagree about a document again.
- **The extensions docstring names every slot that may block, not `pre-merge` alone** (#1100). `src/keel/extensions.py`'s module docstring is the closest thing this repo has to a spec for the frontmatter mini-language it parses, and it said `on_fail: block` was "valid only in the `pre-merge` slot". What `parse_extension` actually enforces is `model.py`'s `may_block` flag, which `guard` (s3), `tester` and `test` (s8) carry too. A *documented* restriction the code does not have is the expensive direction to be wrong in: an author who wants a blocking `guard` reads that line, believes the slot cannot block, and files a bug or builds a workaround for a restriction that was never there — against a backbone whose whole point is that the slot list and its properties are knowable from the documentation. **Two** surfaces had drifted, not one: the docstring, and `CONTRIBUTING.md`'s "Add-only backbone" rule — the first design rule a new contributor reads. `docs/keel/extensions.md`'s rule and its `may block?` column, `docs/proposals/keel-architecture.md` and `AGENTS.md` already named the four. Both drifters said the same wrong thing, so no reader comparing two documents would have caught either; the second was found by the gate review of this change, after the first sweep had already declared the docstring the only one.
  - **Prose cannot compute the set, so a test compares it.** `tests/test_docs_claims.py::TestTheDocumentedBlockingSlotsAreTheBlockingSlots` reads the slot names back out of all four restatements — and out of extensions.md's per-row `may block?` column, resolving its `yes` / `no` / "`tester` and `test` only" answers — and holds them to `{slot.name for slot in model.SLOT_DEFINITIONS if slot.may_block}`: the flag itself, never a list retyped in a test, which is the rule `test_docs_claims.py` already works by. A restatement whose wording moves out from under its pattern fails as loudly as one that names the wrong slots, so the guard cannot go quietly vacuous, and the table check also asserts every backbone slot still has a row.

- **A conflict path is staged as a path, not as an option** (#1097). `swarm_landing` heals a rebase by re-staging the files it resolved, taking their names out of `git status --porcelain` — names from a tree the cluster's agents wrote, not ones the operator vouched for — and passing them to `git add` bare. A file called `-i` would have started interactive mode, `-A` staged the whole tree, `--renormalize` done something else again. `--` now comes first, which is git's own answer and needs no name check here. Found by generalising ai-jury#737, where a repository-derived name reached `ruff` and a file literally called `@pwn.py` made it read a response file pointing outside the checkout: whatever reads a name gets to decide what it means, so a name from data is passed as data. Every other subprocess site in this repository was checked and builds its own argv.

- **A zero-length merge window is a `ConfigError` instead of a window that silently never opens** (#1091). `merge_window: "09:00-09:00"` cleared every check #1076 added — the schema pattern, `parse_window`, and the all-or-nothing pair rule — and `window.is_merge_open` then evaluated `opens <= now < closes` with `opens == closes`, an empty interval, so it answered `False` at every instant of every day. `keel validate` printed `OK`, and every `keel ship` deferred at the merge gate for ever. It is the same class of defect as the missing-timezone case #1076 closed — a window that cannot do what it says — and the most likely way to write it is a typo of a real window (`09:00-19:00`).
  - **The rule lives with the other rules about *meaning*, because a `pattern` cannot state it.** A regex relates no capture to another, so `project.schema.json` takes `09:00-09:00` on its own and always will; comparing the two ends is something only the semantic layer can do. `config.merge_window_issue` — the one function the validator and `keel init --wizard` both ask (#1082) — now returns a message naming what the window would have *done*: never open, every merge deferred as outside the merge window, for ever. The wizard therefore re-asks for a zero-length answer with no change of its own, which is the whole point of the two ends sharing a function rather than each spelling out its own rule.
  - **Only the degenerate equal case is refused.** A window may still wrap midnight — `22:00-06:00` is the all-night window it reads as — and `09:00-09:01`, the neighbouring minute, is a real if narrow window and stays valid. `tests/test_config.py` pins the repro (closed at all 48 half-hours), that the schema pattern accepts the shape on its own, and that both neighbours still parse; `tests/test_cli.py` drives `keel validate` to `INVALID` on the equal case and to `OK` on the two; `tests/test_scaffold.py` walks the wizard through a zero-length answer and a wrap-around one.
- **`keel init --wizard` can no longer scaffold the half-configured merge window `keel validate` refuses** (#1082). Since #1076 `timezone` and `merge_window` are all-or-nothing — exactly one present is a `ConfigError` — but the wizard still asked for them as two independent prompts, each advertising *blank to skip*. Answer one, skip the other, and `keel init` wrote a `project.yaml` that the very next `keel validate` rejected, on the one command whose whole promise is a config that already validates. A typo'd zone or a `29:00-01:00` window landed in the file the same way, because nothing checked either answer on entry.
  - **The pair is one question now.** `Configure a merge window (timezone + hours)? (y/n)` — on *no* neither key is written, on *yes* neither half is skippable. The default stays *yes*, so pressing Enter through the wizard scaffolds the night no-merge window it always did.
  - **Each answer is checked with the function `parse_config` will check it with.** `config.timezone_issue` and `config.merge_window_issue` are the wording `ConfigError` already used, lifted out of `_merge_window_issues` and reused unchanged, so the prompt and the validator cannot drift into disagreeing about the same value. A value that will not evaluate is reported and asked for again, bounded by `wizard.MAX_ATTEMPTS` like the team step — and giving up drops the **pair**, never one half of it.
  - **`merge_window_issue` asks the schema's own `pattern`, not just `parse_window`.** `parse_window` does `time(int(hh), int(mm))`, which reads the single-digit hour in `9:00-18:00`; the bundled `^([01][0-9]|2[0-3]):[0-5][0-9]-…` pattern refuses it — so the wizard, checking only the looser of the two, still scaffolded a `project.yaml` that `keel validate` rejected. `config.merge_window_pattern()` now compiles the regex read out of `project.schema.json` itself, so there is one spelling of the shape rule and a schema edit moves both ends at once. Normalising `9:00` to `09:00` was the alternative and was rejected: it rewrites the operator's answer, and it would leave the wizard and `keel validate` still disagreeing about what a valid window *is*. The `parse_window` call stays as defence in depth for the day the pattern is relaxed.
  - **`scaffold.render_config` refuses half a pair outright**, the way it already refuses an unknown `consent_mode`, so no future caller of the scaffolder can write that file either. `tests/test_scaffold.py` walks every path through the wizard — yes with valid values, yes after a bad zone, yes after a bad window, giving up on either half, and no — and parses the result with `parse_config` each time.
- **A merge window that cannot be evaluated is a `ConfigError`, not a silently open window or a traceback** (#1076). `parse_config` certified three shapes `keel window`/`keel ship` could not honour, and each one landed somewhere worse than a config error. `merge_window` **without** `timezone` was accepted, and every surface reads a missing half as "no window configured" and reports the window *open* — so a project that wrote `merge_window: "00:00-00:01"` and forgot `timezone` published `window_open=True` at three in the morning with `bypassed_window=False`, merging straight through the night no-merge window it had just asked for, with nothing anywhere saying so. Hours **24–29** passed the schema pattern `^[0-2][0-9]:…`, and `is_merge_open` then raised `ValueError: hour must be in 0..23` from inside the pipeline. And `timezone` was any string at all, so `Definitely/Nowhere` reached `ZoneInfo` and raised `ZoneInfoNotFoundError` at evaluation time. `keel validate` printed `OK` for all three.
  - **The pair is all-or-nothing.** Both absent is a project that has not asked for a window and still parses exactly as before; both present is the configured case; exactly one present is now rejected, naming the key that is missing and what its absence would have done. That is the half that matters, because it is the one that fails *quietly*: the other two at least produced a traceback somebody had to read.
  - **Each half must actually evaluate, and the check asks the same functions the evaluator will.** `parse_window` on the window and `ZoneInfo` on the zone, converted into the ordinary `ConfigError` entries every other malformed knob produces, rather than a second implementation of the same rules that could drift from `keel.window`. The schema pattern is tightened to `^([01][0-9]|2[0-3]):[0-5][0-9]-([01][0-9]|2[0-3]):[0-5][0-9]$` as well — it is the contract consumers read, and it fails earlier — which leaves the semantic check as defence in depth for a caller passing `parse_config` its own `schema=`.
  - **The `window_gate` invariant is what this protects.** `AGENTS.md` lists the night no-merge window among the invariants no config or extension may override; a config that could not be evaluated overrode it by accident. `tests/test_config.py` pins the vacuous reading `assess` gives a half-configured pair *and* that no `ProjectConfig` can carry that shape into `is_merge_open` any more.
- **The review contract no longer claims `--hotfix` bypasses the merge window only** (#1078). `ship.resolve_review_contract` published `merge_gate.hotfix_bypasses_window_only: True` on every tier, and `docs/keel/parameter-reference.md` said the same in prose — but `cli._cmd_merge` under `--hotfix` also records `gates_sha: {bypassed: true, reason: "hotfix"}` and merges with **no** gates-pass ledger record for the head. A consumer reading the contract to learn what a hotfix merge did not check would have treated it as gates-verified when nothing had verified it. The behaviour is the deliberate half — `docs/keel/cli.md`, `docs/keel/command-contracts.md` and `tests/test_cli.py` have documented and pinned the gates-SHA skip since it landed — so the contract and the parameter reference are what moved.
  - **The boolean is replaced by the two lists it was standing in for**: `merge_gate.hotfix_bypasses: ["gates_sha", "window"]` and `merge_gate.hotfix_never_bypasses: ["checkpoint_gate", "ci", "evidence", "findings", "lock"]`, both sourced from `ship.HOTFIX_BYPASSES` / `ship.HOTFIX_NEVER_BYPASSES`. The names are the keys of the `keel.merge.v1` record itself, so a reader can look up each one in the payload a merge emitted; the one exception is `findings`, which blocks at the review verdict and never reaches `keel merge` to have a key there. `keel.command-contract.v1` is unchanged: this is a false statement withdrawn from a block whose other five keys are untouched.
  - **A test now derives the list rather than restating it.** `tests/test_cli.py::test_the_hotfix_contract_lists_what_this_command_actually_bypasses` runs a real `keel merge --hotfix --dry-run`, collects every payload key marked `bypassed: true`, and asserts the contract's list *equals* that set — so adding a bypass to `_cmd_merge` without naming it in the contract fails the suite instead of shipping a second stale boolean. The same test asserts every name in `hotfix_never_bypasses` is present and un-bypassed in that record.
- **A failure while initialising a resource claim no longer leaks an ownerless merge lock** (#1077). `lock._claim_path` creates the claim directory with an atomic `mkdir`, then scaffolds the runtime gitignore and writes `owner.json`. Neither call was guarded, so a `PermissionError` on a read-only `.keel` or an `OSError` from a full disk escaped `claim_resource` with the directory left behind and no owner file in it. That is the worst state the primitive has: `_holder` reads a claim with no readable owner as `UNKNOWN_HOLDER` — deliberately, so a lock is never mistaken for free — so every later `claim_resource` for that resource returned `denied` / `resource-already-claimed` held by `<unknown>`, and every merge on that checkout was blocked until an operator removed the directory by hand. Reproduced on `main` by making `workspace.ensure_runtime_gitignore_for` raise.
  - **The unwind removes the directory this very call created, and nothing else.** A directory that already existed leaves through the `FileExistsError` branch above, but that only rules out a claim older than the `mkdir`: while a slow caller is stalled mid-initialisation an operator can run the documented any-owner recovery on its ownerless directory and a second caller can claim the resource behind it, so by the time the handler runs the path can denote somebody else's live claim. **Neither the owner name nor the ownerless reading can tell that apart.** An owner is a name, not a claim id — `merge_lock` always claims as `merge-lock`, `keel merge` derives one name per pull request — so the live claim routinely carries the very string the failing call does; and the window between another caller's `mkdir` and its finished `owner.json` reads as `<unknown>` exactly like our own half-built claim, as does a torn write, because `_write_owner` is a plain `write_text`. So the directory is **identified** instead: `_claim_path` records `(st_dev, st_ino, st_birthtime)` — the birth time only where the platform reports one — straight after its own `mkdir` and holds the directory open for the life of the call — a pinned inode is not recycled, so that pair names one directory and only one — and `_discard_partial_claim` re-stats the path and removes nothing unless the answer matches. The owner check is kept as a second condition, for a directory that still is ours but by then records somebody else. The owner file is unlinked through the pinned descriptor, so that removal is bound to the inode; `rmdir` has no by-descriptor form in POSIX, so it is by name with the identity re-read immediately before it, and the residual exposure is another caller's *empty* claim directory created in that same instant (`rmdir` refuses a non-empty one). The same floor applies between the `mkdir` and the pin: anything already in the directory the pin latched means the claim is not ours — contention is reported and nothing is unwound — and only an empty stranger's directory taken in that instant is indistinguishable. The owner file is created exclusively (`O_EXCL`) through the same descriptor, so a claim taken underneath while the scaffold runs is never overwritten — the create fails and the caller sees contention or the original I/O failure — and a directory that cannot be listed is an I/O failure, not an empty one. The honest limit beyond that: Windows cannot open a directory, so there the check rests on `stat` alone and the unwind removes no file at all (a torn owner file of our own stays as the documented leak) — an NTFS file id whose sequence number is bumped when the record is reused, plus `st_birthtime` on 3.12+ — and this remains a single-host primitive, not a distributed lock.
  - **It raises rather than returning a `denied` result**, matching `_release_path`, which already propagates its `OSError` unless the caller asked for `best_effort`. Structured feedback in this module is the vocabulary of *contention* — `denied`, `missing`, `not-owner` — and callers back off and retry on it; reporting an unwritable lock root as `denied` would send them into a retry loop against an error no retry fixes. The contract block, its statuses and `deny_mode` are unchanged, and `docs/keel/command-contracts.md` now states the split.
  - **Cleanup is best effort, and never masks the cause.** If the failing step left a file behind, or the directory itself cannot be removed, the `rmdir` error is swallowed and the original exception is what the caller sees — the operator needs to know *why* the claim failed, and the leaked directory is there either way. So an ownerless claim is now rarer but not impossible, and the docs say so: `UNKNOWN_HOLDER`'s note and `docs/keel/command-contracts.md` list all three remaining ways in — the unmaskable kill (`SIGKILL`, container teardown, power loss) where no handler of ours runs, a cleanup the handler could not finish, and one it declined because the directory no longer answers with the identity it recorded — with `keel release <resource>` (no `--owner`) as the recovery for each.
- **A jury verdict for an earlier head can no longer downgrade the current head's gating jury to advisory** (#1069). `evidence.jury_participating_vendors` reads `vendors: <N>` off a posted `keel.jury-verdict.v1` and, below `jury.min_vendors`, that count downgrades a gating jury `gating → advisory` — which drops `jury-verdict` from the required evidence entirely (#1015). It read through `evidence._matches_head`, whose blank-head reading is *do not filter*, so on a run that resolved no head any trusted verdict on the pull request answered for the head being verified. **It was reachable on the surface a project actually runs in CI**: `keel evidence-verify --head-sha` defaults to unset and nothing fills it in offline, and a fixture-driven verify of a tier-3 change carrying a `vendors: 1` verdict for `three-heads-ago` published `required` **without** `jury-verdict` — a requirement removed by evidence nobody re-checked.
  - **One reader moved, and only that one.** `jury_participating_vendors` now asks `juryavail.is_pinnable_head` — the blank-head predicate #1068 wrote for the panel pins — before it reads a comment: an exact commit or no count at all. A *mismatched* head was never the reachable half and needed no change; `_matches_head` is already exact once a head is known, and permissive only when none is. The reproduction confirmed both: at `--head-sha current-head` the stale verdict was already ignored, and at `--head-sha ""` (and with the flag omitted) it relaxed the gate.
  - **The permissive reading itself is untouched, deliberately.** `_matches_head` filters evidence items inside a gate that, with no head resolved, is head-agnostic end to end — every `review-verdict` counts too — so tightening it there would refuse half a gate whose other half is already unfiltered. `jury_panel_size` keeps that reading because it feeds `max(declared, minimum_vendors)` and can only ever *raise* what a tier owes; `panel_verdict_posted` keeps it because its single caller, `juryavail.pin`, already refuses a blank head and owns the pin order. The distinction is now stated in all four docstrings, and it is a property of what each reader's answer can **do** — remove a requirement, or not — rather than of which module asks.
  - **`--jury-vendors N` is unchanged**, as is the #1015 downgrade itself: an operator's explicit count is a measurement, not evidence read off a comment, and a verdict posted for the head under verification still relaxes the gate exactly as before.
- **`evidence_require_distinct_vendors` is opt-in again: unset means `false`, on every risk tier** (#1065). 1.20.0 made the unset knob resolve from the resolved risk tier — on from TIER-2 up — which meant installing keel was enough to make every tier-2 and tier-3 change unmergeable for a project whose reviewers all come from one vendor. That is the normal case for someone with a single agent CLI installed, and it bit immediately: a tier-3 pull request with three genuinely independent verdicts (a lead, a cross-vendor gate seat, and a third reviewer) was refused by `keel merge` with `FAIL review-vendor-distinctness: require_distinct_vendors: review verdicts share a vendor: anthropic`, because two of the three happened to run on the same vendor. The same shape merged on 1.19.3.
  - **The knob conflated two claims and only one of them is keel's to make.** The *count* claim — three reviewers looked at this — is verifiable from posted evidence for any bench a project configured. The *independence* claim — three **independent** opinions looked at this — needs distinct vendors, and it is a property the cross-vendor jury panel provides, not one every tier-3 review must assert. Defaulting the independence claim on had keel assert, on the project's behalf, something the project never said, and block the merge when the world did not match. The claim is worth having; making it the unopted default was the error.
  - **The rule itself is untouched.** Set `knobs.evidence_require_distinct_vendors: true` and `keel evidence-verify` enforces exactly what it enforced before, with the same `review-vendor-distinctness` finding and the same message: every required review verdict must carry `vendor:` provenance and no two may share a vendor, and on a tier whose panel *is* the jury the panel must span `jury.min_vendors`. `keel evidence-verify --require-distinct-vendors` still turns it on for a single run. The setting stays tri-state at the config boundary so an explicit `false` remains distinguishable from silence; the two now resolve identically.
  - **The middle option — deriving it from a tier whose `knobs.team.review.by_tier` names the jury panel — was considered and not taken.** It would make the effective value depend on whether the calling surface resolved a team: `keel step-verify` without `--project` passes no assignment, so it would resolve `false` where `keel merge` on the same pull request resolved `true` — a live disagreement between two surfaces about a hard requirement, in the direction of understating it. Plain opt-in resolves the same value on every surface regardless of caller, and the jury coupling belongs with the panel-availability probe in #1066.
  - **`config_hash` does not move for anyone.** The hash has always folded this knob in as `bool(...)` of the tri-state — an unset knob hashed as `False` before this change and hashes as `False` after it, and the tier was never an input to the hash. Every project's `config_hash` is byte-identical across the change, whether it sets the knob or not.
  - **keel's own `projects/keel.yaml` is unchanged.** It already sets the knob to `false` explicitly, with the comment saying why: its tier-3 bench is anthropic + google + anthropic. That line is now redundant with the default and stays deliberately, because a project that has decided something should say so.

### Removed
- **`keel.agents.resolve_agent`, the second implementer resolver, is retired** (#1099). It answered "who implements this role?" from `knobs.implementer_agents` alone — the spelling `project.schema.json` marks DEPRECATED in favour of `knobs.team.implement.by_role` — so a project that had migrated got a different answer out of this published helper than out of every path that actually ships. It was listed in `agents.__all__`, so an extension author reading the module's surface had no way to know it was the odd one out; its only caller was `tests/test_agents.py`, which is why nothing was visibly broken and why it could stay wrong for as long as it did.
  - **Retired rather than repaired, because the defect is two rules and repairing it leaves two.** `team._implement_seat`, reached through `team.resolve_assignment`, is the rule: bench > `team.implement.by_role` > `team.implement.default` > the deprecated map > host, resolving a `Seat` — provider, kind, model, effort — *and* the config path it came from, which is what lets a surface report `knobs.implementer_agents.core (deprecated)` instead of silently honouring it. `resolve_agent` returned a bare string. Teaching it the same precedence meant flattening that seat back down to one, which is a second, lossier statement of the same rule: the same defect one call deeper, and a helper that would disagree again the next time a seat grew a field. Deleting it leaves one implementation in one place, which is what the issue asked for.
  - **Nothing else moves.** `agents` keeps the delegate vocabulary and the attribution it owns, `legacy_team_seats` included — the bridge that reads the deprecated knob as `by_role` seats so the one resolver can fall back to it — and `knobs.implementer_agents` keeps working exactly as it did while it is still supported. No slot, no contract, no CLI surface and no `config_hash` changes. `tests/test_reexport_surface.py` already holds `__all__` to the module's whole public surface in both directions, so the name cannot come back undeclared.
  - **Its tests are asked of the resolver that survived.** `TestOneImplementerResolver` drives `team.resolve_assignment` and asserts that the same routing written in either vocabulary resolves to the same seat while the resolver still reports *which* key answered, that an unknown role falls back to the host agent, that an explicit `--delegate` still wins, and that every legacy delegate token passes through unchanged — the property `test_resolve_agent_is_unchanged` guarded — plus that `agents` publishes no second resolver.

### Changed
- **The config suite imports `unittest` one way, and two local re-imports of `keel.config` are gone** (#1102). CodeQL's `py/import-and-import-from` alert #100 read `import unittest` beside `from unittest import mock` at the top of `tests/test_config.py`; it is now `import unittest.mock as mock` — the same one-line transformation that closed alert #28 on `tests/test_workspace.py` in #489, and the spelling `tests/test_lock.py` and ai-jury's `tests/test_adapter_key.py` already use. `tests/test_config.py` was the last module on `main` carrying that pair: an AST sweep of all 113 test modules that import `unittest` found no other. The neighbouring shapes are left alone deliberately rather than overlooked — sixteen modules pair `import unittest` with `from unittest.mock import patch`, which names a *different* module and which the query does not report, so rewriting their call sites would be churn answering no alert. The same read turned up two local `from keel import config as cfg` statements inside `TestSwarmReviewEvidenceKnob`, shadowing the module-level import ten lines from the top of the file — the defect #1074 fixed in `tests/test_install.py`. Both are deleted rather than kept with a comment, because the one thing that would have made them load-bearing is absent: nothing in the repository reloads `keel.config` or replaces it in `sys.modules` (the suite's only `importlib.reload` is `tests/test_yaml_helper.py` on `keel.yaml_helper`), so a local re-import binds the very module object line 11 already names. The file's one `mock.patch.object(cfg, ...)` is a context manager that mutates that shared object and restores it on exit, which a re-import could not have seen either. Test-only: no `src/` line moves, so the 100 % gate is untouched.
- **The three repeated local `import json` statements in `tests/test_install.py` are one module-level import** (#1074): CodeQL's last three `py/repeated-import` findings on `main`, closed by fix rather than dismissed, in the spirit of #1070.
- **The four re-exporting modules declare `__all__`, so a cross-module re-export is a statement the scanner can read** (#1070). `keel.agents`, `keel.delegate`, `keel.api_delegate` and `keel.juryavail` import names purely so *other* modules read them from there — the vendor vocabulary, the effort vocabulary, `OPENAI_COMPATIBLE`, and the three jury names `keel.team` has to own because the import runs the other way. #1062 spelled them `X as X`, which is what `ruff`'s F401 honours; CodeQL's `py/unused-import` counts *same-module* uses only and reported five of them (alerts 84–88) as unused imports with readers in `cli.py`, `agents.py`, `delegate.py` and `api_delegate.py`. A name in `__all__` is used by definition, so the alerts close on a fix rather than on five dismissals of a rule that found two real defects in `tests/test_external_promises.py` the same week (#1063).
  - **The choice was measured, not assumed.** Each module's public surface is small and already almost entirely consumed elsewhere: `agents` publishes 23 public names of which 22 are read by other modules or tests, `delegate` 29 of which 25 are, `api_delegate` 11 of which 10 are, and `juryavail` 25 of which 14 are. At that size `__all__` is an honest sentence rather than a list that invites drift — which is what tipped it against the alternative of dismissing all five and writing the exemption into `AGENTS.md`. The deciding argument is recurrence: a dismissal settles five alerts and leaves the *next* re-export to trip a sixth, trains the reader to wave `py/unused-import` through, and states nothing a tool can check; `__all__` makes future re-exports silent by construction.
  - **The declaration is the whole surface, in definition order.** A `__all__` holding only the re-exports would be a smaller lie than none at all — `from keel.delegate import *` and every documentation tool would read three names as the module and hide the other twenty-six. `delegate.EFFORTS` is listed too: `keel.cli` and `tests/test_delegate.py` read it, so it is published even though it is used in-module and never needed the `X as X` spelling. `api_delegate` is included although it carries no alert (its `OPENAI_COMPATIBLE` re-export happens to be read in-module as well), because the idiom is settled once for every module that uses it rather than once per alert.
  - **`tests/test_reexport_surface.py` holds it in both directions.** It discovers the re-exporting modules by parsing the package — a fifth joins the guard by existing — and fails when one declares no `__all__`, when an `X as X` re-export is missing from it, when a top-level public definition is missing from it, when a listed name does not resolve on the imported module, and on a duplicate or an underscore-prefixed entry. The completeness half is what keeps the list from rotting into a partial statement as a module grows. `AGENTS.md` records the convention so the next contributor spells a re-export the same way instead of reopening the question.

- **`keel.vocab`: the delegate vocabulary is a leaf module, so nothing reaches for it through a function-local import** (#1050): `keel.agents` and `keel.config` import `keel.team` at module scope for the policy type, so `keel.team` could not name `BUILTIN_DELEGATE_VENDORS` (agents) or `EFFORTS` / `supports_effort` (delegate) at module scope without closing a real cycle. It imported them *inside* `team_issues` and `_effort_issues` instead — benign at run time, four CodeQL cyclic-import alerts (58–61), and a paragraph of explanation every reader had to reconstruct. The names now live in a new dependency-free `keel.vocab`, which imports nothing from the package, and every consumer names them at module scope.
  - **Re-exported under their original spellings**, so `agents.BUILTIN_DELEGATE_VENDORS`, `agents.CLI_VENDORS` / `LOCAL_VENDORS` / `API_VENDORS`, `delegate.EFFORTS`, `delegate.EFFORT_VENDORS`, `delegate.supports_effort` and `api_delegate.OPENAI_COMPATIBLE` are unchanged for every caller and every test — a move, not a copy, asserted with `assertIs`. `config_hash` is untouched and all 66 argparse surfaces are byte-identical to 1.20.0.
  - **The two sibling local imports go with it.** `config._validate_delegate_profiles` reached into `keel.agents` for the same constant and now reads `keel.vocab` at module scope; `wizard._effort_capable` / `_efforts` reached into `keel.delegate` to keep the config graph out of `keel.scaffold`, which the leaf makes free. `keel.team` has no function-local import left at all.
  - **The invariant is a test, not a comment.** `tests/test_vocab_leaf.py` walks the package's real module graph — every import, module-scope *and* function-local, the way a scanner reads it — and fails on any cycle through `keel.team`, on any function-local import of a moved name, and on any intra-package import added to `keel.vocab`. A fresh interpreter that imports `keel.team` first must end up with exactly `keel.team` and `keel.vocab` loaded.
  - **The guard searches without a bound.** The cycle search is exhaustive — every simple cycle through `keel.team`, of any length — because a depth-bounded search reports clean on a cycle one edge longer than the bound, which is the one failure a guard exists to catch. The function-local check follows the *binding*, not just the imported name, so `from . import vocab` inside a function followed by `vocab.EFFORTS` counts as the reach it is; that widening found a real one in `runtime._provider_capabilities`, which read `agents.CLI_VENDORS` / `LOCAL_VENDORS` / `API_VENDORS` off a function-local import and now names them from `keel.vocab` at module scope (same objects, same behaviour).

### Tests
- **The two error-level code-scanning alerts in the external-promise guards are fixed rather than dismissed** (#1063): CodeQL reported `payload` (`test_the_tap_serves_an_installable_formula`) and `published` (`test_the_version_is_not_one_pypi_already_published_differently`) as possibly used before initialisation. Both were false positives — every `except` branch ends in `self.fail(...)` or in `_could_not_look(...)`, which never returns — but the guarantee lived in a helper one screen away, invisible to the analyser and to a reader skimming the file. Each read now sits in an `else:` on its own `try`, so the binding is provable at the point of use. The meaning is unchanged and the three outcomes the module exists to keep apart stay apart: a wrong digest or a repo version behind PyPI still **fails**, a fetch that could not happen still **fails** through `_could_not_look`, and only a fetch that succeeded reaches the assertion. `TestNotLookingIsNotAPass` pinned the *helper* offline; because an `else:` depends on its callers rather than on the helper, that suite now also drives **every handler each `else` depends on**, enumerated from the source rather than assumed to be one per check: the tap check's missing-artifact branch (`HTTPError` with `.code == 404`, reported by a direct `self.fail`), its fall-through past that `if` to the shared helper on any other status, and its unreachable-host clause; and the PyPI check's single `except OSError`, driven with a plain `OSError` and with an `HTTPError`, which subclasses it and so lands in that same clause rather than one of its own. Each row asserts the failure came *from the handler under test*, matched against the raised message rather than the formatted traceback — whose printed source lines would otherwise satisfy the fragment for a handler that never ran — and each row's sensitivity is established by mutation: a returning stub in place of that handler's failure call makes that row, and only that row, fail. Without it, a handler that stopped ending in a never-returning call would skip the `else` and pass silently, where the old shape raised a loud unbound-name error. Both online classes sit behind `KEEL_CHECK_EXTERNAL=1` and that job is not a required check, so the new guard is the only blocking signal that would notice.
- **`assertTrue(a < b)` in the review-contract suite reports both values on failure** (#1063): `tests/test_review_contract_agreement.py`'s convergence check (the posted-verdict requirement is a strict superset of the published floor) is now `assertLess`, so a regression prints the two sets instead of `False is not true`. It was the only comparison-shaped `assertTrue` in the module; the other six assert plain truthiness.

### Added
- **s7 probes the jury panel before it dispatches it, and a project can allow a recorded host-bench fallback when it cannot be staffed** (#1066). On a tier whose `knobs.team.review.by_tier` names `jury`, the panel *is* the review, and #1014 round 3 deliberately made it so no operator flag can take it back off. That is right while the panel can run. When it cannot — an agent CLI not installed, unauthenticated, or out of quota — the tier had no way forward at all: `keel evidence-verify --pr N` reported `missing: jury-verdict` against a panel the machine could not convene, and nothing an operator could type would change it.
  - **The probe asks the panel s7 would actually dispatch.** That is the `jury` binary and the agents *it* is configured with, not keel's own delegate inventory: a host with `claude` and `codex` on `PATH` and no ai-jury installed has an inventory, not a panel, and reporting it staffable published the panel bench and left s7 to fail at the invocation — past the point where `on_unavailable` could still apply. `keel.providerprobe.jury_availability` runs `jury --doctor --json`, ai-jury's own readiness document (`schema_version: ai-jury.doctor.v1`), which establishes the binary is present and runnable *and* names which of its agents are usable. keel does not thereby depend on ai-jury: a missing binary, a timeout, a crash or unreadable output each become a recorded reason, and a runner that identifies itself but names no agents is still usable — its vendor inventory then comes from the machinery `keel doctor --providers` already runs (a `PATH` lookup and one `--version` call per CLI vendor, an env-var *name* check per hosted API, one loopback request for Ollama), which is the one answer keel already had rather than a second prober that could disagree with it. **That document is also the binary's identity** (#1068 round 5): a `jury` on `PATH` that exits 0 without printing an `ai-jury.doctor.*` report has not established that it is ai-jury, so it is unusable and keel's own inventory cannot staff a panel behind it — the provider fallback stands in for a panel ai-jury declined to enumerate, never for a panel runner nobody established is there. Reading a clean exit as usable reintroduced exactly the failure this probe exists to prevent, and it protected nothing: an ai-jury genuinely too old for the flag parses arguments strictly and exits 2 on an unrecognized `--json`, which the exit-code branch already reports. An unusable runner is listed first under `availability.unavailable` as the `jury` seat, so every surface that renders the seats names the thing to install. The probe runs **only** on a tier whose review policy is the panel, so a project that has not made the panel its review (keel's own `projects/keel.yaml` included) pays nothing and behaves exactly as before.
  - **The policy is a configured allowance: `knobs.team.jury.on_unavailable: fallback | block`.** `fallback` — the default — staffs a host bench of the seat count the tier requires (three at tier-3), exactly as a tier without a panel resolves. `block` refuses the run with a message naming each unavailable provider and the probe's reason for it, which preserves today's strictness for a project whose product claim *is* cross-vendor review — refusing the work the panel would have reviewed, not everything on the host: `keel swarm plan` can still plan a wave of tier-1 clusters on a machine with no panel, and is refused the moment a cluster whose review is the panel comes up.
  - **The fallback changes who sat, never how many.** It publishes the tier's own reviewer count and the tier's own required evidence — three `review-verdict-*` items at tier-3, not two. It drops `jury-verdict`, and it must: there is no panel to produce one, and requiring it would leave the tier stuck one layer down. `--reviewers` stays **ignored** on the fallback bench, exactly as it is ignored while the panel sits: a flag that was inert on a staffable panel and resized the bench the moment the probe failed would make a missing agent CLI a policy change, and `--reviewers 2` would publish a two-verdict requirement where the tier asks for three. The ignore is named in `assignment.warnings` rather than silent.
  - **Nothing about it is silent**, which is the whole point — [ai-jury #682](https://github.com/berkayturanci/ai-jury/issues/682) exists because a panel that quietly collapsed to one vendor still reported success. The probe's verdict — which seats were unavailable and why — is carried on `assignment.jury.availability` and `review_merge_contract.jury.availability`, named in `assignment.warnings`, distinguished from a tier that never had a panel by `reviewer_source: jury-fallback`, written into the run ledger at `run_context.jury_panel`, and rendered as a **Jury panel:** line on the closure comment — followed, when the record carries a head, by `<!-- keel.jury-panel.v1 head=… decision=… -->`, the machine-readable half a verification surface elsewhere reads that decision from. **Every** panel decision renders that line and that marker, `available` (`panel sat — the cross-vendor panel reviewed this change`) included (#1068 round 7): the marker is the run's own record in the one place that travels with the pull request, and a run that said nothing there could not outrank an earlier ship of the same commit that had. Only a run of a project with no panel posts the comment it always did, byte for byte.
  - **#1014's principle survives.** What may not take the panel off is an operator's *preference*; there is no flag that asserts availability. Availability is a fact about the world, and it is allowed to change the outcome precisely because it is recorded.
  - **A surface that only verifies is pinned to what the ship measured, not to its own machine.** Recording the measurement is not by itself enough: `keel evidence-verify` and `keel merge` run wherever CI puts them, so re-probing there rewrote the contract of a change reviewed somewhere else — a panel project whose ship measured two vendors requires the panel verdict, and the same pull request checked on a bare runner measured one, dropped the panel item and demanded `review-verdict-1..3` from a host bench nobody sat. Both surfaces now take the ship's decision: first from the `ship_run` ledger entry written for **this head** of that pull request at `run_context.jury_panel`, which carries fallbacks too so a fallback-reviewed change is not held to a panel a workstation *could* have convened; failing that from the same decision read back off the **closure comment** that run posted on the pull request, which carries `<!-- keel.jury-panel.v1 head=… decision=… -->` beside its human `Jury panel:` line; and failing that from a head-pinned `keel.jury-verdict.v1` posted on the pull request (the panel sat, and the ballots prove it). **The run's own record outranks the posted verdict** (#1068 round 5): the ledger records what this run actually did, a comment records what somebody put on the pull request, and with the verdict ranked first a run that had shipped under the fallback — three host reviewers, recorded in its own ledger at this exact head — had that contract replaced by a leftover or collaborator-posted verdict and dropped `review-verdict-1..3` from the required set. A same-head record that says nothing about a panel is the run speaking too: it did not ship under one, so no comment may say otherwise on its behalf, and the surface measures instead. **And the run's own record has to be readable where the pull request is** (#1068 round 6): the ledger lives under the gitignored `.keel/state/`, so on a hosted `evidence-verify` or `merge` — the CI check, or any machine other than the one that shipped — there was no same-head record at all, the pin fell straight through to the verdict, and round 5's precedence held on the shipping workstation and nowhere else. The closure comment is the *same statement*, rendered from that same record, in the one place that travels with the pull request; it is read only from a trusted author (the same fail-closed `author_association` check a posted verdict gets — keel posts it on the operator's behalf, which is that authority and no more), only inside an actual closure comment, and only when its marker names the head under verification, so no untrusted comment can relax the contract in either shape. **And both run-record sources have to select the *latest* record for the head** (#1068 round 7): they are two copies of one statement, so ranking them against each other is meaningless unless they agree about *which ship* they are quoting — otherwise the machine with a ledger answers for the newest ship and the machine without it for the oldest. `ledger.latest_ship_run_for_pr` was already last-wins over chronologically appended rows; `evidence.shipped_panel_decision` was first-match over `pr_comments`, which GitHub returns oldest first, and round 6 rendered no marker at all for a panel that *sat*, so the later run had nothing to outrank the older one with even in principle. Ship a commit once under the fallback and again on a machine where the panel convenes, and on CI — no ledger, so the closure comment is the only voice — the old `decision=fallback` marker still pinned the host-bench contract onto a panel-reviewed change: leftover `review-verdict-*` LGTMs satisfied it and the panel's own `keel.jury-verdict.v1`, which may be a rejection, was never required. `closure._jury_panel` now renders every decision, `available` included, and `shipped_panel_decision` keeps the last match; `TestBothRunRecordSourcesSelectTheLatest` drives both sources from one sequence of ships and holds them to the same answer. **A missing key is not a `null`, either** (#1068 round 7): rank 3 silences the lower sources for a same-head record whose `run_context.jury_panel` says nothing, which is right for every row this feature writes — `ledger._run_context` always writes the key, so `null` there is the run answering "this tier named no panel". A row written *before* #1066 has no key at all: missing vocabulary, not a statement. Silencing on its behalf answered a question that run was never asked, and on a workstation still carrying one a change the panel really did jury would have had its posted ballots ignored and `review-verdict-1..3` demanded by a probe of the local machine. `juryavail.states_panel` draws the line; key-absent rows fall through to the closure comment and then to the ballots, exactly as they did before #1066 existed. The whole precedence now lives in one function, `juryavail.pin`, rather than in the order of two `if`-statements at a call site. Every source is pinned to the current head, and for the same reason: a pull request outlives its heads, so a ship of an earlier head that fell back would otherwise still be answering for the head being verified now — a stale run relaxing a live gate. A record from any other head is no record at all, and the surface measures for itself instead, which refuses either way round. Only with no record of the run's own and no posted verdict does a verification surface probe, as it did before. `availability.source` publishes which it was — `probe`, `pull-request`, `run-ledger` or `closure-comment` — and a pinned record carries `probed: false` whichever of them it came from, because "we were told" and "we checked" are not the same claim. **No source is read without a head.** The two pins were hardened one at a time and diverged: the ledger pin refused a blank head while the posted-verdict pin fell through to `evidence._matches_head`, which reads a blank `head_sha` as *no head filter* — right for counting evidence inside a gate that is head-agnostic throughout, wrong for a pin, which takes `review-verdict-1..3` off the required set outright. Any trusted jury marker anywhere on the pull request therefore counted as this head's on a run that could not resolve one (a detached checkout, an API answer with no `headRefOid`). One predicate, `juryavail.is_pinnable_head`, is now asked once above every source: with no head the surface pins nothing and measures its own machine, the same answer a record from another head already got. `evidence._matches_head` keeps its permissive reading and now says why it differs. The residue is narrower still and fails closed: a fallback-shipped change verified with neither a readable ledger nor its closure comment in reach, on a machine that *can* staff the panel, is held to the panel it did not run. Two machines can still *plan* the same tier differently, and each still says which it did.
  - **One measurement, seven resolvers.** `team._review_seats` is reached from exactly one function, so the call sites of `team.resolve_assignment` are the complete list of places a bench is resolved: `cli._review_assignment` (six review-aware surfaces), `contracts.build_command_contract`, `ship.assess`, the standalone commands, and `swarm.resolve_cluster_assignment`. The swarm was missed at first and it mattered — a cluster published `review_panel: jury` with `reviewer_count: 0` while the child `keel ship` the same machine launched fell back and seated three host reviewers, the very disagreement this closes one layer down. It is threaded now via `providerprobe.jury_availability_for_any_tier`, which measures once for the first tier whose policy names the panel (a swarm scores each cluster's tier while it partitions, so its caller cannot name one up front) and hands the one record to every cluster. An AST sweep holds the rule rather than the site: every `resolve_assignment` call in the package is handed the measurement, or is the one documented exception — `keel fixloop --no-project`, which resolves against an empty `TeamPolicy` that can never name the panel. The `block` refusal is raised in `team._review_seats` rather than copied to each caller — the one place a bench is resolved is the one place the check can neither be forgotten nor over-reach. Raising it at the *probe* was the over-reach: a probe answers a question about the machine, and `jury_availability_for_any_tier` asks the widest form of it — could *any* tier or band name the panel — before the swarm has scored a single cluster, so a `block` project could not plan a wave of entirely non-panel work on an unstaffable host. The probe now measures and never refuses; `decision: block` travels to every cluster and the run refuses on the ones whose review really is the panel. `JuryUnavailableError` and `refusal_message` moved to `keel.team` for the same reason and are re-exported from `keel.juryavail` under their original names — `keel.juryavail` imports `keel.team`, so the refusal has to live on the resolver's side of that edge, and it is one class under two spellings rather than two.
  - **One reading of "is this run's review the panel".** The probe's predicate is `team.panel_review_source`, the same tier-plus-bench overlay `team._review_seats` resolves the bench with. Written twice they drifted: the probe read `review.by_tier` alone, so a project whose `team.profiles.<name>.review` or `team.by_difficulty.<band>.review` named the panel had `keel ship --team-profile strict` publish `review_panel: jury` with `availability: null` on a host that could not convene one — the same bug by a second route, and one the call-site sweep could not see because that site *was* handed a measurement, just a `None` one. A surface that cannot name its band yet (the swarm scores difficulty inside the partition the record is measured for) asks the wider question — could any configured band make the panel the review — which is the same superset it already takes across tiers.
  - **`config_hash` does not move for anyone who does not use it.** `on_unavailable` is absent from the canonical `team` block when unset, like every other optional field there, so a project that never names the setting keeps the hash it had. Writing it explicitly — even as `fallback` — is a config change and rotates the hash, which is exactly the guarantee `knobs.team` has made since #1014: the hash changes *iff* `team` does. Depends on #1065: with the independence claim opt-in, a same-vendor fallback bench is mergeable, which is what makes this useful for a single-vendor project.
- **`website/params.js` is generated now, so its own header comment is finally true** (#1051): the file has opened with "generated from src/keel/adapters/commands frontmatter" since it was written, and no generator existed — it was hand-synced under a comment saying it was not, which is the most reliable way to drift. It did: `/keel:swarm` carried `--rebalance` and `--landing <batch|funnel|auto>` chips for flags its adapter body never defined, and the `--review-delegate` hint change from #1039 never reached the site at all (#1044 round 2 caught it). `TestTheSiteArgumentHintsAreTheAdapters` could see the drift and could not fix it.
  - **`keel install-adapter site` / `make site-params`** renders the file from the adapter frontmatter, in the same repo-level shape `plugin` / `make plugin` already had — `keel.install.site_params_files()` is the pure generator, `install_site_params()` the thin write. It is deliberately *not* part of `install-adapter all`, which installs the per-project surfaces; a project that installs keel's adapters must not acquire a `website/params.js`.
  - **Everything the file publishes is derived, so there is no hand-maintained region.** `desc` is the frontmatter `description`, `hint` is its `argument-hint`, and the flag chips are that hint's top-level `[...]` groups (`keel.install.hint_flags`) — brackets nested inside a placeholder stay inside their chip, prose outside brackets is not one. Regenerating reproduced the committed file field-for-field except the `ship` card's stale `--compound` chip, which the hint had already folded into `--compound|--profile <standard|compound>`. Commands are emitted sorted by adapter filename, the ordering rule every other generated surface uses; the site reads the object by key, so the order is free.
  - **CI fails on a stale copy the way it already does for the plugin command bodies.** `tests/test_install.py::TestSiteParamsGenerator` asserts the committed file is byte-identical to the generator's output. `TestTheSiteArgumentHintsAreTheAdapters` stays on as the independent second opinion: it reads the frontmatter with its own regexes rather than through the generator, so a bug in the generator cannot make the check vacuous.
  - **`parameter-reference.md` listed four install targets while the CLI accepted five.** The value cell and examples for `keel install-adapter` predated `site`, which the command's own `--help` and its `unknown target …; valid: …` refusal had named from the first commit — the same species of drift this entry removes, a hand-typed list beside a machine-checked one. Both list it now, and `tests/test_docs_claims.py::TestTheDocumentedInstallTargetsAreTheAcceptedOnes` reads the accepted set out of the dispatcher itself (the `args.agent == "…"` branches plus whichever `install.<TUPLE>` its fan-out branch tests), *runs* every scraped target against a temporary root so a regex artifact cannot redefine what "accepted" means, and fails when the reference disagrees or when a repo-level target has no pasteable example.
  - **The flag-chip check caught invented chips and not missing ones.** `test_every_published_flag_is_in_the_hint_it_came_from` asked only whether each published chip appeared somewhere in its hint — the wrong direction, since the chips are what the generator computes: a scanner made to drop a chip regenerated the file without it and left both suites green. `test_the_published_flags_are_exactly_the_hints_own_groups` replaces it, scanning each adapter hint for its own top-level `[...]` groups — reduced innermost-first, deliberately a different route from `hint_flags`' single depth-counting pass — and asserting the published list *equals* that scan. A sibling test pins the other half of the split's silence: nothing in any of the seventeen hints sits outside a bracket, so what the split discards is empty today, and the day a hint grows prose it fails instead of swallowing it.
  - **Two corrections to the guards themselves.** `test_the_header_carries_no_equals_sign` credited its rule to "the site's own drift check"; nothing under `website/` parses `params.js` — the browser just runs the assignment, and the readers that split on the first `=` are these tests. The rule stands, the attribution is now true. And the byte-identity guard compares the parsed payloads entry by entry before the bytes, so a stale file names the command and the field that moved instead of printing a truncated 200-line diff above the advice to regenerate.
...
- **`knobs.implement_mode: tdd` — a test-first s4, with the commit order verified rather than promised** (#1020): some implementers skip parts of an issue, and nothing catches it until a reviewer reads the diff. keel had no way to ask for TDD. `implement_mode: tdd` (or `--tdd` for one run) makes s4 run **two phases against the same provider**, one `keel delegate run` call and one commit each — phase A commits the failing tests derived from the issue's acceptance criteria as a test-only diff, with the gates expected red; phase B implements until they are green — and adds one blocking gate at s8 that checks it actually happened.
  - **The `tdd-order` gate is a pure function of the commit list and the path policy.** The new `keel.tdd` module parses a `git log` into commit records and decides: the branch history is readable at all (an unreadable one blocks — it is not an empty branch), the first non-merge commit touches at least one path and **only** test paths, that commit *adds or modifies* at least one test (a first commit that only runs `git rm` over the suite is the opposite of writing it), no later commit *removes* a test — deleted outright **or renamed out of the test paths**, since `git mv tests/test_a.py src/legacy_test_a.py` stops the suite collecting it exactly as `git rm` does, while a rename within the test paths stays an ordinary move and a copy is not a removal at all (its source still exists) — a later commit touches an implementation path, and the rest of the gate run is green. The only I/O is one `git.commit_log` read behind the existing seam, so the whole gate is unit-tested offline against commit lists at 100 % line + branch.
  - **It reads commit order and paths, and nothing else**, and says so where the guarantee is claimed (the `keel.tdd` module docstring, `check_order`, and a call-out in `configuration.md`). It never runs phase A's tests, so it cannot report that they were red, and it cannot tell whether the committed tests assert anything. Three residuals are documented rather than papered over: a first commit adding an *empty* file under `tests/` satisfies the "adds a test" rule, and so does one that merely renames an existing test within the test paths; a test deleted inside a **merge from a side branch** is not judged, because merges are skipped so that a deletion made on the base is never blamed on this implementer — telling a base merge from a side merge is not something the commit list can do, both being a second parent; and nothing here says the tests are good. Each is a reviewer's catch; the gate makes the *shape* of a test-first run machine-checkable, not the quality of the tests.
  - **The branch is read by ancestry, not by date**: `git log --topo-order --first-parent --reverse --name-status base..HEAD`. Commit-date order (git's default) meant that once a branch integrated its base at s10, a base commit dated before the tests commit sorted ahead of it and was judged as this implementer's first commit — the same topology passing or blocking on nothing but timestamps. `--first-parent` follows only this branch's own line, so commits a base merge brought in are not on it; the merge commits themselves stay and are skipped rather than judged. `--name-status` rather than `--name-only`, because a name alone cannot tell an addition from a deletion.
  - **A project says where its tests live, or the gate fails closed.** The paths come from `policy_pack.test_groups.*.test_paths` — a new optional field. The fallback is **whole-config, not per-group**: with no `test_paths` anywhere the globs are the union of every group's `paths`, and as soon as **any** group declares `test_paths` the globs are the union of the declared ones only — a group that declares none then contributes nothing, its `paths` included. Group `paths` are *selectors* (keel's own `unit` group selects `src/**` as well as `tests/**`); read as test paths they would make the check vacuous, and the implementation commit that must follow would have nowhere to land. A project declaring nothing at all blocks with the key to add, rather than passing on a check nothing performed.
  ...
  - **Both phases are recorded, with who ran each.** `keel plan`/`keel ship --json` publish `contract.implement_mode` (`mode`, `source`, `phases`, `gate`) as a sibling of `workflow_profile` — `tdd` is an s4 profile the way `compound` is a workflow profile, and the backbone step ids are unchanged. The ledger's `run_context` gains `implement_mode` and one `implement_phases` entry per phase carrying that phase's commit **and its implementer**, so *"the same provider wrote the tests and the implementation"* — the rule the profile rests on, which nothing else in the record could show — is auditable rather than assumed (emit-only, like the `knobs.team` gate seat: keel records what the orchestrator reports). `keel ship --phase-implementer tests=<label>` records a phase whose implementer differed from `--implementer`. The closure comment renders `Implement: TDD (tests <sha> by <implementer> → implementation <sha> by <implementer>)` — a line emitted only for a TDD run, so every closure comment keel has already posted reads identically.
  - **What changes for a run that does not use the profile:** its ledger `run_context` gains two keys, `implement_mode: null` and `implement_phases: []`, and nothing else. The rendered closure comment is byte-identical, and a record written before this release stays readable (both keys degrade to those values).
  - There is no `--no-tdd`: a project that configured the contract has said the contract is the policy, and a flag that switched it off from a command line would make it advisory. `implement_mode: default` is omitted from the canonical config, so adopting the knob does not rotate `config_hash` for a project that has not. **keel's own `config_hash` does rotate in this release**, for the unrelated reason that `projects/keel.yaml` (and its `.keel/project.yaml` twin) adopts the new `policy_pack.test_groups.unit.test_paths` so `--tdd` on keel itself checks against `tests/**` rather than the `src/**` selector.
...
- **The swarm has an org chart: CTO → team lead → worker, with difficulty scoring and per-cluster staffing** (#1017): `work-block`, `overnight` and `swarm` passed only `operator_consent.delegated_agent_scope` to their children — `--delegate`, `--review-delegate` and any notion of a team were dropped at the handoff, so a parallel batch could not say *this cluster is hard, give it the strong implementer at high effort; the easy ones go to the cheap model*. `swarm.py` produced waves and clusters with no notion of difficulty, no assignment and no lead, and its worker records carried placeholder `claude`/`default` fields whatever the project's policy said.
  - **Difficulty is a band, and a band is not a risk tier.** `keel swarm-plan --json` now publishes a `difficulty` per cluster — `band` (`easy`/`standard`/`hard`), `score`, `tier`, `file_count`, `dependency_depth`, and the `signals` that produced the score. It is a pure function of four things that already exist: the risk tier from `knobs.tier3_globs`, the predicted file count, `priority:*`/`size:*` labels, and how much already-scheduled work the cluster depends on. A tier answers *how dangerous is this change*; a band answers *how much work is it*. A one-line fix to a tier-3 glob is dangerous and trivial; a twelve-file docs migration is safe and long, and only the second question decides whether the expensive implementer is worth spending on it.
  - **`knobs.team` gained the three keys that staff a batch**: `lead` (the seat that coordinates a batch of ships and that its workers report through), `by_difficulty` (band → bench) and `profiles` (operator-selectable benches, chosen with `--team <profile>`). A **bench** is one type with four optional fields — `lead`, `implement`, `review`, `effort` — and both tables hold it, so "staff this work from this bench" resolves through one code path rather than two that can disagree. Each field resolves down the bench list on its own, so a `--team` profile naming only reviewers leaves the band's implementer standing instead of silently dropping it. Precedence, per field: per-run flags > `team.profiles.<--team>` > `team.by_difficulty.<band>` > `team.implement.by_role` > `team.implement.default` > the deprecated `implementer_agents` > the host agent — a bench outranking the role deliberately, because the role says *which part of the system this is* and only the band can say *what this piece of work costs*. `effort` is the one inversion: a seat naming both a provider and an effort is one statement, so the seat beats the bench and only `--effort` beats the seat.
  - **The swarm calls the same resolver as everything else.** Each cluster is staffed by `team.resolve_assignment` with *that cluster's* role, tier and band, so `swarm-plan` and the child `keel ship` cannot disagree about who runs an issue. `keel validate` extends to the new seats: an unknown provider inside a bench is reported with its config path, a bench implementer counts for the `gate.distinct_from: implementer` check, a bench's reviewer list obeys the same empty/≤3/`jury` rules as a tier's, and a `by_difficulty` key that is not a band keel emits is refused — otherwise a `medium:` row beside `easy:`/`hard:` matches nothing the scorer produces and the table silently does nothing.
  - **Scoring and staffing never move the waves.** Both run after the conflict partition and never feed back into it, so re-staffing a backlog — a new `by_difficulty` row, a `--team`, a `--delegate` — changes who runs a cluster and cannot change which wave it lands in. That is asserted as one comparison in `tests/test_swarm.py`, and it is what makes the partition and the staffing independently reviewable.
  - **`/keel:swarm` is now a CTO.** It clusters, launches **one team lead per cluster**, and lands waves — it does not implement, review, or drive a child ship. A lead runs its cluster's ships with the providers its `assignment` names, appends that team to each child (`--delegate`, one `--review-delegate` per staffed slot, `--role`), and reports through the existing worker status record. `keel swarm-run` appends those flags for the runs it starts itself, and a lead never re-scores or re-staffs its own cluster: if the work turns out heavier than the band said, it reports that and the CTO re-plans.
  - **The board shows the chain.** `SwarmWorkerStatus` carries `lead` and `difficulty`, seeded from the plan's own assignment rather than the record's defaults, and `keel swarm-status` gained **Lead** and **Band** columns — so the operator's question *who is running this, and why that provider* is answered where a running swarm is actually watched. The worker-state update was rewritten to `dataclasses.replace`: the field-by-field rebuild it replaced had to name every field, so each field added to the record was reset to its default by the first status update.
  - **`assignment` gained five always-present keys** in `keel plan --json`, `keel ship --json` and every command that publishes the block: `lead` (the seat coordinating a batch — the host agent when `knobs.team.lead` is unset), `effort` (the implementer seat's effective reasoning effort), `difficulty` (the band it was resolved for, `null` outside a swarm plan), `team_profile` (the `--team` name, `null` when none) and `bench` (config paths of the benches that applied, `[]` when none). Present on every resolution rather than only when staffing is configured, so a reader can always ask *who led this, at what effort, from which bench* without testing for the keys' existence.
  - **`keel ship` and `keel plan` accept `--effort` and `--team` too**, because they are the *children*: the published `child_args` named both flags and the adapters told every batch to append them verbatim to `/keel:ship`, while `keel ship`'s own parser answered `unrecognized arguments`. A promise in the contract the parser could not keep. Both flags now resolve through the same `cli._review_assignment` every review-aware command shares, and through `ship.assess`, which *replaces* the planned assignment once the real tier is known — dropping them there would have published a ship contract whose team disagreed with the plan the operator had just read. So a child inherits its parent's difficulty bench and re-resolves to the same seats; `--delegate` on the child's line still overrides what the bench chose.
  - **All six commands that resolve a review contract accept `--effort` and `--team`.** `ship`, `plan`, `review`, `step-verify`, `evidence-verify` and `merge` share one resolver precisely so they cannot disagree about the bench, and a flag that reached only the dispatching half reintroduced the disagreement it was built to end: a `--team` naming a one-seat profile dispatched one reviewer while `evidence-verify` and `merge` still demanded the tier's three verdicts — a gate no run of that project could satisfy. `keel plan` also *accepted* both flags and threw them away, publishing `effort: null` / `team_profile: null` / `bench: []` with no unknown-profile warning while `keel ship` on the identical command line published all four; and ship's own **preflight** contract — the only one an operator sees when a run halts before gates — was built without them, so a halted run named no team where a completed run named one. All three `build_command_contract` call sites now thread both, and `ship.md`'s s7/s10/s12 prose tells the host to carry the same values to `review`, `evidence-verify` and `merge`, with `contract.assignment` named as the source of truth.
  - **A bench `effort` is validated against every seat it could land on, and only against the seats a run can actually reach. One run resolves exactly one difficulty band and at most one `--team` profile, so two entries of the same table never apply together: reporting `by_difficulty.hard`'s effort against `by_difficulty.easy`'s implementer described a combination no run can produce, which is how a validator teaches people to ignore it. The reachable set is the bench's own `implement` seat, else the *other* table's implement seats plus the role/default/legacy ones every run falls through to.**  `by_difficulty.hard: {effort: high}` beside `implement.default: {provider: agy}` validated clean and resolved to an effort `agy` cannot honour without a model — the bench bypassed every rule a seat-level `effort` has faced since #1014, because a seat's effort sits beside its provider and a bench's lands on a seat written somewhere else. It is now checked against each candidate implementer under the same rules and in the same words, plus one the bench case adds: a `subagent:` implementer has no reasoning-effort dial at all. A seat naming its own `effort` never receives the bench's and is not flagged; the host-agent fallback is not checked, because validation that depends on one operator's default is the rule #1014 refused.
  - **A role label cannot break the batch.** A role becomes a `--role` argv token in every child, so `role:--live` on one issue would have been parsed as a flag and killed every child ship in the run. A role outside `[A-Za-z0-9][A-Za-z0-9._-]*` is dropped — there is no quoting that stops `--live` being a flag — and the reason lands in `assignment.warnings`, where a lead is already told to look. The child resolves its role from the issue's own labels instead.
  - `dependency_depth` records the depth actually observed and the **cap bites the points**, not the number: a cluster sitting on nine earlier issues and one sitting on three score the same, but they no longer *read* the same in a plan that is the only place anyone would see the difference.
  - **`work-block`, `overnight` and the three `swarm-*` commands accept `--delegate`, `--review-delegate`, `--effort` and `--team <profile>`** and hand every one that was set to each child `/keel:ship`. `session_contract.work_block.delegation` publishes both halves — `effective` (what the operator passed) and `child_args` (the exact flag list to append) — with a fixed order, because the handoff line is quoted verbatim into the session report and a set that reordered itself between two issues would read as two different teams having run. The adapters name the flags, carry the handoff template, and record the effective staffing in the report; `tests/test_adapter_team_propagation.py` holds the three together, failing the build if a published flag is one a command does not accept or an adapter does not mention.
...
### Changed
- **The s7 panel → s9 fixloop handoff is written down** (#1052): #1015 gave `keel review --from-jury --json` a `panel` block whose findings are "the s9 fix-loop input", and #1016 gave s9 its findings through `keel fixloop brief --findings <file>`. The two compose exactly — the `panel` block *is* a `{"findings": [...]}` envelope, the shape `--findings` already reads as-is — but no sentence said so, leaving the one hop between the two commands to be guessed at by whoever ran a panel tier. s9 now names the field and the command, and says the array holds the panel's **verified** consensus: a claim the verification round did not uphold is not in it and must not hold a merge.
...
### Fixed
- **Three documentation claims that made a reader or an agent fail** (#1055), found by the 1.20.0 pre-release audit. Docs only — no source behaviour changed.
  - **`keel run-gates --concurrency N` never existed.** `cli.md`'s `run-gates` heading listed the flag and a "Concurrent Gate Execution" paragraph offered `knobs.concurrency: N` as the alternative; `README.md` repeated the claim as a feature bullet, and the marketing site's "Security & policy presets" card (`website/content.js`) sold "concurrent gate execution (`--concurrency`)" as something a project gets "with zero custom scripts". `build_parser()` defines no such flag, and `concurrency` is not in `knobs.properties`, so a project that took the paragraph at its word failed `keel validate` with `$.knobs: unknown property 'concurrency'`. Concurrency is real but **library-only**: `keel.gates.run_gates(..., concurrency=N)`, which the CLI never calls with anything but the default. The heading, the README bullet and the site card all drop the claim; the paragraph now says what the capability actually is and where it lives.
  - **`keel window <project.yaml> --root .` exits 2.** `keel window` reads no repository state and takes no `--root`; #1019 corrected four adapter bodies and missed the two that spell the command in prose rather than in a shell fence — `morning.md` step 5 and `overnight.md`'s main loop — which is exactly the blind spot `TestEveryDocumentedInvocationParses` leaves, since it only parses fenced lines. Fixed in both adapter sources and regenerated across the six surfaces (`commands/`, `.claude/commands/keel/`, `.agents/skills/keel-*`).
  - **`AGENTS.md` taught the deprecated dispatch.** The Agents section resolved the implementer from `knobs.implementer_agents`, which the schema marks deprecated by `knobs.team.implement.by_role` and which keel's own config no longer sets, and it named only two of the five seats. Rewritten against `team.implement`, with a line each for the mandatory `team.gate` second opinion; for the panel-as-the-review mechanism a project opts into with `team.review.by_tier."<tier>": jury`, which makes s7 the panel rather than a host bench — and for keel's deliberate decision *not* to adopt it at its own tier 3, which stays three named seats; for the `team.fix` seat `keel fixloop brief` resolves at s9, escalating it along `fixloop.STAGES` (`implementer` → `gate` → `host`); and for the `team.lead` seat that coordinates a batch of ships, which keel leaves at its default of the host agent. All five seats are now named.
- **Nine features shipped in 1.20.0; four of them were invisible on the website and two had no per-flag reference** (#1056), the second half of the pre-release documentation audit. Docs, one schema description string, and two adapter-prose lines — no source behaviour changed, and `config_hash` is byte-identical before and after (`0c404c09…` for `projects/keel.yaml` and its `.keel/project.yaml` twin), because it hashes the resolved `ProjectConfig` and never the schema.
  - **`docs/keel/parameter-reference.md` stopped calling itself exhaustive, and gained the two commands a 1.20.0 run cannot be driven without.** It opened as "the exhaustive per-flag reference for the `keel` CLI" while covering 41 of 61 subcommands, and named neither `keel review --from-jury` nor `keel fixloop` — the posting half of the panel-as-review feature and the whole of the s9 fix loop. Full sections are added for `keel review` (including `--from-jury`: the ballot-per-verdict mapping, the mutually-exclusive bundle rule, the panel block that feeds s9, and the panel-tier precedence) and `keel fixloop brief` (the findings shape, the `implementer → gate → host` ladder, the budget's non-zero exits, and the no-config refusal). The remaining **eighteen** uncovered subcommands are now named in the opening, with the scope restated as the `/keel:ship` grammar and the subcommands a ship run drives; `cli.md` remains the reference with a section for every one, a property `tests/test_docs_claims.py` already enforces.
  - **The website described a pre-1.20.0 product.** `website/content.js`, `index.html` and `docs.html` contained no mention of `knobs.team`, `--from-jury`, `implement_mode` or `--tdd`. The `project.yaml` sample on the home page now carries a `knobs.team` block (implementer, gate seat, per-tier reviewers with `"3": jury`, and `fix`) plus `implement_mode: tdd`; the config-key table, the CLI list (`keel review --reviews`, `keel review --from-jury`, `keel fixloop brief`), the `/keel:ship` card, the s4/s7 backbone blurbs, the Configuration / Supported-AI-models / Parameter-reference doc entries and three new FAQ answers cover the team policy, the panel-as-review and the test-first mode. The parameter-reference card no longer repeats the exhaustiveness claim the page itself dropped. **The sample was validated before it was published, and did not pass** — so it is fixed too: it showed `gates:` as a mapping of names to commands (the schema takes an array of gate *names*, with the commands in `knobs.build_gate_cmd` / `knobs.lint_cmd`), `policy_pack.labels` as a flat list (it is an object of label groups), `tier3_globs` under `policy_pack` (it is a knob), and `core:` in place of the required `extends` + `core_version`, with `policy_pack.name` missing. Four of those are the exact `unknown property` / `expected type` refusals the paragraph beside it advertises. `keel validate` now says `OK` on the block as published.
  - **`website/content.js` pointed readers at a deprecated knob.** The Keel Swarm cross-model routing paragraph named `knobs.implementer_agents`; it now names `knobs.team.implement.by_role`. `docs/keel/commands.md` listed `implementer_agents` among the values every command reads with no deprecation marker at all — it now names `team` and carries the marker with a link.
  - **`README.md` never mentioned the test-first implement mode.** One bullet beside the team-policy bullet: the two s4 phases, the blocking `tdd-order` gate at s8, the published `contract.implement_mode`, and `--phase-implementer` for a run whose phases had different seats.
  - **`docs/keel/models.md` and `docs/keel/command-contracts.md` described host reviewers only.** Each gains a section on `knobs.team.review.by_tier."<tier>": jury` and `keel review --from-jury` — the ballot-per-verdict mapping with its vendor/model provenance, the same-call head pinning, the `panel` block as the fix-loop input, and the fact that no per-run flag can take a panel back off. Both say plainly that **keel has not adopted this for its own tier 3**, which stays three named seats with `jury.mode: advisory`; the measured difference is visible in the evidence set's own wording (*"Distinct posted ai-jury panelist verdict"* on a panel tier versus *"Distinct posted s7 reviewer verdict"* on keel's).
  - **The `knobs.team` schema description omitted the `fix` seat.** It enumerated implement, gate, review and jury while `fix` sits in the same object and surfaces in the published `assignment`; schema descriptions reach users through `keel validate` errors, so an omitted seat is an undocumented one. Description string only — the config hash is unchanged, as measured above.
  - **Three aspirational or mis-rendered lines.** `docs/proposals/keel-architecture.md`'s CLI-surface sketch shows seven subcommands that were never implemented plus `keel merge --window-check`, and is now fenced by a note saying so and pointing at `cli.md` (attribution is `keel attribution`; the merge window is re-checked inside the lock unconditionally). `docs/proposals/keel-swarm.md`'s `--max-workers N` line offered a `knobs.swarm_concurrency` default that `keel validate` rejects as an unknown property — the flag ships with a fixed default of 4, and the line now says which half is real. `src/keel/adapters/commands/flake-audit.md` rendered its own `--runs` argument immediately after a `keel run-gates` invocation, reading as a flag on that command; `keel run-gates` has no `--runs` (adapters regenerated).
- **`keel review` was jury-blind: it now takes the same jury flags as every other surface** (#1043): after #1014/#1039 the reviewer bench was a pure function of config + tier + role + `--reviewers` / `--review-delegate`, and all six review-aware surfaces agreed on it. One asymmetry survived, and it was jury-only: `keel review`'s parser defined none of `--jury` / `--no-jury` / `--jury-advisory`, and `_cmd_review` hardcoded all three false when it resolved the review contract and again when it built the namespace for `--verify`. So on a plain (non-panel) tier-3 project a run shipped as `keel ship --no-jury` and posted with `keel review --verify` re-checked a *stricter* contract than the one it was planned against — `jury-verdict` reported as required, for a verdict the ship run was told never to post. Found by the third reviewer's 810-evaluation cross-surface sweep on #1039 (three configs × three tiers × fifteen flag sets × six surfaces), which reported 140 divergences: bench identical in every one, only the jury line differing, and permissive-only.
  - **The flags are accepted rather than the asymmetry documented.** `keel review` now defines the same three flags and threads them into `ship.resolve_review_contract` and into the `--verify` re-check, so the jury line it resolves is the one `plan`, `ship`, `step-verify`, `evidence-verify` and `merge` resolve from the same inputs. Precedence is unchanged and still `resolve_jury`'s: a `knobs.team` jury-panel tier > `--no-jury` > `--jury` > tier-3 auto > off. **The bench still does not move with them** — that invariant is what #1014 exists to hold, and the flags own the jury line only.
  - **`--from-jury` is orthogonal to the three, and documented as such.** `--from-jury` says *where the verdicts come from* (the panel's ballots, plus its consensus record, whose declared size sizes the bench on a panel tier); the flags say *whether the contract requires a jury verdict*. On a jury-panel tier the panel outranks both, and `--no-jury` / `--jury-advisory` stay recorded in `assignment.warnings` and not applied.
  - **`--from-jury` now hands over the panel's vendor span, not only its size.** `_cmd_review` passed `jury_panel_size` and nothing else, so on a *non-panel* tier — where a jury spanning fewer than `min_vendors` vendors is downgraded `gating → advisory` and its verdict drops out of the required evidence — this surface measured no vendor count and kept resolving `gating`. Measured: a plain tier-3 project with a three-ballot single-vendor panel had `keel review` reporting `jury-verdict` as required while `evidence-verify`, reading the `vendors: 1` line off the verdict `keel review` had just written, resolved `advisory` and required no such thing. It now passes `jury_participating_vendors=len(panel.vendors)` from the panel it is holding — the same number `evidence.jury_participating_vendors` recomputes from the posted verdict — and both the short-panel and the two-vendor case are pinned in the agreement suite.
  - **One definition, six surfaces.** The flag group moved into `cli._add_jury_flags`, which every review-aware parser now calls; six hand-written copies is how the review parser came to have none. `keel review --json` publishes the contract it resolved as `review_contract` (and the human summary gains a `jury:` line), so the surfaces can be *compared* rather than assumed equal. `tests/test_review_contract_agreement.py` drives `keel review` through the flag cross-product, pins the jury line across plan/ship/review/evidence-verify/step-verify on a teamless tier-3 config (the only shape where the flags bite — a panel tier outranks them), asserts the parser group on all six, and adds an AST sweep refusing a `--jury` spelled anywhere but the helper. Removing the fix fails 17 of that module's 18 tests, so the sweep is not vacuous.
  - Docs: the `keel review` section of `docs/keel/cli.md` gains a jury-flag subsection with the `--from-jury` precedence; the s7 prose in `src/keel/adapters/commands/ship.md` now tells the host to repeat the run's jury flags on the `keel review` call (adapters regenerated). `docs/keel/configuration.md` also records, next to `knobs.team.jury`, that `jury.mode` is one **global** knob while review panels are **per-tier**, so `keel validate` refuses "gating panel at tier-3 + advisory jury at tier-1/2" whole-config although `resolve_jury` would scope it correctly at run time.
