...
- **s7 probes the jury panel before it dispatches it, and a project can allow a recorded host-bench fallback when it cannot be staffed** (#1066). On a tier whose `knobs.team.review.by_tier` names `jury`, the panel *is* the review, and #1014 round 3 deliberately made it so no operator flag can take it back off. That is right while the panel can run. When it cannot — an agent CLI not installed, unauthenticated, or out of quota — the tier had no way forward at all: `keel evidence-verify --pr N` reported `missing: jury-verdict` against a panel the machine could not convene, and nothing an operator could type would change it.
  - **The probe is the one keel already runs.** `keel doctor --providers` answers "is this provider usable here, right now" per provider, with the reason it is not. `keel.providerprobe.jury_availability` reuses that machinery — a `PATH` lookup and one `--version` call per CLI vendor, an env-var *name* check per hosted API, one loopback request for Ollama — rather than adding a second prober that could disagree with the first. It runs **only** on a tier whose review policy is the panel, so a project that has not made the panel its review (keel's own `projects/keel.yaml` included) pays nothing and behaves exactly as before.
  ...
  - **The fallback changes who sat, never how many.** It publishes the tier's own reviewer count and the tier's own required evidence — three `review-verdict-*` items at tier-3, not two. It drops `jury-verdict`, and it must: there is no panel to produce one, and requiring it would leave the tier stuck one layer down.
  ...
  - **#1014's principle survives.** What may not take the panel off is an operator's *preference*; there is no flag that asserts availability. Availability is a fact about the world, and it is allowed to change the outcome precisely because it is recorded. The consequence is stated rather than hidden: two machines can resolve the same tier differently, and each says which it did.
  - **One measurement, eight resolvers.** The probe is taken once per run and threaded through every site that resolves a bench — `cli._review_assignment`, `contracts.build_command_contract`, `ship.assess` and the standalone commands — so the published assignment and all six review-contract surfaces agree. The `block` refusal is raised at the probe itself rather than copied to each caller, because the one place the measurement is taken is the one place it cannot be forgotten.
