...
### Fixed
- **A verdict that names a symbol without calling it is a review, not a receipt** (#1106). `verdict_substance`'s anchor set read a dotted identifier only *with* the parentheses, so a gate verdict on ai-jury#753 that traced `cache.cache_key`, `config.config_hash`, `JuryConfig.__post_init__` and `adapters.GenericCLIAdapter._prompt_mode` through eleven symbols, four call sites and three test classes was told it "names nothing concrete". Every anchor missed for the same reason — the reviewer wrote `cache.cache_key`, and adding parentheses to something you are not calling is worse prose. A gate that refuses genuine review over punctuation teaches the operator to reword verdicts until the checker is happy, which is the receipt-shaped behaviour #926 exists to stop; and a verdict is head-pinned, so the only remedies were deleting a real review or re-running the seat and hoping it phrased things differently. Four shapes are read now: a file named without its directory (`evidence.py`, `CHANGELOG.md`), a dotted identifier with no parentheses (`cache.cache_key`, `Class.method`), a bare `snake_case` identifier (`collect_static_hints`, `_prompt_mode`) — any **two distinct** of which together stand in for an anchor, none of them alone — and an escape clause opened by any of six verbs — `checked`, `traced`, `read`, `ran`, `inspected`, `verified` — whose object may sit on the next line as a bullet, because "Checked:" with the list under it is the same clause laid out.
  - **Measured against the record, not asserted.** Replayed over all 1,323 review verdicts posted across keel and ai-jury (384 pull requests): every one of the 816 that passed still passes, 111 of the 507 refused now pass, and the number of pull requests carrying at least one accepted verdict rises from 254 to 299. Of the 75 verdicts in that corpus opening `Reviewed <title>…`, 72 stay refused, and all ten renderings of the default `render_review_verdict` template stay refused.
  - **A dot is not enough to make a symbol.** `module.symbol` and `github.com` are spelled the same way, so the dotted token must carry a mark prose does not use — an underscore, an internal capital, a run of capitals, or a capitalised segment — or name a kind of file. Read without that, the commonest dotted "anchor" in the corpus was the `claude.ai` in a *Generated by Claude Code* footer, which let eight verdicts pass on their own signature. The direction matters: the rule lists the file types a review can name, because that list closes, and does not list the hostnames to refuse, because anyone can register the next one. The mark is not sufficient either, and the next bullet is why: `GitHub.com` carries the capitalised segment exactly as `Config.parse` does.
  - **A bare identifier is read by its underscore**, which is a mark English prose and product names do not use. Reading CamelCase meant "The GitHub and GitLab side of this is unchanged" cleared a floor of two while naming nothing in the change, and nothing separates `GitHub` from `JuryConfig` lexically. CamelCase symbols still read wherever they are written with a dot or in backticks, which is how a class is normally named. What the underscore joins is two alphanumeric segments, so `post_init` is what matches inside `__post_init__` and a lone `_private` or `__dunder__` was never read — the comment claimed otherwise and has been corrected. The floor stays at two *distinct* tokens — at one, the corpus says the rule readmits 22 of the 75 rubber stamps #926 is named for; at two it admits none.
  - **An unbackticked token corroborates; it does not anchor.** Twice this rule was widened and twice the widening admitted something that is not a symbol — `claude.ai` in a *Generated by Claude Code* footer, then `GitHub`/`GitLab` as bare CamelCase — and twice the answer was a finer character class. The third round found `Node.js`, `Next.js`, `Vue.js`, `D3.js` reading as source files and `GitHub.com`, `GitLab.com`, `OpenAI.com`, `SourceForge.net` reading as dotted symbols, which is where refining stops being worth doing: `Node.js` and `evidence.py` are one shape, `GitHub.com` and `Config.parse` are another, and a blocklist of products or TLDs cannot be finished any more than a blocklist of hostnames could. So the *count* carries it instead of the spelling. A `path/to/file.py`, a `file.py:42`, a backticked token and a called `module.function()` still anchor alone, because the backticks and the punctuation are the author pointing; a bare filename, a bare dotted token and a bare identifier anchor only in pairs, or singly inside an act-of-review clause where the verb supplies the other half. One token counts once however many patterns read it, so `cache.cache_key` is one piece of evidence and not two. Measured: this refuses all eight strings above, still regresses nothing that passed before #1106, and costs 30 of the 141 recovered verdicts — 22 of those 30 being the default template's "Scope reviewed:" line with a single token dropped into it, which is the receipt shape #926 is named for. Dropping the two bare forms outright instead refuses the same eight but keeps only 98 of the 141 and loses a review that names `evidence.py` and `contracts.py`, so the corpus chose corroboration over the narrower fallback.
  - **An act-of-review verb has to name what it read.** "Reviewed the whole diff and everything looks correct here" was already refused, but swapping the first word for `read` — one of the five verbs added here — made the same sentence pass, because the clause asked for a verb and eight characters and never for an object. The five added verbs now require an object that names something. `checked` keeps its free-form object: it is the incumbent and it has a record, and holding it to the same bar refuses 13 real reviews in the corpus that say "Checked the formula syntax, the version URL and the checksum placeholder" — three things named in English and no symbol. The asymmetry is the measurement rather than a preference; one verb is calibrated and five are new.
  ...
  - **The extensionless path the issue proposed was refuted by the corpus and is not shipped.** A bare `word/word` token was meant to read `docs/CHANGELOG` and `.github/workflows`; replayed, it read `1/3`, `2/3`, `I/O`, `offline/live` and `build/lint/bandit` — slash-separated alternatives are simply how these reviewers write — matching 305 of the 507 refused verdicts on prose that points at nothing. A stricter two-slash variant was no better in kind and reached only nine verdicts no other anchor did; `docs/CHANGELOG` in the verdict this issue is about is read by nothing — `_verdict_corroborators` returns an empty set for it — and that verdict passes on everything else it names, which is the point: a review that mentions eleven symbols does not need its one extensionless path to be read. `tests/test_verdict_substance.py::AnExtensionlessPathIsNotAnAnchor` records the decision so it is not re-proposed from the issue text alone.
  ...
  - **An added verb governs its own sentence, not the rest of the line.** The object capture ran to the end of the line, so a token in a *later* sentence earned the clause for a verb that never governed it: "Read the whole diff. See `cli.py` for context." passed, and "Read the whole diff" is exactly the receipt the object requirement exists to refuse. The boundary is a period followed by a space or a line end, not any period, because `evidence.py` has to survive inside an object. Re-measured on a fresh pull of every verdict comment in both repositories — 1,421 by then, against the 1,323 of the replay above, because the gap is this week's own verdicts and because this pull did not filter through `_is_review_verdict_body`; the two numbers are different fetches, not a disagreement: still **zero** verdicts that pass on `main` are refused, and it costs two of the recovered ones — both of which named their subject only as `s8`, `s10` and `step-verify`, forms this rule does not read at all, so the line-scoped capture had been rescuing them by accident rather than by design. Found by the gate review.
  - **A verb governs an anchor or two tokens, not one of either.** The object test had a floor of one corroborator, and `Node.js` is a corroborator — so "Ran the Node.js suite and everything is fine" was a receipt wearing a verb, under the verbs this change had just added. An anchor still governs alone, because backticks, a slash or parentheses are the author pointing; a lone bare token does not, because nothing separates `evidence.py` from `Node.js`. The cost is that "Read evidence.py end to end" must become "Read `evidence.py` end to end" or name a second file — and on `main` a bare `evidence.py` was read by nothing at all, so no verdict that passes today is refused by it. A sentence also ends at `!`, `?`, `;` and an ellipsis now, not only at `.`, and **`checked` reads a filename too** — the incumbent clause still died at the first period, so "Checked evidence.py" was refused by exactly the punctuation pedantry this change is about. Re-measured across 1,421 verdicts: **zero** that pass on `main` are refused, 96 are recovered, and of the 19 given up against the looser round, **17 are the default `Verdict: LGTM / Findings: - none` template** — the shape #926 exists to refuse. Five rounds of gate review, every finding real.
  - **Three shapes it still accepts, recorded as tests rather than chased.** Two product names in one sentence corroborate each other; so does English written with underscores ("the code_quality and test_coverage are maintained"); and a missing space forges a dotted token ("the whole diff.See cli.py"). Each needs a list — of products, of English words, of hostnames — and every such list needs a new entry for the next one. A period-plus-capital rule was written to close the third and **reverted**: measured, it truncated `module.Class` and every bulleted object, so `Traced module.Class …` stopped matching the clause at all. It closed one shape and broke two. The line is drawn at the shape instead — two tokens, or one the author pointed at — and none of the three appears in any of the 1,421 verdicts measured. This gate refuses rubber stamps; it does not resist an author working to defeat it, and anyone willing to type a backtick always satisfies it.
  - **The verb widening is not shipped, because it turned out to decide nothing.** #1106 set out to add `traced|read|ran|inspected|verified` beside `checked` as ways to open a clean review. They could not keep `checked`'s free-form object — "Read the whole diff and everything looks correct" is the #926 receipt with a synonym at the front — so their object was required to name something. But that is the test the whole prose already takes, and the object is part of the prose: across all 1,421 verdicts the branch decided **zero**. It is removed rather than shipped as dead code with a docstring explaining what it did. `checked` keeps its free-form object, which 35 verdicts depend on. Widening the vocabulary needs the object judged by something other than the prose test, and this change did not find it — recorded here so the next attempt does not rediscover the same dead end.
