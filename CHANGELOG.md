...
### Added
- **`keel merge` and `keel verify-merge` run where GraphQL is blocked** (#1175). Both read the pull request through `gh pr view --json` and merge through `gh pr merge`, which go over GitHub's **GraphQL** endpoint. On a host whose egress proxy serves REST and blocks GraphQL — the remote environment #1169, #1170 and #1171 were shipped from — every one of those failed before the merge claim, the window re-check, the CI rollup read or the evidence verification had run at all, and the operator was pushed off *"the only merge path is `keel merge` at s10"* onto a hand-driven squash with `keel window` and `evidence-verify` run around it by hand; the post-merge drift check never ran at all, since its two reads are `gh pr view --json` too. The same questions now go over REST when they have to: the pull request, its rollup (assembled from `check-runs` and `statuses` and translated into the shape the reducer already reads — GraphQL spells those SCREAMING_CASE where REST spells them lower, and the difference *is* the case), its file list, the merge itself, and the drift check's window and overtaking set. Two REST shapes are read the way the endpoints actually answer, not the way they read at a glance: `commits/<sha>/check-runs` returns an **object** (`{total_count, check_runs}`) rather than an array, so a head with more than a hundred checks arrives as `{…}{…}` and reading that as a list of check runs produced two entries with no name, no status and no conclusion — neither failing nor pending, so the reducer counted them as checks that had reported and returned **pass** for a head whose real checks it never read. And the REST merge window requires all four of its fields, exactly as the GraphQL reader does: an unmerged pull request comes back with `created_at` and `base.ref` and a null `merged_at`, so "any field present" reported a window for one. REST's `merge_commit_sha` is not GraphQL's `mergeCommit.oid` either: GitHub fills it with the *speculative test-merge* SHA while the pull request is open and replaces it once the merge lands, so "merged and the field is filled" is true on the first post-merge read while the cached test SHA is still being served — the window read waits until that SHA is **reachable from the base branch**, and the merge prefers the SHA its own response returns. A commit status carries its verdict in `state`, which the rollup reducer does not read — so the half that reports non-Actions CI is translated into the `status`/`conclusion` pair it does read, without which a **failing** Jenkins status counted as a check that had reported and could only ever turn `no-checks` into `pass`. Either half being unreadable refuses rather than reading as an empty one, since an empty rollup is a state the docs-only carve-out is allowed to merge through. **Only the wire changes.** The claim, the window, the rollup semantics, the evidence contract, the SHA-pinned gates-pass and `MERGED` as the authoritative outcome are the same objects either way, and `--transport auto|graphql|rest` plus a recorded `transport: gh-graphql | gh-rest` say which one answered. The probe runs **after** a failed read rather than before every run: a host that can reach GraphQL must cost exactly what it cost before, and asking first would put an API call on every merge everywhere to answer a question nearly all of them answer the same way — but it is still the probe that decides, because a failed `gh pr view --json` says nothing on its own (no such PR, no auth, a rate limit and a blocked endpoint all exit non-zero) and matching on `gh`'s wording to tell them apart is the kind of guess that silently re-routes a merge. The merge is never the call that discovers it: the transport is settled by the reads, before anything is written, so no pull request can be merged twice by being retried over a second wire. The REST merge is the **stricter** of the two — it sends `sha=<the head the gates-pass was checked against>`, which `gh pr merge` does not pin by default. Two REST-only hazards are handled rather than inherited: `gh api --paginate` concatenates one JSON document per page instead of merging them, so a two-page rollup is `[…][…]` and `json.loads` rejects it outright — read as a syntax error that would have reported *no checks*, which the merge gate reads as "CI has not run"; and REST cannot sort pull requests by merge time, so the overtaking page is ordered by `updated` descending and #937's truncation rule is re-derived on that key rather than transplanted — merging a pull request updates it, so `merged_at <= updated_at`, and a page whose oldest row was updated before the window opened has seen the window whole.
