...
## 2024-05-24 - SSRF risk in urlopen
**Vulnerability:** urllib.request.urlopen follows HTTP redirects by default, enabling SSRF if the endpoint redirects to an internal IP.
**Learning:** Safe fetch operations in Python require an explicit OpenerDirector that omits HTTPRedirectHandler.
**Prevention:** Always construct a custom OpenerDirector with only HTTPHandler, HTTPSHandler, HTTPErrorProcessor, and HTTPDefaultErrorHandler when fetching untrusted URLs.
