...
## 2025-02-21 - Transient aria-label re-capture in copy buttons
**Learning:** Reading a button's aria-label inside a click handler without an early return allows rapid double-clicks to permanently capture and restore transient states like "Copied to clipboard", breaking the accessible name.
**Action:** Always capture the original semantic label outside the click handler (or use a re-entrancy guard) and use a tracked timeout to ensure the original name is faithfully restored.
