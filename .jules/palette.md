...
## 2026-09-20 - An `aria-label` replaces the content, so hiding a glyph under one changes nothing
**Learning:** The 2026-06-25 entry treats "an `aria-label` or text content" as one case. They are opposites. A button named *from its content* carries every glyph in that content into its name, so a decorative `✓` is worth hiding. A button with an `aria-label` takes its name from the label and its content is never consulted (accname step 2C ends the computation before 2F), and a button's children are presentational, so they are not exposed on their own either. keel#1238 wrapped the `✓` of "Copied! ✓" in `aria-hidden` on two buttons whose handlers set `aria-label="Copied to clipboard"` on the very next line: the accessibility tree read `button "Copied to clipboard"` before and after. The only real change was `textContent` → `innerHTML` in two handlers.
**Action:** Before hiding anything inside a control, read its accessible name in the browser's accessibility tree and find out which of the two names it. Under an `aria-label` there is nothing to fix. Under a name from content, hide the glyph — built with `createElement` and `textContent`, never by switching a handler from `textContent` to `innerHTML`.
