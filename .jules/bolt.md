...
## 2024-10-24 - Optimize Regex Matching with | (OR) operator
**Learning:** Checking a string against multiple regex patterns by condensing them into a single pattern using the | (OR) operator is significantly faster than evaluating them individually via multiple re.search calls or any() generator expressions. Caching the output of re.escape() avoids redundant function calls.
**Action:** Combine multiple regex checks into a single pattern using | where applicable to reduce overhead.
