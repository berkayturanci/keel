...
## 2026-09-09 - Regex compilation for multi-glob matching is not always worth it
**Learning:** Compiling multiple `fnmatch` globs into a single regex pattern using the `|` (OR) operator is technically faster than iteratively evaluating multiple globs in a loop with `fnmatch.fnmatch()` on a synthetic benchmark. However, `fnmatch.translate()` does not apply `os.path.normcase`, meaning it alters matching semantics on Windows (failing to case-fold and normalize slashes). Furthermore, on cold paths (like evaluating a PR's changed files against configuration globs), the absolute execution time saved is negligible (e.g. microseconds), rendering the optimization pointless and risky.
**Action:** Always evaluate the absolute performance savings on real input sizes before applying an optimization. Do not optimize string matching loops using `fnmatch.translate` if Windows compatibility is required and the loop is only executed on small inputs.
