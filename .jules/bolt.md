...
## 2024-08-01 - Condensing regex patterns and caching escaped variables
**Learning:** Checking a string against multiple regex patterns by condensing them into a single pattern using the `|` (OR) operator is significantly faster than evaluating them individually via multiple `re.search` calls or `any()` generator expressions. Also, caching the escaped string in a local variable avoids redundant function calls.
**Action:** When evaluating multiple regex patterns, use string concatenation with `|` instead of loops or comprehensions, and cache regex sanitized variables.
