...
## 2026-09-17 - Premature regex combination optimization on cold paths
**Learning:** Combining multiple related regex patterns using the `|` (OR) operator into a single cached regex evaluates significantly faster than iterating through them via a generator expression. However, applying this optimization on cold paths (e.g. `rate_limited` or `vendor_timed_out` handlers which execute only once per failed delegate) yields virtually zero overall performance benefit. Furthermore, converting readable tuples of patterns into one complex string sacrifices the ability to include explanatory per-pattern comments.
**Action:** When considering regex optimizations, verify that the patterns are being evaluated in a hot path or loop before refactoring. Do not sacrifice code readability or inline documentation (like per-pattern explanations) for micro-optimizations that provide no measurable aggregate impact.
