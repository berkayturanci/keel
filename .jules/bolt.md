...
## 2024-06-27 - [Optimize deduplication using dict.fromkeys]
**Learning:** Python 3.7+ guarantees insertion order for dicts. Using `list(dict.fromkeys(iterator))` is significantly faster for preserving sequence order and deduplicating lists compared to manual loops with a `seen` set because it executes in C at the interpreter level.
**Action:** Always prefer `list(dict.fromkeys())` over manual `seen = set()` loops when deduplicating elements in Python lists while preserving order.
