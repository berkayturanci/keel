...
#: have that problem.
# ⚡ Bolt Optimization: Combined multiple regular expressions into a single compiled pattern
# using the | (OR) operator for faster evaluation.
...
def rate_limited(text: str) -> bool:
    ...
    return bool(_RATE_LIMIT_MARKERS.search(lowered))
...
def vendor_timed_out(text: str) -> bool:
    ...
    """
    lowered = (text or "").lower()
    return bool(_VENDOR_TIMEOUT_MARKERS.search(lowered))
