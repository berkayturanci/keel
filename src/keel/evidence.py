...
#:
#: The path anchors below read *any* one-to-five-character extension, because a
#: directory has already proved the token is a path. With no directory the
#: extension carries that proof alone, so it has to name a kind of file rather
#: than be any three letters after a dot: ``github.com``, ``pypi.org`` and
#: ``claude.ai`` are the same shape as ``config.py`` and differ only in what
#: follows the dot. The rule therefore runs in the direction that closes —
#: listing the file types a review can name — and not in the direction that
#: never does. A list of hostnames to refuse cannot be finished, because anyone
...
#: capitals, or a capitalised segment. **A dot on its own is not evidence** —
#: it is also how a hostname is spelled, and reading every dotted pair let the
#: ``claude.ai`` in a "Generated by Claude Code" footer anchor a verdict that
#: pointed at nothing. ``cache.cache_key``, ``JuryConfig.__post_init__`` and
#: ``subprocess.DEVNULL`` all keep a mark; ``github.com`` and ``pypi.org`` have
#: none. What this cannot separate is an all-lowercase ``Class.method`` —
#: ``str.endswith`` reads exactly like ``jury.dev`` — so those are given up
#: rather than bought with a blocklist; backticks still carry them.
#:
...
#: symbol, a source file, or a dotted identifier — called or not (#1106).
#: Presence of *structure*, never a judgement about whether the review was good
#: — the same line ai-jury's ``emitted_findings_block()`` draws.
#:
#: The dotted form used to require the parentheses, and that refused a real
#: review for its punctuation: a verdict that traced ``cache.cache_key``,
#: ``config.config_hash`` and ``JuryConfig.__post_init__`` was told it "names
#: nothing concrete", because writing ``cache.cache_key()`` for something you
#: are not calling is worse prose and the reviewer declined to (ai-jury#753).
#: The called form stays as its own pattern only because it also reads
#: single-character segments, which the general one deliberately does not:
#: nothing that anchored a verdict before this change stops anchoring one.
_VERDICT_ANCHORS = (
...
#: A **bare** identifier: ``snake_case``, ``_private``, ``__dunder__``. Weaker
#: evidence than a backticked or dotted one, so it anchors only when several
#: *distinct* ones appear together — the shape of a review that walked a call
#: graph, and the shape a rubber stamp does not have.
#:
#: **The underscore is the whole rule.** It is a mark English prose and product
#: names do not use, and it is the only lexical difference between a symbol and
#: a capitalised noun: ``GitHub`` and ``GitLab`` are CamelCase in exactly the
#: way ``JuryConfig`` is, so reading CamelCase let "The GitHub and GitLab side
#: of this is unchanged" clear a floor of two while naming nothing in the
#: change. No pattern can separate those two, and a list of product names to
#: refuse would need a new entry every time a reviewer mentions a new product.
#: CamelCase symbols are still read everywhere they are written with a dot or
#: in backticks, which is how a class is normally named.
#:
#: Measured over every verdict posted across keel and ai-jury: a floor of one
#: readmits 22 of the 75 ``Reviewed <title>: <affirmation>`` stamps #926 is
#: named for, a floor of two admits none of them.
_VERDICT_BARE_IDENTIFIER = re.compile(r"\b_*[a-z0-9]+(?:_[a-z0-9]+)+_*\b")
_VERDICT_BARE_IDENTIFIER_FLOOR = 2
...
def _names_something(text: str) -> bool:
    """Whether ``text`` points at anything at all — one anchor, or one identifier.

    The floor is *one* here, where :data:`_VERDICT_BARE_IDENTIFIER_FLOOR` is two,
    ...
    return any(pattern.search(text) for pattern in _VERDICT_ANCHORS) or bool(
        _VERDICT_BARE_IDENTIFIER.search(text)
    )
...
def verdict_substance(body: str, *, pr_title: str = "") -> tuple[bool, str]:
    ...
    * **An anchor.** A path, a ``path:line``, a backticked symbol, a source
      file, a dotted identifier, or several distinct ``snake_case`` ones — or an
      act-of-review clause naming what was looked at, because a genuinely clean
    ...
    * **Novelty against the title.** Prose that is substantially the PR title
    ...
    anchored = any(pattern.search(prose) for pattern in _VERDICT_ANCHORS) or (
        len(set(_VERDICT_BARE_IDENTIFIER.findall(prose))) >= _VERDICT_BARE_IDENTIFIER_FLOOR
    )
