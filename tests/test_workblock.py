...
class TestWorkBlockContract(unittest.TestCase):
    def test_daytime_contract_shape(self):
        ...
        self.assertEqual(contract["queue"]["accepted_inputs"],
                         ["explicit_issue_numbers", "queue_selector"])
        ...
        self.assertTrue(
            contract["failure_policy"]["daytime_operator_can_redirect_between_items"]
        )
        self.assertEqual(contract["final_report"]["outcome_buckets"],
                         list(workblock.OUTCOME_BUCKETS))
