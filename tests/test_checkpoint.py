...
class TestCheckpointRecords(unittest.TestCase):
    def test_schema_stability_and_io(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(
                checkpoint.parse_checkpoint(checkpoint.encode_checkpoint(record)), record
            )
...
class TestResumeObservesReality(unittest.TestCase):
    ...
    def test_pending_without_a_pull_request_is_not_ambiguous(self):
        ...
        plan = checkpoint.resume_plan_as_dict(self._rec(merge_state="pending", pull_request=None))
    ...
    def test_a_checkpoint_without_a_head_says_nothing(self):
        plan = checkpoint.resume_plan_as_dict(self._rec(head_sha=None), live_head_sha="b" * 40)
        self.assertEqual(plan["warnings"], [])
