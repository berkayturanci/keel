...
- **Bump the version with one command:**
  ...
  - `.claude-plugin/plugin.json` — **not** regenerated by `make plugin`; a stale value fails
    `test_plugin_manifest_version_matches_keel_version`,
  - the pinned-install `keel@v<version>` references in `README.md`, `website/index.html`, and
