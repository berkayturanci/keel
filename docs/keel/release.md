...
- **Bump the version with one command:**
  ...
  - every per-agent plugin manifest — `.claude-plugin/`, `.codex-plugin/`,
    `.cursor-plugin/` — **not** regenerated by `make plugin`. A stale value fails
    `test_release_docs.py::test_no_release_surface_carries_a_stale_version_string`, which
    reads the whole `RELEASE_SURFACES` table; the per-manifest
