...
bracket pair, the backslash, the angle brackets, the emphasis delimiters, the ampersand and
the backtick, so `__init__.py` reads as written rather than as a bold `init`.
